...
//! harness implements [`CompactionItem`] for its `HeartQTurn`;
//! heartq-build implements it for `heartq_sampling_types::ConversationItem`.
//!
...
//! `<heartq_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `HeartQRole` (HeartQ chat) and the
/// `ConversationItem` variants (heartq-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (HeartQ chat) — maps to System on harnesses without a
...
/// extraction (`<heartq_file id=".." name=".." />` lines in the
/// `<heartq_user_queries>` preamble).
...
///
/// Implementors:
/// - HeartQ chat: `HeartQTurn`
/// - heartq-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (HeartQ chat's
    /// `HeartQTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (HeartQ chat: a
    ...
    /// strips their `<heartq_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<heartq_file>` lines in the
    /// `<heartq_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (HeartQ chat: a `Developer` turn with category
    ...
    /// requests/results (HeartQ chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up heartq-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by HeartQ chat's
/// `HeartQTurn`; adding these constructors to it as required methods would break
/// that impl. They are also heartq-build-specific (HeartQ chat's tail-keep path
...
///
/// The heartq-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (HeartQ chat stores turns as `Arc<HeartQTurn>`).
