//! Observability seam for the full-replace (heartq-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (heartq-build:
...
//!
//! Emission points are part of the behavior contract: the heartq-build observer
