...
//!
//! The canonical `sample → classify → retry` loop, used by **both** heartq-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and HeartQ chat's intra `Shared` summarizer
...
//! the winning summary (heartq-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the heartq-build host uses to step down its input size).
