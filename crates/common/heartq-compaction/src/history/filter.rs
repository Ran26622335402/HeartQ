...
//!
//! Behavior is byte-for-byte identical for HeartQ chat (`T = Arc<HeartQTurn>`).
...
///   summaries so their `<heartq_user_queries>` sections can be split out.
...
/// Extracts **all** `<heartq_user_queries>...</heartq_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<heartq_user_queries>";
    let end_tag = "</heartq_user_queries>";
...
/// Extract a `<heartq_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<heartq_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<heartq_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<heartq_user_queries>\n");
    ...
    result.push_str("</heartq_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<heartq_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<heartq_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<heartq_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<heartq_user_queries>` block.
...
/// `<heartq_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<heartq_user_queries>` from earlier rounds.
...
///   would see the prior `<heartq_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<heartq_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<heartq_user_queries>"));
        assert!(out.ends_with("</heartq_user_queries>"));
        assert!(out.contains("<heartq_query>hello world</heartq_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<heartq_user_queries>\n<heartq_query>first</heartq_query>\n</heartq_user_queries>";
        let inner2 = "<heartq_user_queries>\n<heartq_query>second</heartq_query>\n</heartq_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<heartq_user_queries>\n<heartq_query>Q1</heartq_query>\n</heartq_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<heartq_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<heartq_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries = "<heartq_user_queries>\n<heartq_query>Q</heartq_query>\n</heartq_user_queries>";
