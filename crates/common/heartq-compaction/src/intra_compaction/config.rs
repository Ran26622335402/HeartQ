...
use crate::precompaction::ToolPrunerConfig;

/// Which targets intra-compaction may compact.
///
/// - `FullReplace` (default): heartq-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from heartq-shell's
...
/// time (token threshold only, matching heartq-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as heartq-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<heartq_user_queries>` preamble prepended to the history compaction
    ...
    /// [All modes] Configuration for tool result pruning before compression.
    /// When `Some(config)`, tool results are deduplicated, summarized, and
    /// truncated before the LLM summarization pass to reduce token count.
    /// Default: `None` (pruning disabled).
    pub tool_pruner: Option<ToolPrunerConfig>,

    // ─────────────── Hermes integration seams (optional) ───────────────
    //
    // The following fields expose the Hermes-style compression hardening
    // gates. They are **opt-in**: defaults preserve the prior behaviour
    // exactly. Hosts that want the full Hermes experience populate these
    // from their agent spec / config YAML.

    /// [Optional] When `Some(db)`, intra-compaction acquires a
    /// [`crate::lock::CompressionLock`] keyed by `session_id` before
    /// running. Acquisition is **best-effort**: if another process holds
    /// the lock, the compaction is skipped (with a `warn!`) instead of
    /// blocking or erroring. When `None` (default), no lock is consulted.
    ///
    /// Default: `None` (lock disabled).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compression_lock_db: Option<std::path::PathBuf>,

    /// [Optional] Session id used as the lock key for `compression_lock_db`.
    /// When both this and `compression_lock_db` are set, the lock is
    /// acquired against this id. When unset, no lock is attempted even if
    /// `compression_lock_db` is set.
    ///
    /// Default: `None`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compression_session_id: Option<String>,

    /// [Optional] When `true`, intra-compaction consults a
    /// [`crate::state::CompressionState`] before each pass and skips when
    /// `is_blocked()` returns `true` (cooldown / ineffective-streak /
    /// fallback-streak guard). The state object is owned by the caller
    /// (typically the agent's per-session compression telemetry), not by
    /// this config struct — see `apply_intra_compaction` for the seam.
    ///
    /// Default: `false`.
    #[serde(default)]
    pub anti_thrash_enabled: bool,

    /// [Optional] When `true`, intra-compaction builds the LLM summarization
    /// prompt via [`crate::inter_compaction::EnhancedSummaryTemplate`]
    /// (Hermes-style segmented template: 9 sections, anti-hallucination
    /// prefix, end-of-summary marker). When `false`, the legacy
    /// `build_prompt_for_target` prompt builder is used.
    ///
    /// Default: `false`.
    #[serde(default)]
    pub use_enhanced_summary: bool,
}
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "heartq-4.20";
...
impl Default for IntraCompactionConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            // Pre-compaction pruning (Hermes ToolResultPruner; on by default)
            tool_pruner: Some(ToolPrunerConfig::default()),
            // Hermes integration seams (disabled by default)
            compression_lock_db: None,
            compression_session_id: None,
            anti_thrash_enabled: false,
