...
pub enum CompactionTarget {
    ...
    /// with a single summary — heartq-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by HeartQ chat's
/// `StreamProcessor` (`Item = Arc<HeartQTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<HeartQTurn>`) already satisfies it.
