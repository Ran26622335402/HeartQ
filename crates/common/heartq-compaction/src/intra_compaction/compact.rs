...
// The `Shared` summarizer reuses heartq-build's full-replace summarization core
...
use crate::precompaction::{PruneStats, ToolResultPruner};
...
where
    T: CompactionItemBuilder + Send + Sync + AsRef<str>,
...
/// Hermes-style hardening wrapper: same as [`apply_intra_compaction`] but
/// optionally consults a per-session [`CompressionState`] (anti-thrash)
/// and acquires a [`crate::lock::CompressionLock`] (cross-process
/// mutual exclusion) before running the mode-specific body.
///
/// Both gates are **opt-in**:
///
/// - `compression_state` — when `Some(_)`, the gate fires only if
///   `policy.anti_thrash_enabled` is also `true`. The state is mutated
///   on success/failure of the underlying compaction so callers can
///   observe the outcome.
///
/// - The `compression_lock_db` / `compression_session_id` fields on
///   `policy` together form the lock key. When both are set, the lock is
///   acquired for the duration of this call and released on return
///   (best-effort; on contention we `warn!` and skip; on hard failure we
///   `warn!` and proceed without a lock).
pub async fn apply_intra_compaction_with_gates<T, S, P>(
    stream_proc: &S,
    sampler: &P,
    policy: &IntraCompactionConfig,
    trigger: IntraCompactionTrigger,
    ...
    observer: &dyn IntraCompactionObserver,
    active_reminder: Option<&str>,
    compression_state: Option<&mut crate::state::CompressionState>,
) -> Result<IntraCompactionResult, IntraCompactionError>
where
    T: CompactionItemBuilder + Send + Sync + AsRef<str>,
    S: CompactionStreamProc<Item = T> + ?Sized,
...
{
    // ───── Gate 1: anti-thrash ─────
    if policy.anti_thrash_enabled {
        // Borrow-immutably for the read-only `is_blocked()` check; we
        // borrow-mutably later for telemetry writes.
        if let Some(state) = compression_state.as_deref() {
            if state.is_blocked() {
                tracing::debug!(
                    "[IntraCompaction] skipped by anti-thrash state (cooldown / ineffective streak)"
                );
                return Ok(IntraCompactionResult::default());
            }
        }
    }

    // ───── Gate 2: cross-process lock ─────
    //
    // Best-effort. Any failure mode (db open, init table, acquire)
    // produces a `warn!` log and we proceed without a lock; the only
    // hard exit is when `try_acquire` reports "held by another process",
    // which causes us to skip this turn entirely (the lock holder will
    // finish before the lock TTL expires).
    match try_acquire_policy_lock(policy) {
        LockGateOutcome::NoLock => {} // proceed without lock
        LockGateOutcome::HeldByOther => {
            return Ok(IntraCompactionResult::default());
        }
        LockGateOutcome::Held(_guard) => {
            // _guard drops at function exit, releasing the lock.
        }
    }
    ...
    if let Some(state_ref) = compression_state {
        match &result {
            Ok(r) => {
                if r.tokens_after >= r.tokens_before && r.tokens_before > 0 {
                    // No reduction → ineffective.
                    state_ref.record_ineffective();
                } else {
                    state_ref.record_successful();
                }
            }
            Err(_) => {
                state_ref.force_block();
            }
        }
    }
...
/// `FullReplace` strategy (default): heartq-build's full-replace — summarize the
...
/// `<heartq_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching heartq-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    let mut source_turns = stream_proc.get_all_turns_for_compaction().await;
    if source_turns.is_empty() {
        return Err(IntraCompactionError::NothingToCompact);
    }

    // 1b. Apply pre-compaction tool result pruning if configured.
    //     Runs before token counting so that pruning affects size estimates.
    let _prune_stats = apply_precompaction_pruning(&mut source_turns, policy);
    ...
    // 2. Summarize the whole conversation through heartq-build's shared core.
    ...
    //     Shared with HeartQ chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    let mut source_turns = source_turns;

    // 1b. Apply pre-compaction tool result pruning if configured.
    //     Runs before split-point selection so that pruning decisions
    //     affect which turns get summarized.
    let _prune_stats = apply_precompaction_pruning(&mut source_turns, policy);
    ...
    // 3a. For `History` target, split prior `<heartq_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<heartq_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<heartq_user_queries>` preamble below — stays intra's.
    //
    //     When `policy.use_enhanced_summary` is set, the Hermes-style
    //     [`EnhancedSummaryTemplate::format_prompt_for_turns`] prompt is
    //     used **instead** of the legacy `build_prompt_for_target` for the
    //     `Legacy` branch. The `Shared` branch always uses the shared
    //     summarization core (which already produces structured output).
    let summary_text = match policy.summarizer {
        ...
        IntraSummarizer::Legacy => {
            let prompt = if policy.use_enhanced_summary {
                let user =
                    crate::inter_compaction::EnhancedSummaryTemplate::format_prompt_for_turns(
                        &turns_for_llm,
                    );
                CompactionPrompt {
                    system: String::new(),
                ...
                build_prompt_for_target(target)?
            };
        ...
        // New (default): heartq-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<heartq_user_queries>` preamble so
...
/// — heartq-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and heartq-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // heartq-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // heartq-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on HeartQ chat's
...
/// the literal error messages produced by the HeartQ chat sampler —
/// keep these in sync if either side changes (the
/// `compaction_sample_error_to_intra*` tests below guard the mapping).
/// Three-state outcome of the cross-process lock gate.
enum LockGateOutcome {
    /// No lock configured, or a soft failure — caller proceeds without
    /// a lock. A `warn!` was already emitted if applicable.
    NoLock,
    /// Another process holds the lock — caller should skip this turn
    /// entirely (return `IntraCompactionResult::default()`).
    HeldByOther,
    /// Lock acquired; the guard releases on Drop.
    Held(crate::lock::CompressionLock),
}

fn try_acquire_policy_lock(policy: &IntraCompactionConfig) -> LockGateOutcome {
    let (db_path, sid) = match (
        policy.compression_lock_db.as_ref(),
        policy.compression_session_id.as_ref(),
    ) {
        (Some(p), Some(s)) => (p, s),
        _ => return LockGateOutcome::NoLock,
    };
    ...
    let conn = match rusqlite::Connection::open(db_path) {
        ...
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = ?db_path,
                "[IntraCompaction] failed to open lock db; proceeding without lock"
            );
            return LockGateOutcome::NoLock;
        }
    };

    if let Err(e) = crate::lock::init_lock_table(&conn) {
        tracing::warn!(
            error = %e,
            "[IntraCompaction] failed to init lock table; proceeding without lock"
        );
        return LockGateOutcome::NoLock;
    }

    match crate::lock::CompressionLock::try_acquire(
        &conn,
        sid,
        crate::lock::CompressionLock::DEFAULT_TTL.as_secs(),
    ) {
        Ok(Some(guard)) => {
            tracing::debug!(
                session_id = %sid,
                "[IntraCompaction] acquired compression lock"
            );
            LockGateOutcome::Held(guard)
        }
        Ok(None) => {
            tracing::warn!(
                session_id = %sid,
                "[IntraCompaction] compression lock held by another process; skipping"
            );
            LockGateOutcome::HeldByOther
        }
        Err(e) => {
            tracing::warn!(
                error = %e,
                "[IntraCompaction] failed to acquire compression lock; proceeding without lock"
            );
            LockGateOutcome::NoLock
        }
    }
}
...
/// Apply tool result pruning before compression if configured.
///
/// Returns the prune stats if pruning was applied, or `None` if disabled.
/// Pruning removes redundant tool results, summarizes large outputs, and
/// truncates oversized arguments to reduce token count before LLM summarization.
fn apply_precompaction_pruning<T: CompactionItemBuilder + AsRef<str>>(
    turns: &mut [T],
    policy: &IntraCompactionConfig,
) -> Option<PruneStats> {
    let config = policy.tool_pruner.as_ref()?;

    info!(
        max_result_length = config.max_result_length,
        dedup_window = config.dedup_window,
        min_length_for_summary = config.min_length_for_summary,
        "[IntraCompaction] applying pre-compaction tool pruning"
    );

    let pruner = ToolResultPruner::new(config.clone());
    let stats = pruner.prune(turns);
    ...
    info!(
        ...
        "[IntraCompaction] pre-compaction pruning complete"
    );
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the HeartQ chat sampler when the
    ...
    /// — guards the forwarding that HeartQ chat (`Arc<HeartQTurn>`) relies on.
    #[test]
