...
//!
//! - HeartQ chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `HeartQTurn::get_num_tokens`.
//! - heartq-build estimates with `bytes / 4`.
//!
...
//! (HeartQ chat: `ItemTokenCounter<Arc<HeartQTurn>>`), so exactly one mechanism
//! is in play.
