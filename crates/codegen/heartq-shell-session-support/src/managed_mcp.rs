...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match heartq_http::shared_client()
        .get(url)
...
impl ManagedMcpState {
    ...
    /// connector re-authorized on heartq.com can retry. The reactive path must NOT
...
///
/// Servers with names starting with this prefix are managed by heartq.com —
...
///
/// Examples:
///   `heartq_com_linear`  → managed by heartq.com
///   `heartq_com_slack`   → managed by heartq.com
///   `my_company_api`   → user-managed (local)
///
/// Single source of truth lives in `heartq-workspace` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use heartq_workspace::permission::resolution::MANAGED_MCP_PREFIX;

/// `"Linear"` -> `"heartq_com_linear"`. Shares normalization and the
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    heartq_shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
...
/// 1. Server name starts with `MANAGED_MCP_PREFIX` ("heartq_com_")
/// 2. Server URL matches a managed config endpoint
///
/// This prevents false injection if a user accidentally names a server `heartq_com_*`
...
/// Inject managed OAuth headers into `heartq_com_`-prefixed MCP servers.
///
...
mod tests {
    ...
    fn reauth_first_attempt_allowed_then_backs_off() {
        let mut state = ManagedMcpState::default();
        let now = Utc::now();
        assert!(state.reauth_allowed("heartq_com_slack", now));
        assert!(!state.reauth_is_terminal("heartq_com_slack"));

        state.record_reauth_failure("heartq_com_slack", now);
        // 2^1 = 2s backoff: not eligible now, eligible after the window.
        assert!(!state.reauth_allowed("heartq_com_slack", now));
        assert!(state.reauth_allowed("heartq_com_slack", now + chrono::Duration::seconds(2)));
        assert!(!state.reauth_is_terminal("heartq_com_slack"));
    }
    ...
    fn reauth_goes_terminal_after_max_attempts() {
        ...
        assert!(!state.reauth_allowed("heartq_com_slack", now + chrono::Duration::seconds(3600)));
    }
    ...
    fn reauth_success_resets_cooldown() {
        ...
        state.record_reauth_success("heartq_com_slack");
        assert!(state.reauth_allowed("heartq_com_slack", now));
        assert!(!state.reauth_is_terminal("heartq_com_slack"));
    }
    ...
    fn complete_fetch_preserves_cooldown_clear_resets_it() {
        ...
        let now = Utc::now();
        {
            ...
            for _ in 0..MAX_REACTIVE_REAUTH_ATTEMPTS {
                state.record_reauth_failure("heartq_com_slack", now);
                state.record_reauth_failure("heartq_com_linear", now);
            }
            assert!(state.reauth_is_terminal("heartq_com_slack"));
            assert!(state.reauth_is_terminal("heartq_com_linear"));
            ...
            assert!(state.reauth_allowed("heartq_com_slack", now));
            assert!(state.reauth_allowed("heartq_com_linear", now));
            assert!(!state.reauth_is_terminal("heartq_com_slack"));
            assert!(!state.reauth_is_terminal("heartq_com_linear"));
        }
    }
