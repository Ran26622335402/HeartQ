...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `heartq-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `heartq-shell::session::events`.
    ...
    /// in `heartq-shell::session::events`.
    ...
    /// discriminator constants in `heartq-shell::session::events`.
    ...
    /// `heartq-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `heartq-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `heartq-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `heartq-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `heartq-shell`'s `GoalPauseReason`. The two types
