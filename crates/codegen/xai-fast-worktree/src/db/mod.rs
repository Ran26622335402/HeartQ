...
impl WorktreeDb {
    /// Open (or create) the DB at `heartq_home/worktrees.db`.
    pub fn open(heartq_home: &Path) -> Result<Self> {
        Self::open_at(&heartq_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.heartq/worktrees.db`.
    ///
    /// Discovers heartq home via `$HEARTQ_HOME`, falling back to the canonicalized
    /// `$HOME/.heartq` (matching `heartq_config::heartq_home`).
...
pub fn resolve_heartq_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("HEARTQ_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $HEARTQ_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .heartq
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with heartq_config::default_heartq_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".heartq"))
}

/// Serializes tests that mutate the process-global `HEARTQ_HOME` env var so they
...
#[cfg(test)]
static HEARTQ_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`HEARTQ_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `HEARTQ_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `HEARTQ_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct HeartQHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated heartq home; pass to `WorktreeDb::open` to read the same DB
...
#[cfg(test)]
impl HeartQHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = HEARTQ_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("heartq-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via HEARTQ_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until HEARTQ_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("HEARTQ_HOME");
        unsafe { std::env::set_var("HEARTQ_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for HeartQHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("HEARTQ_HOME", p),
                None => std::env::remove_var("HEARTQ_HOME"),
            }
        }
    }
}
