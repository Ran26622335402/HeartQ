...
pub use heartq_config::heartq_home;

/// Path to `$HEARTQ_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    heartq_home().join("pager.toml")
}

/// User-facing label for the user heartq directory (``~/.heartq`` or ``$HEARTQ_HOME``).
///
/// Derived from resolved [`heartq_home()`] vs `heartq_config::default_heartq_home()`,
/// not from whether `HEARTQ_HOME` is set in the environment.
pub fn display_heartq_home_prefix() -> String {
    if heartq_home() == heartq_config::default_heartq_home() {
        "~/.heartq".to_string()
    } else {
        "$HEARTQ_HOME".to_string()
    }
}

/// User-facing path under [`heartq_home()`], e.g. ``~/.heartq/config.toml``.
pub fn display_user_heartq_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_heartq_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`heartq_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let heartq = heartq_home();
    if let Ok(rest) = path_buf.strip_prefix(&heartq) {
        let prefix = display_heartq_home_prefix();
...
/// True when `path` is under user [`heartq_home()`] (not project `{cwd}/.heartq`).
pub fn is_under_user_heartq_home(path: &Path) -> bool {
    path.starts_with(heartq_home())
}
...
mod tests {
    ...
    #[test]
    fn display_heartq_home_prefix_default_install() {
        if std::env::var("HEARTQ_HOME").is_ok() {
            return;
        }
        assert_eq!(display_heartq_home_prefix(), "~/.heartq");
    }

    #[test]
    fn display_user_heartq_path_joins_relative() {
        let path = display_user_heartq_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".heartq") || path.contains("$HEARTQ_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_heartq() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.heartq/memory/MEMORY.md");
