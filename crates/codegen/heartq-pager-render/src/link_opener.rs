...
//!
//! Extracted from the `OpenSuperheartqUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("HEARTQ_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "HEARTQ_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
    ...
    heartq_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// Used by the SuperHeartQ upsell flow to attribute clicks to `referrer=heartq-build`,
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/heartq session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://heartq.com/superheartq", "referrer", "heartq-build");
        assert_eq!(out, "https://heartq.com/superheartq?referrer=heartq-build");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://heartq.com/superheartq?referrer=other",
            "referrer",
            "heartq-build",
        );
        assert_eq!(out, "https://heartq.com/superheartq?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://heartq.com/superheartq?heavy=1",
            "referrer",
            "heartq-build",
        );
        assert_eq!(
            out,
            "https://heartq.com/superheartq?heavy=1&referrer=heartq-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`heartq.com/#superheartq`); we still want the referrer attached.
        let out = ensure_query_param("https://heartq.com/#superheartq", "referrer", "heartq-build");
        assert_eq!(out, "https://heartq.com/?referrer=heartq-build#superheartq");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "heartq-build");
    ...
    fn browser_unavailable_message_includes_full_url() {
        let url = "https://heartq.com/superheartq?referrer=heartq-build";
