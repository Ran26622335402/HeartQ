...
//!
//! In dev mode, watches ~/.heartq/pager.toml for changes and hot-reloads.
...
///
/// In dev mode: reads from ~/.heartq/pager.toml, watches for changes.
...
impl ConfigWatcher {
    ...
    ///
    /// - In dev mode: reads/creates ~/.heartq/pager.toml, watches for changes
    ...
    fn start_static() -> io::Result<Self> {
        let config = heartq_config::user_heartq_home()
