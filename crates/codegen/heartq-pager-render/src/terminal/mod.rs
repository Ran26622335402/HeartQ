...
pub enum TerminalName {
    ...
    JetBrains,
    /// HeartQ Desktop (Electron app).
    #[strum(to_string = "HeartQ Desktop")]
    HeartQDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` heartq is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches heartq, which
    ...
    pub fn telemetry_snapshot(&self) -> heartq_telemetry::events::TerminalTelemetry {
        ...
        heartq_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: heartq_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> heartq_shared::session::FeedbackTerminalInfo {
        use heartq_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(heartq_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `heartq ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `heartq-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "heartqdesktop" => Some(TerminalName::HeartQDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.heartq/pager.toml` and
