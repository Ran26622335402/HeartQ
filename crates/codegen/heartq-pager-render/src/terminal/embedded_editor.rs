//! Detects whether heartq is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` heartq is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `heartq-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
