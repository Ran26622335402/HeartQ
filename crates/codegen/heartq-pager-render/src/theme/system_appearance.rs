...
/// in `config.toml`. When `None`, defaults to `HeartQNight` / `HeartQDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::HeartQDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::HeartQNight),
    }
}
