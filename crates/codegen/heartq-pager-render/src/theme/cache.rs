...
//! lookup must be cheaper than re-loading from `~/.heartq/config.toml`.
...
//! `heartq_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::HeartQNight as u8);
...
/// back to `HeartQNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::HeartQNight as u8 => ThemeKind::HeartQNight,
        x if x == ThemeKind::HeartQDay as u8 => ThemeKind::HeartQDay,
...
/// When `None`, `to_theme_kind()` defaults to `HeartQNight` / `HeartQDay`.
...
///
/// On the first call, reads from `~/.heartq/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    heartq_markdown::set_color_level_cap(if locked {
        heartq_markdown::ColorLevel::Basic
    } else {
        heartq_markdown::ColorLevel::TrueColor
    });
    heartq_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`HEARTQ_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `HeartQNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::HeartQNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `HeartQNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `heartq_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = heartq_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = heartq_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::HeartQNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::HeartQDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::HeartQNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::HeartQDay);
            assert_eq!(applied, ThemeKind::HeartQNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::HeartQNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::heartqday().text_primary,
                "must not serve the cached (HeartQDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!heartq_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                heartq_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // heartqday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::HeartQDay), false),
                ThemeKind::HeartQDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::HeartQNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_heartqnight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::HeartQNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_heartqday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::HeartQDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_heartqnight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_heartqnight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::HeartQDay), true);
            assert_eq!(result, ThemeKind::HeartQDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::HeartQDay);
            assert_eq!(current_kind(), ThemeKind::HeartQDay);
        });
    }
