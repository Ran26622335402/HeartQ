...
//! The default theme is HeartQNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! HeartQNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::HeartQNight, ThemeKind::HeartQDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::HeartQNight => "heartqnight",
            Self::TokyoNight => "tokyonight",
            Self::HeartQDay => "heartqday",
    ...
    /// when quantized to 256 or 16 colors. HeartQNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::HeartQNight => false,
            Self::TokyoNight => true,
            Self::HeartQDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "heartqnight" | "heartq-night" | "dark" => Some(Self::HeartQNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "heartqday" | "heartq-day" | "light" | "day" => Some(Self::HeartQDay),
...
/// `"heartqnight"` → `"HeartQ Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "heartqnight" => "HeartQ Night",
        "heartqday" => "HeartQ Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.heartq/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::HeartQNight => Self::heartqnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::HeartQDay => Self::heartqday(),
            ...
            // if reached, fall back to HeartQNight.
            ThemeKind::Auto => Self::heartqnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `HEARTQ_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on heartqnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., HeartQNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // HeartQNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    heartq_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::HeartQNight.is_auto());
        assert!(!ThemeKind::HeartQDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::heartqday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::heartqnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::heartqday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. HeartQNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::HeartQNight => Theme::heartqnight(),
                ThemeKind::HeartQDay => Theme::heartqday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_heartqnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark HeartQNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::heartqnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("heartqnight"),
            Some(ThemeKind::HeartQNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::HeartQNight));
        assert_eq!(ThemeKind::from_name("heartqday"), Some(ThemeKind::HeartQDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::HeartQDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("heartqnight", ThemeKind::HeartQNight),
            ("heartq-night", ThemeKind::HeartQNight),
            ("dark", ThemeKind::HeartQNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("heartqday", ThemeKind::HeartQDay),
            ("heartq-day", ThemeKind::HeartQDay),
            ("light", ThemeKind::HeartQDay),
            ("day", ThemeKind::HeartQDay),
