...
//! Dark themes (HeartQNight, TokyoNight) share `heartq-night.tmTheme`;
//! HeartQDay uses `heartq-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `HeartQNight` (so leftover kind-keyed paths
...
pub use heartq_markdown::Syntect;
...
static SYNTECT_HEARTQNIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_HEARTQDAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `HeartQNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_HEARTQNIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/heartq-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::HeartQDay => SYNTECT_HEARTQDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/heartq-day.tmTheme"))),
    }
}
