...
use heartq_mcp::servers::McpState;
use heartq_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use heartq_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<heartq_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<heartq_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<heartq_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<heartq_hooks::error::HookError>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<heartq_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn heartq_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `HEARTQ_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$HEARTQ_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<heartq_home>/workspace` by default). The upload queue spills here.
    ...
    /// (`HEARTQ_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    ...
    /// enabled (`HEARTQ_WORKSPACE_TOOL_DEFS_ENABLED=true`).
...
impl WorkspaceShared {
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<heartq_workspace_types::WorkspaceEvent> {
