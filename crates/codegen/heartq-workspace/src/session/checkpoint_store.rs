...
/// Directory (under `<cwd>/.heartq`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.heartq/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
