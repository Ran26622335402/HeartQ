...
//!
//! Reads OIDC credentials from `~/.heartq/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use heartq_workspace::config::WorkspaceServerMetadata;
use heartq_workspace::daemonize;
use heartq_workspace::diag_server;
use heartq_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
const SERVICE_NAME: &str = "prod_heartq_workspace";
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.heartq.com/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `HEARTQ_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "HEARTQ_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.heartq/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "HEARTQ_WORKSPACE_PROJECT_LSP_TRUSTED",
    ...
    /// Override with `HEARTQ_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "HEARTQ_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/heartq-preview-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("HEARTQ_WORKSPACE_OTLP_ENDPOINT") {
    ...
    {
        use heartq_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("HEARTQ_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized HEARTQ_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if heartq_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via HEARTQ_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = heartq_sandbox::is_active();
            ...
            tracing::info!(
                ...
                heartq_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = heartq_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("HEARTQ_SESSION_ID").ok();
    ...
    heartq_workspace::init_metrics();
    let ws_handle = heartq_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = heartq_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            heartq_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("HEARTQ_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        ...
        assert_eq!(
            ...
            Some("https://heartq.com/preview-auth")
        );
