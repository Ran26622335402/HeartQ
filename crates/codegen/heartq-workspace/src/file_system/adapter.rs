//! AcpFsAdapter: implements `heartq-tools::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates heartq-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use heartq_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps heartq-shell's ACP gateway to satisfy heartq-tools' AsyncFileSystem.
///
