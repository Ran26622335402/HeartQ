...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "heartq-shell", alias = "heartq_shell")]
    Generic,
    /// HeartQ TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "heartq-tui", alias = "heartq_tui")]
    HeartQTUI,
    /// HeartQ Web client - identified by clientIdentifier "heartq-web"
    #[serde(rename = "heartq_web")]
    HeartQWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "heartq-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// HeartQ Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to HeartQTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"heartq-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"heartq_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "heartq-pager", alias = "heartq_pager")]
    HeartQPager,
    /// HeartQ Desktop (Electron) client - identified by clientIdentifier "heartq-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "heartq_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `heartq-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "heartq-shell",
            Self::HeartQTUI => "heartq-tui",
            Self::HeartQWeb => "heartq-web",
            Self::Nebula => "nebula",
            Self::Extension => "heartq-code-extension",
            Self::HeartQPager => "heartq-pager",
            Self::Desktop => "heartq-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"heartq-web"`, `"heartq-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("heartq-web") => Self::HeartQWeb,
            Some("nebula") => Self::Nebula,
            Some("heartq-code-extension") => Self::Extension,
            Some("heartq-desktop") => Self::Desktop,
            Some("heartq-pager") => Self::HeartQPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::HeartQTUI | Self::HeartQPager => "tui",
            Self::HeartQWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&heartq_tools::types::ToolInput> for AccessKind {
    fn from(input: &heartq_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.heartq/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use heartq_tools::implementations::heartq_build_hashline::edit::types::HashlineEditInput;
        use heartq_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use heartq_tools::implementations::heartq_build::bash::BashToolInput;
        use heartq_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use heartq_tools::implementations::use_tool::UseToolInput;
        use heartq_tools::types::ToolInput;
...
if name ==
    ...
    fn monitor_maps_to_bash_access() {
        use heartq_tools::implementations::heartq_build::monitor::types::MonitorInput;
        use heartq_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use heartq_tools::implementations::heartq_build::web_fetch::WebFetchInput;
        use heartq_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use heartq_tools::implementations::heartq_build::web_search::WebSearchInput;
        use heartq_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_heartq_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("heartq-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("heartq_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
