...
impl WorkspaceIdentity {
    ...
    /// `HeartQAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `heartq-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$HEARTQ_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("HEARTQ_SANDBOX_PROFILE").ok(),
            heartq_sandbox::is_inside_bwrap(),
