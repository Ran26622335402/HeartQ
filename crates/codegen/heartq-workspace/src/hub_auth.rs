//! Hub [`AuthProvider`] from `~/.heartq/auth.json` for the standalone
...
//!
//! The in-leader `heartq workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let heartq = heartq_config::user_heartq_home()
        .ok_or_else(|| anyhow::anyhow!("no user heartq home (set $HEARTQ_HOME or $HOME)"))?;
    Ok(heartq.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `heartq login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `heartq login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.heartq/auth.json`).
pub fn provider(
    hub_url: &Url,
