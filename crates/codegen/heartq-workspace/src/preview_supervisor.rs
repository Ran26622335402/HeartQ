...
//! spawns the unchanged `/usr/local/bin/heartq-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/heartq-preview-proxy";
...
/// `heartq_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "heartq_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `heartq-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`HEARTQ_SERVER_KEY` /
/// `HEARTQ_SESSION_ID`) reach the proxy by env inheritance — never argv.
...
/// `heartq-preview-proxy`'s `/__control/activity` route).
...
/// the default in `heartq-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://heartq.com/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match heartq-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "HEARTQ_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
