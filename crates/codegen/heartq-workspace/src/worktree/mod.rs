...
//!
//! Moved from `heartq-shell/src/session/worktree.rs` into the workspace
...
// Canonical in heartq-workspace-types; re-exported for existing paths.
pub use heartq_workspace_types::rpc::worktree::{
...
/// Resolve the heartq home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`xai_fast_worktree::resolve_heartq_home`), so checkout dirs and
/// the metadata DB always live under the same `.heartq` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `heartq_config::heartq_home()`,
/// so worktree paths also agree with trust/hooks and other heartq-home paths.
fn heartq_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_heartq_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.heartq/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    heartq_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.heartq/worktrees/<repo_name>`)
/// for a given source path, correctly handling heartq-managed worktrees.
///
/// When `source_path` is already under `~/.heartq/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the heartq worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = heartq_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.heartq/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the heartq-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.heartq/worktrees/`.
...
///
/// When the source path is already inside `~/.heartq/worktrees/<repo>/`, the
...
///
/// ACP equivalent of `heartq -w -r <session_id>` (optionally with `--ref`).
...
use xai_fast_worktree::{
    ...
    rebuild_worktree_db, resolve_heartq_home, resolve_worktree_auto_gc_from_layers,
};
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &heartq_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use heartq_config_types::WorktreeKindMaxAge;
...
/// Env + `$HEARTQ_HOME/config.toml` only — **`remote=None` is intentional**.
///
...
/// processes; use `HEARTQ_WORKTREE_AUTO_GC=0` / `HEARTQ_WORKTREE_AUTO_GC_DRY_RUN=1`
...
fn resolve_worktree_auto_gc_local() -> xai_fast_worktree::ResolvedWorktreeAutoGc {
    use heartq_config_types::WorktreeAutoGcSettings;

    let local = if let Ok(home) = resolve_heartq_home() {
...
/// Scan `~/.heartq/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/heartq/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/heartq/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/heartq/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/heartq/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `HEARTQ_HOME` at an isolated tempdir (`resolve_heartq_home` re-reads
    ...
    /// and restores `HEARTQ_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (HeartQHomeFixture is not visible across crates).
