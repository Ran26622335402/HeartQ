...
pub use heartq_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__HEARTQ_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    heartq_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = heartq_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias heartq_alias_probe='echo ALIAS_OK'\nheartq_fn_probe() { echo FN_OK; }\n",
            "heartq_alias_probe && heartq_fn_probe",
        )
        .await;
