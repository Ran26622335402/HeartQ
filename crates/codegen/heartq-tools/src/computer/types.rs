...
/// (`heartq-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (heartq-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in heartq-tools, spawns processes)
/// - `AcpTerminalBackend` (in heartq-shell, calls ACP protocol)
#[async_trait::async_trait]
