...
///   share it (heartq `read_file` → `"Read"`).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::HeartQBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::HeartQBuild => ("heartq_build", "HeartQBuild"),
                ToolNamespace::HeartQBuildConcise => ("heartq_build_concise", "HeartQBuildConcise"),
                ToolNamespace::HeartQBuildHashline => ("heartq_build_hashline", "HeartQBuildHashline"),
