...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.heartq/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.heartq/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.heartq/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.heartq/ or ~/.claude/.
    User { path: PathBuf },
