//! `ToolMetadata` — heartq-tools-specific metadata for tools.
//!
...
/// HeartQ-tools-specific metadata trait.
///
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (HeartQBuild, Cursor, OpenCode, ...).
    /// Used to build the fully-qualified tool ID at registration time
    /// (e.g., `"HeartQBuild:grep"`).
