...
//!
//! `ToolSearchIndex` is defined in `heartq-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `heartq-shell`
...
/// in `heartq-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `heartq-shell` after MCP initialization.
#[derive(Clone)]
