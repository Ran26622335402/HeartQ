...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (heartq build harness) ─────────────
...
///
/// `"/path/.heartq/skills/my-skill/SKILL.md"` -> `"/path/.heartq/skills/"`
...
/// Used by the budgeted (heartq build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// HeartQ build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the heartq
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // heartq-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: heartq build harness (budgeted XML) ───────────
