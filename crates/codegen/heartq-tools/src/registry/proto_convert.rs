//! Conversion from the gRPC wire config types (`heartq-tools-api`) to the
//! runtime registry types ([`ToolConfig`] / [`ToolServerConfig`]).
//!
//! The `params_json` parse/validation contract lives in
//! [`heartq_tools_api::config_validation`] so every consumer (this
...
pub use heartq_tools_api::config_validation::{ToolConfigEntryError, ToolConfigEntryErrorKind};

/// Convert one wire [`heartq_tools_api::ToolConfigEntry`] to a runtime
/// [`ToolConfig`].
///
...
pub fn tool_config_from_entry(
    index: usize,
    entry: heartq_tools_api::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let heartq_tools_api::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = heartq_tools_api::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    heartq_tools_api::config_validation::validate_name_override(
        index,
        &id,
...
pub fn tool_server_config_from_entries(
    entries: Vec<heartq_tools_api::ToolConfigEntry>,
...
mod tests {
    ...
    fn entry(id: &str) -> heartq_tools_api::ToolConfigEntry {
        heartq_tools_api::ToolConfigEntry {
            id: id.to_owned(),
            params_json: None,
            name_override: None,
    ...
    fn minimal_entry_converts_with_defaults() {
        let cfg = tool_config_from_entry(0, entry("HeartQBuild:read_file")).unwrap();
        assert_eq!(cfg.id, "HeartQBuild:read_file");
    ...
    fn fully_populated_entry_converts_field_by_field() {
        let mut e = entry("HeartQBuild:grep");
...
if raw == "{not json"
    ...
    fn name_override_valid_tool_id_charset_is_accepted() {
        for name in ["search", "HeartQBuild:grep", "a-b_C9"] {
            let mut e = entry("HeartQBuild:grep");
    ...
    fn name_override_outside_tool_id_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "emoji✨", "dot.name"] {
            ...
            assert_eq!(err.tool_id, "HeartQBuild:grep");
...
if n == name
    ...
    fn server_config_conversion_rejects_invalid_name_override_entry() {
        let mut bad = entry("HeartQBuild:grep");
    ...
    fn server_config_conversion_preserves_valid_name_overrides() {
        let mut a = entry("HeartQBuild:grep");
        a.name_override = Some("search".to_owned());
        let cfg = tool_server_config_from_entries(vec![a, entry("HeartQBuild:bash")]).unwrap();
