//! LSP server configuration from `.heartq/lsp.json`.
...
/// This is the canonical merge function — both session startup and `heartq inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::heartq_home::heartq_home().join("lsp.json");
    let project_path = cwd.join(".heartq").join("lsp.json");
...
/// Load LSP server configs from `~/.heartq/lsp.json` and `<cwd>/.heartq/lsp.json`.
