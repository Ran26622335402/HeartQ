...
use crate::implementations::heartq_build::task::TaskTool;
use crate::implementations::heartq_build::task::backend::SubagentBackendResource;
use crate::implementations::heartq_build::task::types::{SubagentSnapshot, SubagentSnapshotStatus};
use crate::implementations::heartq_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `HEARTQ_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("HEARTQ_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let heartq_build_bash = Expr::Value(ToolRequirement::Tool {
    ...
    let heartq_build_concise_bash = Expr::Value(ToolRequirement::Tool {
    ...
    vec![heartq_build_bash, heartq_build_concise_bash, opencode_bash]
}
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default heartq-build toolset.
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default heartq-build
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::heartq_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::heartq_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::heartq_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::heartq_build::task::types::SubagentEvent,
    ) -> crate::implementations::heartq_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::heartq_build::task::types::SubagentEvent::Query(q) => q,
