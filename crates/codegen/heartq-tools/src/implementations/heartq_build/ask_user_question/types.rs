...
//!
//! - **`heartq-tools`** — tool blocks on a oneshot, formats the result.
//! - **`heartq-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`heartq-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `heartq-tools`.
...
register_resource!("heartq_build", "UserQuestionSender", UserQuestionSender);
