...
/// params are injected, `HEARTQ_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `HEARTQ_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "HEARTQ_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("heartq_build", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. HeartQ callers leave it `None`.
...
/// session-owned coordinator (in heartq-shell), which performs an ACP
