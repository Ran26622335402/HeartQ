...
//! (in `heartq-tools`) and the subagent coordinator (in `heartq-shell`).
//!
//! ## Resource types
//!
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"heartq-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// defense-in-depth check in `heartq-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "HeartQBuild:run_terminal_cmd" | "HeartQBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `heartq-shell` relies on
...
register_resource!("heartq_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-heartq-build
...
register_resource!("heartq_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// heartq-shell produces it, the channel protocol carries it, and the
...
register_resource!("heartq_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("heartq_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("heartq_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by heartq-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by heartq-shell at turn start.
...
register_resource!("heartq_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("HeartQBuild:run_terminal_cmd", ToolKind::Execute),
                tc("HeartQBuild:read_file", ToolKind::Read),
                tc("HeartQBuild:list_dir", ToolKind::List),
                tc("HeartQBuild:grep", ToolKind::Search),
                tc("HeartQBuild:kill_task", ToolKind::KillTaskAction),
                tc("HeartQBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("HeartQBuild:kill_task", ToolKind::KillTaskAction),
                tc("HeartQBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("HeartQBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("HeartQBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
