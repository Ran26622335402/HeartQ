...
use regex::Regex;
use heartq_config::shell::AmpersandSemantics;
...
pub struct BashParams {
    ...
    /// heartq-build opts up to 10h via heartq-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "heartq_build.Bash";
...
// particular production heartq-build sets it to 10h in heartq-shell's
...
/// [`heartq_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `HEARTQ_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("HEARTQ_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in heartq_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the heartq_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`HEARTQ_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `HEARTQ_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = heartq_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = heartq_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing heartq_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (heartq_build backend) and can surface an educational
...
mod tests {
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // heartq-shell opts up to 10h via max_timeout_secs. Background unaffected.
