...
//! `LspTool` (Tool trait impl) under the `HeartQBuild` namespace.
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    xai_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.heartq/lsp.json or <cwd>/.heartq/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
