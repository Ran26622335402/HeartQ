...
const XAI_IMAGINE_MODEL: &str = "heartq-imagine-image-quality";
// Some Imagine models (e.g. `heartq-imagine-image`, selectable via `model_override`)
...
pub use heartq_tools_api::slash_commands::{
...
/// SuperHeartQ upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperHeartQ feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperHeartQ: https://heartq.com/superheartq?referrer=heartq-build. Do not retry this tool.";
...
pub struct ImageGenClient {
    ...
    /// HTTP call and return the SuperHeartQ upsell prose instead. See
...
impl ImageGenClient {
    ...
    /// with the SuperHeartQ upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// SuperHeartQ upsell prose instead of a doomed request. Set by the
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("heartq-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("heartq-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("heartq-imagine-image")), None)
                .unwrap()
                .model,
            "heartq-imagine-image"
        );
    }
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperHeartQ upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperHeartQ"), "got: {}", t.text);
                assert!(t.text.contains("superheartq?referrer=heartq-build"));
            }
