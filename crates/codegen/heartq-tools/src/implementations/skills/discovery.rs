...
///
/// `skills` is the standard layout (`.heartq/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into HeartQ Build sessions.
...
/// name (e.g. `~/.heartq/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into HeartQ Build.
...
/// `cwd` (exclusive). At each directory, checks for `.heartq/skills/`,
...
/// visits known config roots (`.heartq`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.heartq` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".heartq", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.heartq is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.heartq/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.heartq/ → kept (user content).
        let heartq_shell = tmp.path().join(".heartq").join("skills").join("shell");
        std::fs::create_dir_all(&heartq_shell).unwrap();
        std::fs::write(
            heartq_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (heartq_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.heartq/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .heartq skill in an intermediate dir.
        ...
        .unwrap();
        let heartq_skill = sub.join(".heartq").join("skills").join("heartq-dyn");
        std::fs::create_dir_all(&heartq_skill).unwrap();
        std::fs::write(heartq_skill.join("SKILL.md"), "---\nname: heartq-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"heartq-dyn"),
            "heartq-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only heartq-dyn discovered.
        ...
        assert!(
            names_off.contains(&"heartq-dyn"),
            "heartq-dyn missing: {names_off:?}"
        );
        assert!(
