...
//! after every tool call to check for SKILL.md files in `.heartq/skills/`,
...
/// Directories that contain skill definitions (`.heartq/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".heartq", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.heartq/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.heartq/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.heartq/) are outside the git root so the walk breaks early.
