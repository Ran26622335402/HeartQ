//! Integration tests for heartq-hooks.
//!
...
use heartq_hooks::discovery::load_hooks;
use heartq_hooks::dispatcher;
use heartq_hooks::event::*;
use heartq_hooks::result::HookDecision;
use heartq_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        heartq_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: heartq_hooks::event::StopFailureKind::RateLimit,
...
/// runner-reserved keys (`HEARTQ_HOOK_EVENT`, `HEARTQ_HOOK_NAME`,
/// `HEARTQ_SESSION_ID`, `HEARTQ_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$HEARTQ_HOOK_EVENT" > {f}; echo "NAME=$HEARTQ_HOOK_NAME" >> {f}; echo "SESSION=$HEARTQ_SESSION_ID" >> {f}; echo "ROOT=$HEARTQ_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "HEARTQ_HOOK_EVENT": "spoofed_event",
                                "HEARTQ_HOOK_NAME": "spoofed_name",
                                "HEARTQ_SESSION_ID": "spoofed_session",
                                "HEARTQ_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "HEARTQ_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed HEARTQ_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "HEARTQ_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed HEARTQ_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "HEARTQ_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed HEARTQ_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use heartq_hooks::result::HookRunResult;
    let info = match run {
