...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("HEARTQ_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            let inv = heartq_config::shell::shell_command_argv(&command_str);
    ...
    // plugin) can spoof `HEARTQ_HOOK_EVENT`, `HEARTQ_HOOK_NAME`, `HEARTQ_SESSION_ID`,
    // `HEARTQ_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    let mut child = match cmd
        ...
        .env("HEARTQ_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("HEARTQ_HOOK_NAME", &spec.name)
        .env("HEARTQ_SESSION_ID", ctx.session_id)
        .env("HEARTQ_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `HEARTQ_WORKSPACE_ROOT`; native `.heartq` hooks should use
        // `HEARTQ_WORKSPACE_ROOT`.
...
///   hooks),
/// * the HeartQ process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            resolve_command_path(&spec(
                HandlerType::Command,
                Some("bin/check.sh"),
                "/project/.heartq/hooks"
            )),
            Some(std::path::PathBuf::from(
                "/project/.heartq/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${HEARTQ_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${HEARTQ_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.heartq-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".heartq-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.heartq-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.heartq-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.heartq-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
