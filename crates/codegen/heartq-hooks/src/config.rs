...
pub struct HookSpec {
    ...
    ///    `HEARTQ_SESSION_ID`, `HEARTQ_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are
    ///    stripped at load time with a tracing warning.
    /// 2. Plugin-injected vars merged by the plugin adapter
    ///    (`heartq-agent::plugins::hooks_adapter`): `HEARTQ_PLUGIN_ROOT`,
    ///    `CLAUDE_PLUGIN_ROOT`, `HEARTQ_PLUGIN_DATA`, `CLAUDE_PLUGIN_DATA`, which
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.heartq/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.heartq/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "HEARTQ_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "HEARTQ_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "HEARTQ_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "HEARTQ_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "HEARTQ_HOOKS_PARSE_TEST_MATCHER_VAR";
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "HEARTQ_HOOK_EVENT": "spoofed",
                                    "HEARTQ_HOOK_NAME": "spoofed",
                                    "HEARTQ_SESSION_ID": "spoofed",
                                    "HEARTQ_WORKSPACE_ROOT": "/etc",
