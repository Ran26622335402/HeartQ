...
// (`~/.heartq/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_heartq_home>/trusted-hook-projects`), or `None` when no user heartq home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(heartq_config::user_heartq_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user heartq home (set $HEARTQ_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$HEARTQ_HOME/disabled-hooks`, or `None` when no user heartq
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(heartq_config::user_heartq_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let heartq_dir = dir.join(".heartq");
        std::fs::create_dir_all(&heartq_dir).unwrap();
        heartq_dir.join("trusted-hook-projects")
    }
