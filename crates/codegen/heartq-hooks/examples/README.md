...
Sample hooks for HeartQ. Copy to `~/.heartq/hooks/` to enable globally, or to `<project>/.heartq/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.heartq/hooks/bin
cp examples/hooks/safe-shell.json ~/.heartq/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.heartq/hooks/bin/
chmod +x ~/.heartq/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.heartq/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.heartq/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.heartq/hooks/bin/
chmod +x ~/.heartq/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.heartq/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.heartq/hooks/bin
cp examples/hooks/session-log.json ~/.heartq/hooks/
cp examples/hooks/bin/session-log.sh ~/.heartq/hooks/bin/
chmod +x ~/.heartq/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.heartq/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.heartq/hooks/bin
cp examples/hooks/tool-logger.json ~/.heartq/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.heartq/hooks/bin/
chmod +x ~/.heartq/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.heartq/hooks/bin
cp examples/hooks/stop-verify.json ~/.heartq/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.heartq/hooks/bin/
chmod +x ~/.heartq/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../heartq-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match HeartQ names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.heartq/hooks/`. The hook stops running on the next session.