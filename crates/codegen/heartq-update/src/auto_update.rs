...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_heartq_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use heartq_shell::util::config;
use heartq_shell::util::heartq_home::{heartq_application, heartq_home};
...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `heartq update` to get the latest version.";
...
fn reinstall_hint(installer: &str) -> String {
    match installer {
        "npm" => "Please reinstall via npm:\n  npm i -g @xai-official/heartq".to_string(),
        "gh-release" => "Please reinstall via GitHub Releases:\n  gh release download --repo xai-org-shared/heartq-build --pattern 'heartq-*' --output heartq && chmod +x heartq".to_string(),
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of HeartQ Build is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of HeartQ Build is available.");
        }
        return Ok(());
    }
    ...
    if let Some(latest_version) = status.latest_version.as_deref() {
        println!(
            "HeartQ Build - v{} (latest: {}){}",
            status.current_version, latest_version, channel_label
        );
        return Ok(());
    }

    println!("HeartQ Build - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_heartq_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_heartq_version();
...
/// background download, explicit `heartq update`) is reused as-is. This both
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_heartq_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_heartq_version();
...
/// managed `~/.heartq/bin/heartq` symlink.
///
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("HEARTQ_INSTALLER") {
    ...
    if std::env::var_os("HEARTQ_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("HEARTQ_MANAGED_BY_INTERNAL").is_some() {
...
/// (managed by hypercn via the upstream sync), meaning a pointer rollback is intentional and
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `heartq update` child, `Some` only when a
...
/// (spawns `heartq update` as a detached child process) so the new binary is
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // restart hint". The quit-for-update path's `heartq update` child resolves
...
/// Launch "heartq update" in blocking or non-blocking mode.
///
...
async fn run_update_subcommand(run_mode: UpdateRunMode) -> Result<Option<tokio::process::Child>> {
    ...
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("heartq update failed with {}", status);
            }
            Ok(None)
        }
        UpdateRunMode::NonBlocking => {
            ...
            heartq_tools::util::detach_command(&mut cmd);
            let child = cmd.spawn()?;
            Ok(Some(child))
        }
    }
...
/// Restart heartq with the original command-line arguments to pick up the update.
pub fn restart_heartq() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "HEARTQ_AUTO_UPDATE"));
    eprintln!("Restarting HeartQ...");
...
/// versioned name as the extension (`heartq-0.1.181-linux-x86_64` →
/// `heartq-0.1.tmp`) and therefore collides for every `0.1.x` version. The PID
...
/// versioned base like `heartq-0.1.181` doesn't collide via `with_extension`.
...
/// Delete `~/.heartq/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = heartq_home().join("models_cache.json");
...
/// Remove the stale `heartq-pager` symlink/binary from `~/.heartq/bin/` left by
...
/// config and writes to `~/.heartq/bin/`, so callers must isolate
/// `HEARTQ_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.heartq/downloads/`, not yet
/// activated as the managed `heartq`/`agent`.
...
async fn download_verified_from_base(
    ...
    let heartq_home = heartq_home();
    let download_dir = heartq_home.join("downloads");
    tokio::fs::create_dir_all(&download_dir).await?;

    let binary_name = format!("heartq-{}-{}", version, platform);
    let binary_path = download_dir.join(&binary_name);

    eprintln!("  Downloading heartq v{} ({})...", version, platform);
    ...
    // corrupt download is caught here and never becomes the active heartq.
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let heartq_home = heartq_home();
    let download_dir = heartq_home.join("downloads");
    let bin_dir = heartq_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.heartq/bin/{heartq,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "heartq", &download.version).await;
    cleanup_old_downloads(&download_dir, "heartq-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &heartq_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, heartq_home: &std::path::Path) {
    // Derive $HOME independently — heartq_home may be overridden via HEARTQ_HOME
    // env var, so heartq_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", heartq_home.join("completions/bash/heartq.bash")),
        ("zsh", heartq_home.join("completions/zsh/_heartq")),
        ("fish", user_home.join(".config/fish/completions/heartq.fish")),
    ];
...
///
/// When both paths share a grandparent (e.g. `~/.heartq/bin/heartq` and
/// `~/.heartq/downloads/heartq-0.1.203-linux-x86_64`), returns a relative path
/// like `../downloads/heartq-0.1.203-linux-x86_64`.  When they share the same
/// parent directory, returns just the filename.  Falls back to the absolute
/// `target` path for any other layout.
///
/// Relative symlinks survive Docker bind-mounts where `~/.heartq/` is mapped
...
fn relative_symlink_target(target: &std::path::Path, link: &std::path::Path) -> std::path::PathBuf {
    ...
    // Same directory — just the filename (e.g. heartq-latest -> heartq-0.1.203-…)
    if target_parent == link_parent
        && let Some(name) = target.file_name()
    {
        return std::path::PathBuf::from(name);
    }
    // Sibling directories — ../target_dir/filename (e.g. bin/heartq -> ../downloads/heartq-…)
...
/// Swap `~/.heartq/bin/{heartq,agent}` to point at `binary_path`. Returns the
/// `heartq` link path (for [`regenerate_completions`]).
///
/// `heartq` and `agent` are first-class entry points that the bootstrap
/// installers (`install.sh`, `install.ps1`, `install-enterprise.sh`)
/// maintain in lockstep, and so must the updater — otherwise `heartq update`
/// leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.heartq/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let heartq_name = if cfg!(windows) { "heartq.exe" } else { "heartq" };
    let agent_name = if cfg!(windows) { "agent.exe" } else { "agent" };
    let heartq_link = bin_dir.join(heartq_name);
    let agent_link = bin_dir.join(agent_name);
    let link_paths: [std::path::PathBuf; 2] = [heartq_link.clone(), agent_link];
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running heartq sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"heartq"` or `"heartq-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `heartq-*` matching `heartq-pager-*` or `heartq-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // Skip symlinks (e.g. heartq-latest).
        ...
        // binary (avoids `heartq-latest`, `heartq-pager-*` when prefix is `heartq`).
        let suffix = &name[prefix.len()..];
        if !suffix.starts_with(|c: char| c.is_ascii_digit()) {
            continue;
        }
        // Extract the version portion via the shared parser (handles the
        // internal `heartq-0.1.150-macos-aarch64`, pre-release, and npm
        // `heartq-0.1.150` layouts — see `version_from_versioned_binary_name`).
...
async fn heal_managed_install(installer: &str) {
    ...
    #[cfg(any(unix, windows))]
    {
        ...
        #[cfg(unix)]
        reconcile_agent_to_heartq(&bin_dir).await;

        #[cfg(windows)]
        reconcile_agent_exe_to_heartq(&bin_dir).await;
    }
}

#[cfg(unix)]
async fn reconcile_agent_to_heartq(bin_dir: &std::path::Path) {
    let heartq_link = bin_dir.join("heartq");
    let agent_link = bin_dir.join("agent");

    let Ok(heartq_target) = tokio::fs::read_link(&heartq_link).await else {
        return;
    };
    if tokio::fs::metadata(&heartq_link).await.is_err() {
        return;
    }
    if let Ok(agent_target) = tokio::fs::read_link(&agent_link).await
        && agent_target == heartq_target
    {
        return;
    }
    match atomic_symlink_swap(&heartq_target, &agent_link).await {
        Ok(()) => tracing::info!(
            heartq_target = %heartq_target.display(),
            "reconciled agent bin symlink to heartq target"
        ),
...
#[cfg(windows)]
async fn reconcile_agent_exe_to_heartq(bin_dir: &std::path::Path) {
    let heartq_exe = bin_dir.join("heartq.exe");
    let agent_exe = bin_dir.join("agent.exe");

    if tokio::fs::metadata(&heartq_exe).await.is_err() {
        return;
    }
    match agent_exe_differs(&heartq_exe, &agent_exe).await {
        Ok(true) => {}
        Ok(false) => return,
        Err(e) => {
    ...
    match windows_replace_exe(&heartq_exe, &agent_exe).await {
        Ok(()) => tracing::info!("reconciled agent.exe to heartq.exe"),
        Err(e) => tracing::warn!("failed to reconcile agent.exe to heartq.exe: {e:#}"),
    }
}

#[cfg(windows)]
async fn agent_exe_differs(
    heartq: &std::path::Path,
    agent: &std::path::Path,
) -> std::io::Result<bool> {
    use tokio::io::{AsyncReadExt, BufReader};
    let heartq_len = tokio::fs::metadata(heartq).await?.len();
    match tokio::fs::metadata(agent).await {
        Ok(m) if m.len() != heartq_len => return Ok(true),
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(true),
        Err(e) => return Err(e),
    }
    let mut rg = BufReader::new(tokio::fs::File::open(heartq).await?);
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    heartq_tools::util::detach_command(&mut cmd);
    cmd.envs(heartq_tools::util::pager_env());
...
/// Download and install heartq from GitHub Releases (xai-org-shared/heartq-build).
///
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    eprintln!(
        "  Downloading heartq v{} ({}) from GitHub Releases...",
        version, platform
    );
    ...
    // Update heartq-latest -> versioned binary so any existing symlinks that route
    // through it (e.g. /usr/local/bin/heartq -> ~/.heartq/downloads/heartq-latest)
    // resolve to the newly installed version.
    #[cfg(unix)]
    {
        let latest_path = download_dir.join("heartq-latest");
        ...
        if let Err(e) = atomic_symlink_swap(&rel_target, &latest_path).await {
            tracing::warn!("Failed to update heartq-latest symlink: {e}");
        }
    }

    // Also update /usr/local/bin/{heartq,agent} if either points directly into
    // ~/.heartq/downloads/ (legacy layout — skips the heartq-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["heartq", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".heartq/downloads/") && !target_str.ends_with("heartq-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "heartq", &version).await;
    cleanup_old_downloads(&download_dir, "heartq-pager", &version).await;
...
/// Check if other heartq processes are running (macOS only).
///
/// On macOS, `npm i -g` replaces the vendored binary in node_modules in-place.
/// Any heartq process running from that vendored path will be SIGKILL'd by the
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.heartq/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_heartq_processes_running() {
    ...
    cmd.args(["-f", "heartq"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    heartq_tools::util::detach_std_command(&mut cmd);
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other heartq process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other heartq sessions before updating.");
            eprintln!();
        }
    }
}
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    warn_if_other_heartq_processes_running();

    let version_arg = match target {
        Some(ver) => format!("@xai-official/heartq@{ver}"),
        None => {
...
/// Run the `heartq update` command. Returns `Ok(Some(version))` when the target
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!("  ✓ heartq v{} installed successfully!", version);
        eprintln!("  Please restart HeartQ.");
    ...
    {
        eprintln!(
            "Forcing reinstall of HeartQ {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
        eprintln!("Updating HeartQ {} → {}", effective_current, install_target);
        &install_target
    };
    ...
    refresh_deployment_config().await;
    eprintln!("  ✓ heartq v{} installed successfully!", target_version);

    if !force && std::env::var_os("HEARTQ_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart HeartQ.");
    }
...
async fn refresh_deployment_config() {
    if !heartq_shell::managed_config::has_principal() {
        return;
    }
    if !heartq_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    heartq_shell::managed_config::clear_orphan();
    if !heartq_shell::config::is_managed_config_stale_for(
        &heartq_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match heartq_shell::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `heartq setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `heartq setup` to retry.");
        }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        // name onto a single `heartq-0.1.tmp`; the helper must keep distinct
        // versions distinct AND make repeated attempts (same process, e.g.
        // concurrent tokio tasks) unique.
        let dest_181 = std::path::Path::new("/home/u/.heartq/downloads/heartq-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.heartq/downloads/heartq-0.1.182-linux-x86_64");
        ...
        assert!(
            name.starts_with("heartq-0.1.181-linux-x86_64."),
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.heartq/downloads").into(),
    ...
    async fn test_atomic_symlink_swap_replaces_regular_file() {
        ...
        let link = dir.path().join("heartq");
        // Simulate an old installation where heartq is a regular file.
    ...
    async fn test_reconcile_agent_repoints_diverged_agent() {
        ...
        std::fs::write(downloads.join("heartq-0.2.101-macos-aarch64"), "new").unwrap();
        std::fs::write(downloads.join("heartq-0.1.199-macos-aarch64"), "old").unwrap();

        std::os::unix::fs::symlink("../downloads/heartq-0.2.101-macos-aarch64", bin.join("heartq"))
            .unwrap();
        std::os::unix::fs::symlink("../downloads/heartq-0.1.199-macos-aarch64", bin.join("agent"))
            .unwrap();

        reconcile_agent_to_heartq(&bin).await;

        assert_eq!(
            std::fs::read_link(bin.join("agent")).unwrap(),
            std::path::PathBuf::from("../downloads/heartq-0.2.101-macos-aarch64"),
        );
        assert_eq!(std::fs::read_to_string(bin.join("agent")).unwrap(), "new");
        assert!(downloads.join("heartq-0.1.199-macos-aarch64").exists());
    }
    ...
    async fn test_reconcile_agent_heals_legacy_unversioned_agent() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(downloads.join("heartq-0.2.101-macos-aarch64"), "new").unwrap();
        std::fs::write(downloads.join("heartq-macos-aarch64"), "legacy").unwrap();

        std::os::unix::fs::symlink("../downloads/heartq-0.2.101-macos-aarch64", bin.join("heartq"))
            .unwrap();
        std::os::unix::fs::symlink("../downloads/heartq-macos-aarch64", bin.join("agent")).unwrap();
    ...
    async fn test_reconcile_agent_noop_when_consistent() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(downloads.join("heartq-0.2.101-macos-aarch64"), "new").unwrap();
        let target = "../downloads/heartq-0.2.101-macos-aarch64";
        std::os::unix::fs::symlink(target, bin.join("heartq")).unwrap();
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_reconcile_agent_skips_when_heartq_dangling() {
        let (_dir, bin, downloads) = managed_layout();
        std::os::unix::fs::symlink("../downloads/heartq-0.2.101-macos-aarch64", bin.join("heartq"))
            .unwrap();
        std::fs::write(downloads.join("heartq-0.1.199-macos-aarch64"), "old").unwrap();
        ...
        assert_eq!(
            std::fs::read_link(bin.join("agent")).unwrap(),
            std::path::PathBuf::from("../downloads/heartq-0.1.199-macos-aarch64"),
        );
    }

    #[cfg(unix)]
    #[tokio::test]
    async fn test_reconcile_agent_skips_when_heartq_not_symlink() {
        let (_dir, bin, downloads) = managed_layout();
        std::fs::write(bin.join("heartq"), "copy-binary").unwrap();
        std::fs::write(downloads.join("heartq-0.1.199-macos-aarch64"), "old").unwrap();
        std::os::unix::fs::symlink("../downloads/heartq-0.1.199-macos-aarch64", bin.join("agent"))
            .unwrap();
    ...
    async fn test_sweep_stale_tmp_links_removes_stale_keeps_fresh_and_active() {
        ...
        let leftover_old = dir.path().join("heartq.tmp-link");
        let leftover_new = dir.path().join("heartq.123-0.tmp-link");
        ...
        // `heartq` link (no `.tmp-link` suffix) is untouched.
        ...
        let fresh = dir.path().join("heartq.999-9.tmp-link");
    ...
    async fn test_atomic_symlink_swap_with_absolute_target() {
        ...
        let binary = dir.path().join("heartq-0.1.141");
    ...
    async fn test_atomic_symlink_swap_with_relative_target() {
        ...
        std::fs::write(downloads.join("heartq-0.1.203"), "v203").unwrap();

        let rel_target = std::path::Path::new("../downloads/heartq-0.1.203");
        let link = bin.join("heartq");
        atomic_symlink_swap(rel_target, &link).await.unwrap();

        assert!(link.is_symlink());
        assert_eq!(
            std::fs::read_link(&link).unwrap(),
            std::path::PathBuf::from("../downloads/heartq-0.1.203")
        );
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        // bin/heartq -> ../downloads/heartq-0.1.203
        let target = std::path::Path::new("/home/alice/.heartq/downloads/heartq-0.1.203");
        let link = std::path::Path::new("/home/alice/.heartq/bin/heartq");
    ...
    fn test_relative_symlink_target_same_dir() {
        // downloads/heartq-latest -> heartq-0.1.203 (same directory)
        let target = std::path::Path::new("/home/alice/.heartq/downloads/heartq-0.1.203");
        let link = std::path::Path::new("/home/alice/.heartq/downloads/heartq-latest");
        let result = relative_symlink_target(target, link);
        assert_eq!(result, std::path::PathBuf::from("heartq-0.1.203"));
    }

    #[cfg(unix)]
    #[test]
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/heartq -> /home/alice/.heartq/downloads/heartq-0.1.203
        // Different grandparents — should stay absolute.
        let target = std::path::Path::new("/home/alice/.heartq/downloads/heartq-0.1.203");
        let link = std::path::Path::new("/usr/local/bin/heartq");
        let result = relative_symlink_target(target, link);
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.heartq/downloads/heartq-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.heartq/ layout at path A,
        // then move it to path B and verify the symlink still resolves.
        let dir = tempfile::tempdir().unwrap();

        // Create alice's layout
        let alice = dir.path().join("alice").join(".heartq");
        ...
        let bob_link = bob.join("bin").join("heartq");
    ...
    async fn test_cleanup_old_downloads_keeps_current_plus_one() {
        ...
        // Simulate 5 old heartq binaries in downloads dir.
        for v in ["0.1.140", "0.1.141", "0.1.142", "0.1.143", "0.1.144"] {
            std::fs::write(d.join(format!("heartq-{}-macos-aarch64", v)), v).unwrap();
        }
        // Current version.
        std::fs::write(d.join("heartq-0.1.145-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.145").await;

        // Current must survive.
        assert!(d.join("heartq-0.1.145-macos-aarch64").exists(), "current");
        // Newest old version (0.1.144) must survive.
        assert!(d.join("heartq-0.1.144-macos-aarch64").exists(), "N-1");
        // Everything else should be deleted.
        assert!(
            !d.join("heartq-0.1.143-macos-aarch64").exists(),
            "0.1.143 should be deleted"
        );
        assert!(
            !d.join("heartq-0.1.142-macos-aarch64").exists(),
            "0.1.142 should be deleted"
        );
        assert!(
            !d.join("heartq-0.1.141-macos-aarch64").exists(),
            "0.1.141 should be deleted"
        );
        assert!(
            !d.join("heartq-0.1.140-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_does_not_touch_other_binaries() {
        ...
        // heartq and heartq-pager should not interfere with each other.
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64"), "old-heartq").unwrap();
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "current-heartq").unwrap();
        std::fs::write(d.join("heartq-pager-0.1.140-macos-aarch64"), "old-pager").unwrap();
        std::fs::write(d.join("heartq-pager-0.1.141-macos-aarch64"), "current-pager").unwrap();

        // Cleanup only heartq — pager files must be untouched.
        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.141").await;

        assert!(d.join("heartq-0.1.141-macos-aarch64").exists());
        assert!(d.join("heartq-0.1.140-macos-aarch64").exists()); // only old, kept as N-1
        assert!(
            d.join("heartq-pager-0.1.140-macos-aarch64").exists(),
            "pager untouched"
        );
        assert!(
            d.join("heartq-pager-0.1.141-macos-aarch64").exists(),
            "pager untouched"
        );
    }
    ...
    async fn test_cleanup_old_downloads_removes_stale_tmp_keeps_fresh_tmp() {
        ...
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64.tmp"), "partial").unwrap();
        make_stale(&d.join("heartq-0.1.140-macos-aarch64.tmp"));
        // Fresh tmp: a concurrent updater's in-flight download — kept, or
        // its atomic rename would fail with ENOENT.
        std::fs::write(d.join("heartq-0.1.142-macos-aarch64.77-0.tmp"), "inflight").unwrap();
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "current").unwrap();

        cleanup_old_downloads(d, "heartq", "0.1.141").await;

        assert!(
            !d.join("heartq-0.1.140-macos-aarch64.tmp").exists(),
            "stale tmp cleaned up"
        );
        assert!(
            d.join("heartq-0.1.142-macos-aarch64.77-0.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_keeps_fresh_versioned_binary() {
        ...
        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("heartq-{v}-macos-aarch64")), v).unwrap();
        }
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "current").unwrap();
        make_all_stale(d);
        ...
        std::fs::write(d.join("heartq-0.1.138-macos-aarch64"), "in-flight").unwrap();

        cleanup_old_downloads(d, "heartq", "0.1.141").await;

        assert!(d.join("heartq-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("heartq-0.1.140-macos-aarch64").exists(), "N-1 kept");
        assert!(
            d.join("heartq-0.1.138-macos-aarch64").exists(),
            "fresh just-renamed binary must NOT be deleted"
        );
        assert!(
            !d.join("heartq-0.1.139-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_skips_symlinks() {
        ...
        // heartq-latest is a symlink — must be skipped.
        let target = d.join("heartq-0.1.141-macos-aarch64");
        std::fs::write(&target, "current").unwrap();
        std::os::unix::fs::symlink(&target, d.join("heartq-latest")).unwrap();
    ...
    async fn test_cleanup_old_downloads_empty_dir() {
        ...
        cleanup_old_downloads(dir.path(), "heartq", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_version_prefix_collision() {
        ...
        std::fs::write(d.join("heartq-0.1.14-macos-aarch64"), "current").unwrap();
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64"), "old-140").unwrap();
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "old-141").unwrap();
        std::fs::write(d.join("heartq-0.1.13-macos-aarch64"), "old-13").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.14").await;

        // Current must survive.
        assert!(
            d.join("heartq-0.1.14-macos-aarch64").exists(),
            "current 0.1.14"
        );
        ...
        assert!(
            !d.join("heartq-0.1.13-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_pager_multi_version() {
        // Verify cleanup works for heartq-pager with multiple old versions.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.148", "0.1.149", "0.1.150"] {
            std::fs::write(d.join(format!("heartq-pager-{}-linux-x64", v)), v).unwrap();
        }
        std::fs::write(d.join("heartq-pager-0.1.151-linux-x64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq-pager", "0.1.151").await;

        assert!(d.join("heartq-pager-0.1.151-linux-x64").exists(), "current");
        assert!(d.join("heartq-pager-0.1.150-linux-x64").exists(), "N-1 kept");
        assert!(
            !d.join("heartq-pager-0.1.149-linux-x64").exists(),
            "0.1.149 deleted"
        );
        assert!(
            !d.join("heartq-pager-0.1.148-linux-x64").exists(),
            "0.1.148 deleted"
        );
    }
    ...
    async fn test_cleanup_old_downloads_npm_layout() {
        // npm layout: files are just `heartq-{version}` (no platform suffix).
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        for v in ["0.1.138", "0.1.139", "0.1.140"] {
            std::fs::write(d.join(format!("heartq-{}", v)), v).unwrap();
        }
        std::fs::write(d.join("heartq-0.1.141"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.141").await;

        assert!(d.join("heartq-0.1.141").exists(), "current");
        assert!(d.join("heartq-0.1.140").exists(), "N-1 kept");
        assert!(!d.join("heartq-0.1.139").exists(), "0.1.139 deleted");
        assert!(!d.join("heartq-0.1.138").exists(), "0.1.138 deleted");
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_alpha_versions() {
        // Alpha version filenames include pre-release tags:
        //   heartq-0.1.150-alpha.1-macos-aarch64
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();

        std::fs::write(d.join("heartq-0.1.148-alpha.1-macos-aarch64"), "alpha-148-1").unwrap();
        std::fs::write(d.join("heartq-0.1.148-alpha.2-macos-aarch64"), "alpha-148-2").unwrap();
        std::fs::write(d.join("heartq-0.1.149-alpha.1-macos-aarch64"), "alpha-149-1").unwrap();
        // Current version is the newest alpha.
        std::fs::write(d.join("heartq-0.1.150-alpha.1-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.150-alpha.1").await;

        // Current must survive.
        assert!(
            d.join("heartq-0.1.150-alpha.1-macos-aarch64").exists(),
            "current alpha"
        );
        // Newest old (0.1.149-alpha.1) kept as N-1.
        assert!(
            d.join("heartq-0.1.149-alpha.1-macos-aarch64").exists(),
            "N-1 alpha"
        );
        // Older alphas deleted.
        assert!(
            !d.join("heartq-0.1.148-alpha.2-macos-aarch64").exists(),
            "0.1.148-alpha.2 deleted"
        );
        assert!(
            !d.join("heartq-0.1.148-alpha.1-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_mixed_stable_and_alpha() {
        ...
        std::fs::write(d.join("heartq-0.1.148-macos-aarch64"), "stable-148").unwrap();
        std::fs::write(d.join("heartq-0.1.149-alpha.1-macos-aarch64"), "alpha-149").unwrap();
        std::fs::write(d.join("heartq-0.1.149-macos-aarch64"), "stable-149").unwrap();
        // Current is a stable release.
        std::fs::write(d.join("heartq-0.1.150-macos-aarch64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.150").await;

        // Current must survive.
        assert!(d.join("heartq-0.1.150-macos-aarch64").exists(), "current");
        // Newest old is 0.1.149 stable (semver: 0.1.149 > 0.1.149-alpha.1).
        assert!(
            d.join("heartq-0.1.149-macos-aarch64").exists(),
        ...
        assert!(
            !d.join("heartq-0.1.148-macos-aarch64").exists(),
    ...
    fn test_reinstall_hint_npm_mentions_npm_command() {
        ...
        assert!(
            hint.contains("@xai-official/heartq"),
    ...
    fn test_reinstall_hint_gh_release_mentions_gh_command() {
        ...
        assert!(
            hint.contains("xai-org-shared/heartq-build"),
    ...
    async fn test_cleanup_old_downloads_invalid_current_version_is_no_op() {
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "v141").unwrap();

        // Invalid version string → cleanup must early-return without deleting.
        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "not-a-version").await;
        assert!(d.join("heartq-0.1.140-macos-aarch64").exists());
        assert!(d.join("heartq-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_missing_dir_no_panic() {
        ...
        cleanup_old_downloads(&missing, "heartq", "0.1.141").await;
    }
    ...
    async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
        ...
        // ignored (e.g. heartq-latest, heartq-pager-* when prefix is heartq).
        std::fs::write(d.join("heartq-latest"), "alias").unwrap();
        std::fs::write(d.join("heartq-pager-0.1.141-macos-aarch64"), "pager").unwrap();
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64"), "v140").unwrap();
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        // heartq-latest and heartq-pager-* must be untouched.
        assert!(d.join("heartq-latest").exists());
        assert!(d.join("heartq-pager-0.1.141-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_unparseable_version_skipped() {
        ...
        std::fs::write(d.join("heartq-9garbage-macos-aarch64"), "junk").unwrap();
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(
            d.join("heartq-9garbage-macos-aarch64").exists(),
    ...
    async fn test_cleanup_old_downloads_multiplatform_in_same_dir() {
        ...
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "mac").unwrap();
        std::fs::write(d.join("heartq-0.1.141-linux-x86_64"), "linux").unwrap();
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64"), "old-mac").unwrap();
        std::fs::write(d.join("heartq-0.1.139-macos-aarch64"), "older-mac").unwrap();
        ...
        assert!(d.join("heartq-0.1.141-macos-aarch64").exists());
        assert!(d.join("heartq-0.1.141-linux-x86_64").exists());
        // N-1 (0.1.140) kept, older deleted.
        assert!(d.join("heartq-0.1.140-macos-aarch64").exists());
        assert!(!d.join("heartq-0.1.139-macos-aarch64").exists());
    }
    ...
    async fn test_cleanup_old_downloads_tmp_files_deleted_even_when_unparseable() {
        ...
        std::fs::write(d.join("heartq-junk.tmp"), "partial").unwrap();
        make_stale(&d.join("heartq-junk.tmp"));
        std::fs::write(d.join("heartq-0.1.140-macos-aarch64.tmp"), "partial2").unwrap();
        make_stale(&d.join("heartq-0.1.140-macos-aarch64.tmp"));
        std::fs::write(d.join("heartq-0.1.141-macos-aarch64"), "current").unwrap();
        ...
        assert!(!d.join("heartq-junk.tmp").exists(), "junk tmp deleted");
        assert!(
            !d.join("heartq-0.1.140-macos-aarch64.tmp").exists(),
    ...
    async fn test_cleanup_old_downloads_three_olds_keeps_only_newest() {
        ...
        assert!(d.join("heartq-0.1.141-macos-aarch64").exists(), "current");
        assert!(d.join("heartq-0.1.140-macos-aarch64").exists(), "N-1 only");
        assert!(!d.join("heartq-0.1.139-macos-aarch64").exists());
        assert!(!d.join("heartq-0.1.138-macos-aarch64").exists());
    }

    #[tokio::test]
    async fn test_cleanup_old_downloads_darwin_platform_recognized() {
        // The `darwin` alias for macOS is in PLATFORM_OS — versions on
        // heartq-X.Y.Z-darwin-* layouts must split correctly.
        let dir = tempfile::tempdir().unwrap();
        let d = dir.path();
        std::fs::write(d.join("heartq-0.1.140-darwin-arm64"), "v140").unwrap();
        std::fs::write(d.join("heartq-0.1.141-darwin-arm64"), "current").unwrap();

        make_all_stale(d);

        cleanup_old_downloads(d, "heartq", "0.1.141").await;

        assert!(d.join("heartq-0.1.141-darwin-arm64").exists(), "current");
        assert!(d.join("heartq-0.1.140-darwin-arm64").exists(), "N-1");
    }
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        assert_eq!(
            ...
            "Run `heartq update` to get the latest version."
        );
    }
    ...
    //   1. HEARTQ_INSTALLER (npm | internal | gh-release | gh)
    //   2. HEARTQ_MANAGED_BY_NPM       → npm
    //   3. HEARTQ_MANAGED_BY_INTERNAL  → internal
    ...
    impl InstallerEnvGuard {
        fn isolate() -> Self {
            const VARS: &[&str] = &[
                "HEARTQ_INSTALLER",
                "HEARTQ_MANAGED_BY_NPM",
                "HEARTQ_MANAGED_BY_INTERNAL",
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `HEARTQ_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("HEARTQ_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit HEARTQ_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("HEARTQ_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("HEARTQ_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // HEARTQ_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("HEARTQ_INSTALLER", "internal");
            std::env::set_var("HEARTQ_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    async fn test_windows_replace_exe_creates_dest_when_missing() {
        ...
        let dest = dir.path().join("heartq.exe");
    ...
    async fn test_windows_replace_exe_cleans_stale_old_backup() {
        ...
        let old = dir.path().join("heartq.exe.old");
    ...
    async fn test_windows_replace_exe_rollback_on_copy_failure() {
        // No stale .old: the aside IS heartq.exe.old, so this pins the
    ...
    async fn test_windows_replace_exe_locked_stale_old_does_not_block_update() {
        ...
        let asides: Vec<std::path::PathBuf> = std::fs::read_dir(dir.path())
            ...
            .filter(|p| {
                p.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.starts_with("heartq.exe.old.") && n.ends_with(".old"))
            })
            .collect();
        assert_eq!(
            asides.len(),
    ...
    async fn test_windows_replace_exe_rollback_restores_from_diverted_aside() {
        ...
        let leftover_asides = std::fs::read_dir(dir.path())
            ...
            .filter(|e| {
                ...
                name.starts_with("heartq.exe.old.") && name.ends_with(".old")
            })
            .count();
    ...
    async fn test_windows_replace_exe_sweeps_accumulated_asides() {
        ...
        let aside_a = dir.path().join("heartq.exe.old.1234-0.old");
        let aside_b = dir.path().join("heartq.exe.old.1234-1.old");
