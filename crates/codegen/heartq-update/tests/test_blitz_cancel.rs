...
//!
//! > `~/.heartq/bin/heartq` resolves to a binary that passes the smoke-test, OR it
...
use heartq_update::auto_update::install_internal_from_base;
...
/// (`heartq` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `heartq` should resolve to after an install attempt.
...
/// to both `heartq` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p heartq-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("HEARTQ_BLITZ_ITERS")
        .ok()
