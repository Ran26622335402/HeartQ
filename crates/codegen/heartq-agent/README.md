...
This crate extracts a first-class `Agent` type from `heartq-shell`.
...
`heartq-shell`, another in-process host, or a headless batch runner.
...
in `.heartq/agents/` (project-level) or `~/.heartq/agents/` (user-level).

```rust
use heartq_agent::{AgentDefinition, AgentBuilder};
use heartq_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".heartq/agents/code-reviewer.md")?;
...
```rust
use heartq_agent::discovery;

// Find all .md files in .heartq/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  heartq-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`heartq-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `heartq-agent` depends on it for tool setup.
- **`heartq-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `heartq-agent` (AGENTS.md
...
|---|---|---|
| `heartq-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p heartq-agent

# Test
cargo test -p heartq-agent

# Clippy
cargo clippy -p heartq-agent --fix --allow-dirty
