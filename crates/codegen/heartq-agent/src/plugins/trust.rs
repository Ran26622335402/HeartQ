...
//!
//! Plugins from project directories (`.heartq/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.heartq/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.heartq/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.heartq/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_heartq_home() so a project's `.heartq/trusted-plugins` is never
        // read as the user trust store when neither HEARTQ_HOME nor a home dir resolves.
        let Some(heartq) = heartq_config::user_heartq_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = heartq.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.heartq/trusted-plugins`.
    ...
    /// rewrites `~/.heartq/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.heartq/trusted-plugins`.
