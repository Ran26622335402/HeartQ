//! Agent definition types — parsed from `.heartq/agents/*.md` files.
...
use heartq_tools::implementations::codex;
use heartq_tools::implementations::heartq_build;
use heartq_tools::implementations::heartq_build_concise;
use heartq_tools::implementations::memory;
use heartq_tools::implementations::opencode;
use heartq_tools::implementations::search_tool;
use heartq_tools::implementations::skill_manage;
use heartq_tools::implementations::use_tool;
use heartq_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard HeartQBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&heartq_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&heartq_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&heartq_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&heartq_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&heartq_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_heartq_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_heartq_build_toolset() -> ToolServerConfig {
    let mut tools = default_heartq_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&heartq_build::EnterPlanModeTool).into());
    tools.push((&heartq_build::ExitPlanModeTool).into());
    tools.push((&heartq_build::AskUserQuestionTool).into());
    tools.push((&heartq_build::WebSearchTool).into());
    tools.push((&heartq_build::ImageGenTool).into());
    tools.push((&heartq_build::ImageToVideoTool).into());
    tools.push((&heartq_build::ReferenceToVideoTool).into());
    tools.push((&heartq_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&skill_manage::SkillManageImpl).into());
    tools.push((&heartq_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `heartq-computer` (workspace/sandbox) preset.
fn heartq_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&heartq_build::ReadFileTool).into(),
        (&heartq_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&heartq_build::ListDirTool).into(),
        (&heartq_build::GrepTool).into(),
        (&heartq_build::KillTerminalCommandTool).into(),
        (&heartq_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("heartq-build", workspace_heartq_build_toolset()),
        ("heartq-build-concise", heartq_build_concise_toolset()),
        ("heartq-build-plan", heartq_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("heartq-computer", heartq_computer_toolset()),
    ]
}
...
fn default_heartq_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&heartq_build::ReadFileTool).into(),
            ...
            (&heartq_build::TodoWriteTool).into(),
            task_output_tool_config(),
            wait_tasks_tool_config(),
            task_tool_config(),
            (&heartq_build::SchedulerCreateTool).into(),
            (&heartq_build::SchedulerDeleteTool).into(),
            (&heartq_build::SchedulerListTool).into(),
            (&heartq_build::MonitorTool).into(),
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&heartq_build::UpdateGoalTool).into(),
            (&heartq_build::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
fn heartq_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&heartq_build_concise::BashConciseTool).into(),
            (&heartq_build_concise::ReadFileConciseTool).into(),
            (&heartq_build_concise::SearchReplaceConciseTool).into(),
            (&heartq_build::ListDirTool).into(),
            (&heartq_build::GrepTool).into(),
            kill_task_tool_config(),
            (&heartq_build::TodoWriteTool).into(),
...
pub fn heartq_build_hashline_toolset(
    hashline_tools: Vec<heartq_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<heartq_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&heartq_build::WebSearchTool).into(),
        (&heartq_build::SchedulerCreateTool).into(),
        (&heartq_build::SchedulerDeleteTool).into(),
        (&heartq_build::SchedulerListTool).into(),
        (&heartq_build::MonitorTool).into(),
...
/// HeartQ Build + plan mode toolset.
///
/// Extends the default `heartq-build` toolset with plan mode tools:
...
fn heartq_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&heartq_build::ReadFileTool).into(),
            ...
            (&heartq_build::UpdateGoalTool).into(),
            (&heartq_build::WorkflowTool).into(),
            (&heartq_build::EnterPlanModeTool).into(),
            (&heartq_build::ExitPlanModeTool).into(),
            (&heartq_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&heartq_build::MonitorTool).into(),
            (&heartq_build::WebSearchTool).into(),
            (&heartq_build::WebFetchTool).into(),
            (&heartq_build::ImageGenTool).into(),
            (&heartq_build::ImageToVideoTool).into(),
            (&heartq_build::ReferenceToVideoTool).into(),
...
/// HeartQ Build + plan mode toolset WITHOUT subagent tools.
///
/// Same as `heartq_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn heartq_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&heartq_build::ReadFileTool).into(),
...
/// Default HeartQ Build toolset + `ask_user_question`.
///
/// Same as `default_heartq_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn heartq_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&heartq_build::ReadFileTool).into(),
...
/// - `model: heartq-3-fast` → `Override("heartq-3-fast")`
...
pub enum BuiltinAgentName {
    #[strum(serialize = "heartq-build")]
    HeartQBuild,
    #[strum(serialize = "heartq-build-concise")]
    HeartQBuildConcise,
    #[strum(serialize = "heartq-build-plan")]
    HeartQBuildPlan,
    #[strum(serialize = "heartq-build-plan-no-subagents")]
    HeartQBuildPlanNoSubagents,
    #[strum(serialize = "heartq-build-ask-user")]
    HeartQBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "heartq-build-orchestrator")]
    HeartQBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::HeartQBuild => AgentDefinition::default_heartq_build(),
            Self::HeartQBuildConcise => AgentDefinition::heartq_build_concise(),
            Self::HeartQBuildPlan => AgentDefinition::heartq_build_plan(),
            Self::HeartQBuildPlanNoSubagents => AgentDefinition::heartq_build_plan_no_subagents(),
            Self::HeartQBuildAskUser => AgentDefinition::heartq_build_ask_user(),
            ...
            Self::HeartQBuildOrchestrator => AgentDefinition::heartq_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .heartq/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_heartq_build_toolset")]
...
pub enum AgentScope {
    /// .heartq/agents/ (project-level, highest priority)
    Project,
    /// ~/.heartq/agents/ (user-level)
    User,
    /// ~/.heartq/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_heartq_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.heartq/agent-memory/<name>/`
    User,
    /// `<project>/.heartq/agent-memory/<name>/`
    Project,
    /// `<project>/.heartq/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: heartq_config::heartq_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".heartq/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".heartq/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `heartq_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let heartq = heartq_config::user_heartq_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), heartq.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".heartq/agents/") || path_str.contains(".heartq\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".heartq/bundled/agents/")
            || path_str.contains(".heartq\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `heartq-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<heartq_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["HeartQBuild:read_file", "HeartQBuildHashline:hashline_read"],
            [
                "HeartQBuild:search_replace",
                "HeartQBuildHashline:hashline_edit",
            ],
            ["HeartQBuild:grep", "HeartQBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_heartq_build_toolset(),
    ...
    pub fn default_heartq_build() -> Self {
        Self::base(
            BuiltinAgentName::HeartQBuild,
            "HeartQ Build agent for software engineering tasks.",
        )
    }
    /// HeartQ Build Concise agent definition — concise output format for SFT/RL.
    pub fn heartq_build_concise() -> Self {
        Self {
            tool_config: heartq_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::HeartQBuildConcise,
                "HeartQ Build agent with concise output format.",
            )
        }
    }
    /// HeartQ Build agent with plan mode tools.
    pub fn heartq_build_plan() -> Self {
        Self {
            tool_config: heartq_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::HeartQBuildPlan,
                "HeartQ Build agent with plan mode support.",
            )
        }
    }
    /// HeartQ Build + plan mode WITHOUT subagent tools.
    pub fn heartq_build_plan_no_subagents() -> Self {
        Self {
            tool_config: heartq_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::HeartQBuildPlanNoSubagents,
                "HeartQ Build agent with plan mode (no subagents).",
            )
        }
    }
    /// Default HeartQ Build agent with the `ask_user_question` tool.
    pub fn heartq_build_ask_user() -> Self {
        Self {
            tool_config: heartq_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::HeartQBuildAskUser,
                "HeartQ Build agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// HeartQ Build Orchestrator — GBL model with full HeartQBuild tools
    ...
    pub fn heartq_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::HeartQBuildOrchestrator,
                "HeartQBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_heartq_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("heartq-build").unwrap();
    ...
    fn heartq_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&heartq_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&heartq_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn heartq_computer_preset_is_curated_heartq_build_subset() {
        let gc = toolset_for_preset("heartq-computer").unwrap();
        let gb = toolset_for_preset("heartq-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = heartq_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "heartq-computer tool `{}` must also ship in the heartq-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "heartq-computer should be a curated subset of heartq-build"
        );
    }
    #[test]
    fn heartq_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("heartq-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&heartq_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&heartq_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&heartq_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&heartq_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&heartq_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&heartq_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&heartq_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The heartq-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the heartq-build
    ...
    #[test]
    fn heartq_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("heartq-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "heartq-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn heartq_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("heartq-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&heartq_build::LspTool).id,
            ToolConfig::from(&heartq_build::EnterPlanModeTool).id,
            ToolConfig::from(&heartq_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "heartq-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_heartq_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_heartq_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::HeartQBuildOrchestrator => true,
            BuiltinAgentName::HeartQBuild
            | BuiltinAgentName::HeartQBuildConcise
            | BuiltinAgentName::HeartQBuildPlan
            | BuiltinAgentName::HeartQBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "heartq-build-orchestrator"] {
            assert!(
        ...
        for non_strict in [
            ...
            "heartq-build-plan",
            "heartq-build-concise",
            "heartq-build-ask-user",
            "opencode",
            "browser-use",
            "custom-user-agent",
            "",
            "heartq-build-totally-made-up",
        ] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("heartq-3-fast".to_string()).to_string(),
            "heartq-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("heartq-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.heartq/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.heartq/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_heartq_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have heartq_build tools"
        );
    }
    #[test]
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"heartq-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("heartq-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("heartq-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"heartq-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("heartq-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "heartq-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("heartq-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("heartq-build", BuiltinAgentName::HeartQBuild),
            ("heartq-build-concise", BuiltinAgentName::HeartQBuildConcise),
            ("heartq-build-ask-user", BuiltinAgentName::HeartQBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_heartq_build();
