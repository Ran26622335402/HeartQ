...
//!
//! Searches `.heartq/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.heartq/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.heartq/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".heartq/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.heartq/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.heartq/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), heartq.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    heartq_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, heartq_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.heartq/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.heartq/agents/` walking from `cwd` up to repo root
/// 2. `~/.heartq/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.heartq/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user heartq agents, `.claude`
/// compat agents, then bundled. `.heartq` dirs resolve from `heartq_home`
/// (HEARTQ_HOME-aware) plus the legacy literal `~/.heartq` when HEARTQ_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    heartq_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.heartq, included only when it differs from heartq_home
    // (i.e. HEARTQ_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_heartq = home
        .map(|h| h.join(".heartq"))
        .filter(|legacy| heartq_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = heartq_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_heartq {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let heartq = heartq_config::user_heartq_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), heartq.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, heartq_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let heartq = heartq_config::user_heartq_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), heartq.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, heartq_home);
...
///
/// Project-level `.heartq/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let heartq = heartq_config::user_heartq_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), heartq.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, heartq_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, heartq_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the HeartQ
...
/// Load project agent definitions from every `.heartq/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectHeartQ,
            PluginScope::User => PluginOrigin::UserHeartQ,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_heartq_when_heartq_home_differs() {
        let home = Path::new("/home/u");
        let heartq = Path::new("/custom/heartqhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(heartq))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&heartq.join("agents")));
        assert!(paths.contains(&home.join(".heartq").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&heartq.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".heartq").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_heartq_home_is_dot_heartq() {
        let home = Path::new("/home/u");
        let heartq = home.join(".heartq");
        let count = user_agent_dirs(Some(home), Some(&heartq))
            .into_iter()
            .filter(|(p, _)| *p == heartq.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.heartq/agents when heartq_home == ~/.heartq"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_heartq_build() {
        let def = by_name("heartq-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "heartq-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".heartq").join("agents");
        let agents_dir_2 = inner_dir.join(".heartq").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".heartq").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".heartq")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".heartq")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".heartq").join("agents");
        let bundled_dir = home.join(".heartq").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".heartq")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".heartq")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".heartq").join("agents");
        let bundled_dir = home.join(".heartq").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "heartq-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "heartq-build.md",
            "heartq-build",
        ...
        let def = by_name_in_cwd("heartq-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "heartq-build");
        assert_eq!(def.description, "Custom heartq-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .heartq/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("heartq-build-orchestrator")
            .expect("from_str must resolve heartq-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::HeartQBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "heartq-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("heartq-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find heartq-build-orchestrator");
        assert_eq!(def.name, "heartq-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.heartq/agents/) agent named "explore" should NOT shadow
