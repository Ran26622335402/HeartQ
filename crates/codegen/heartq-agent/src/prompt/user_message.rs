...
//! - `Default` -- the legacy HeartQ Build prefix (built by the shell layer).
...
use heartq_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy HeartQ Build prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .heartq/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `heartq_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.heartq/, ~/.claude/).
    ...
    /// (`~/.heartq/projects/<encoded-cwd>/mcps`). Surfaced in
