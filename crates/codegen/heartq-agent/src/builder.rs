...
use std::sync::Arc;
use heartq_tools::bridge::ToolBridge;
use heartq_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
/// The HeartQ [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-HeartQ
/// tool registry in `heartq-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    heartq_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.heartq/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<heartq_tools::implementations::heartq_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn heartq_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: heartq_tools::implementations::web_search::WebSearchConfig,
    ...
    lsp: Option<std::sync::Arc<dyn heartq_tools::implementations::lsp::LspBackend>>,
    ...
    app_builder_deployer_config:
        heartq_tools::implementations::heartq_build::deploy_app::AppBuilderDeployerConfig,
    ...
    compat: heartq_tools::types::compat::CompatConfig,
    ...
    attribution_callback: Option<heartq_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in heartq-shell).
...
fn ensure_plan_mode_tools(tool_config: &mut heartq_tools::registry::types::ToolServerConfig) {
    use heartq_tools::implementations::heartq_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("HeartQBuild:enter_plan_mode");
    let missing_exit = !existing.contains("HeartQBuild:exit_plan_mode");
    let missing_ask = !existing.contains("HeartQBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&heartq_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&heartq_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&heartq_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut heartq_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(heartq_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<heartq_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn heartq_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: heartq_tools::implementations::heartq_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: heartq_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `HEARTQ_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: heartq_tools::implementations::heartq_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn heartq_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: heartq_tools::implementations::heartq_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: heartq_tools::implementations::heartq_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: heartq_tools::implementations::heartq_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: heartq_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the HeartQBuild Task description.
    ...
    ///
    /// When disabled, `HeartQBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.heartq/skills, ~/.heartq/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use heartq_tools::implementations::heartq_build;
                tool_config.tools.push((&heartq_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use heartq_tools::implementations::heartq_build;
                tool_config.tools.push((&heartq_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&heartq_tools::implementations::heartq_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&heartq_tools::implementations::heartq_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&heartq_tools::implementations::heartq_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&heartq_tools::implementations::heartq_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&heartq_tools::implementations::heartq_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&heartq_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let heartq_build_ns = heartq_tools::types::tool::ToolNamespace::HeartQBuild.to_string();
            let mem_search_id = format!(
                "{heartq_build_ns}:{}",
                heartq_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{heartq_build_ns}:{}",
                heartq_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                heartq_tools::types::tool::ToolNamespace::HeartQBuild,
            );
        ...
        if task_stripped {
            use heartq_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::HeartQBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::HeartQBuildConcise, "run_terminal_cmd", true)
        ...
        if let heartq_tools::implementations::heartq_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["HeartQBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "HeartQBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full heartq toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                heartq_tools::types::resources::TruncationCfg(
                    heartq_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(heartq_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(heartq_sampling_types::HostedTool::XSearch);
            }
        }
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        heartq_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: heartq_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: heartq_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("heartq", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use heartq_tools::types::template_renderer::TemplateRenderer;
        use heartq_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use heartq_tools::computer::local::LocalTerminalBackend;
        use heartq_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "heartq-build / subagents+ask_user",
                profile: AgentDefinition::default_heartq_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "heartq-build / subagents / no-ask-user",
                profile: AgentDefinition::default_heartq_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "heartq-build / no-subagents / ask_user",
                profile: AgentDefinition::default_heartq_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "heartq-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_heartq_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "heartq-build-ask-user / subagents",
                profile: AgentDefinition::heartq_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "heartq-build-ask-user / no-subagents",
                profile: AgentDefinition::heartq_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "heartq-build-plan",
                profile: AgentDefinition::heartq_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "heartq-build-plan / no-ask-user",
                profile: AgentDefinition::heartq_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "heartq-build-plan-no-subagents",
                profile: AgentDefinition::heartq_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "heartq-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::heartq_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use heartq_tools::computer::local::LocalTerminalBackend;
        use heartq_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_heartq_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use heartq_tools::computer::local::LocalTerminalBackend;
        use heartq_tools::implementations::heartq_build::ask_user_question::AskUserQuestionParams;
        use heartq_tools::notification::ToolNotificationHandle;
        use heartq_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_heartq_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "HeartQBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use heartq_tools::computer::local::LocalTerminalBackend;
        use heartq_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_heartq_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("HeartQBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use heartq_tools::computer::local::LocalTerminalBackend;
        use heartq_tools::implementations::heartq_build::bash::BashParams;
        use heartq_tools::notification::ToolNotificationHandle;
        use heartq_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_heartq_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their HeartQ
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_heartq_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their heartq
    ...
    /// treat `mcp__*` as always-on, so heartq keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use heartq_tools::computer::local::LocalTerminalBackend;
        use heartq_tools::implementations::heartq_build::web_fetch::WebFetchConfig;
        use heartq_tools::implementations::web_search::WebSearchConfig;
        use heartq_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_heartq_build();
    ...
    /// heartq-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to heartq's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, heartq_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, heartq_sampling_types::HostedTool::XSearch)),
