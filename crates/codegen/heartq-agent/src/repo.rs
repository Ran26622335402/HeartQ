...
//! primitive consumed cross-crate by `heartq-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.heartq/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.heartq`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `heartq-workspace`, which depends on THIS crate) to keep the dep edge
