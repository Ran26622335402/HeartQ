...
//!
//! Catalog file lookup, in order: `.heartq-plugin/marketplace.json` (preferred),
//! `.heartq-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.heartq-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.heartq-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_heartq_dir() {
        let dir = tempfile::tempdir().unwrap();
        let heartq_dir = dir.path().join(".heartq-plugin");
        std::fs::create_dir_all(&heartq_dir).unwrap();
        std::fs::write(
            heartq_dir.join("marketplace.json"),
            r#"{"name": "heartq", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "heartq");
    }

    #[test]
    fn load_index_heartq_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".heartq-plugin", "heartq"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "heartq");
    }
