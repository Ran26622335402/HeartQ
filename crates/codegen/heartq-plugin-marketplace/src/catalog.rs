...
//! `.heartq-plugin/plugin-index.json` (preferred), then
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_heartq_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".heartq-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_heartq_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".heartq-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".heartq-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".heartq-plugin",
            r#"{
                "$schema": "https://x.ai/heartq/plugin-index.schema.json",
                "version": 1,
