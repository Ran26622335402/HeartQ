...
//! Cache root: `~/.heartq/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = heartq_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(heartq_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    heartq_config::heartq_home().join("marketplace-cache")
}
...
mod tests {
    ...
    #[test]
    fn default_cache_root_under_heartq() {
