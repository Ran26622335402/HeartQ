...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p heartq-voice --bin voice-probe -- --seconds 5
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,heartq_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                heartq_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    let env_base = std::env::var("HEARTQ_XAI_API_BASE_URL").ok();
    ...
    if let Ok(home) = std::env::var("HEARTQ_HOME")
...
Environment:
  ...
  RUST_LOG        optional (default info,heartq_voice=debug)

Reads [voice] from ~/.heartq/config.toml unless --config PATH is set.
"#
    );
}
