...
use heartq_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (heartq_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    heartq_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use heartq_shell::tools::{TodoItem, TodoPriority};
