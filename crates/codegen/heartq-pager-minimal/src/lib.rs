//! Minimal (scrollback-native) render mode — `heartq --minimal`.
//!
...
//! # Wiring
//!
//! `heartq-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`heartq_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`heartq-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `heartq-pager`.
///
...
/// subsequent calls are ignored (see [`heartq_pager::minimal_hook`]).
pub fn install() {
    heartq_pager::minimal_hook::install(heartq_pager::minimal_hook::MinimalHooks { draw });
}
