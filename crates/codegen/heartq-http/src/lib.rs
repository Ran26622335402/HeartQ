...
//! `heartq_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `HEARTQ_POOL_*` /
//! `HEARTQ_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `HEARTQ_SAMPLER_SHARED_CLIENT=0` falls back to
...
use heartq_workspace::permission::ClientType;

/// Startup span timer, local to this crate.
///
/// Replaces `heartq_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `heartq_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use heartq_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `heartq-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `heartq-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `heartq-telemetry` consumes
// the same type via `heartq_sampler::OriginClientInfo`. The shell-specific
...
/// `HEARTQ_CLIENT_VERSION` env vars. Returns `None` when
/// `HEARTQ_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`HEARTQ_CLIENT_NAME` env var, default `"heartq-shell"`).
...
/// (`heartq -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-heartq-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `heartq -p`, `run_headless_inner` for
/// `heartq agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn heartq_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(heartq_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "heartq-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "heartq-desktop".to_string(),
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "heartq-pager".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("heartq-desktop/1.2.3 heartq-shell/"));
        ...
        assert!(without_version.starts_with("heartq-web heartq-shell/"));
        assert!(!without_version.starts_with("heartq-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        let ua = UserAgent {
            origin: OriginClientInfo {
                product: "heartq-shell".to_string(),
        ...
        assert_eq!(ua.render(), "heartq-shell/0.1.171 (macos; aarch64)");
    }
}
