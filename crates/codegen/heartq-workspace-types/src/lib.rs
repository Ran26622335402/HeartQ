//! Wire types for the `heartq-workspace` API.
//!
...
//! `heartq-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `heartq-workspace-types` (instead of `heartq-workspace` or
/// `heartq-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `heartq_workspace::permission`
