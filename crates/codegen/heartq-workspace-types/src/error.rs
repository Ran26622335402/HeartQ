...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] heartq_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `heartq-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `heartq_tools::ToolError`) happens
