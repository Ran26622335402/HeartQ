...
//! agent-session config types in `heartq-config` and friends.
...
/// `heartq-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `heartq_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
