...
//! `heartq-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.heartq/skills
    Local,
    /// repo_root/.heartq/skills
    Repo,
    /// ~/.heartq/skills
    User,
    /// ~/.heartq/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.heartq/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the heartq-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.heartq/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.heartq/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.heartq/plugins/infra-plugin",
            "plugin_data": "/root/.heartq/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "heartq-4",
            "effort": "high",
            "user_invocable": true,
