...
//!
//! `heartq_hooks` pulls in `git2`/`reqwest`/`heartq-tools`, too heavy for
...
/// Wire mirror of `heartq_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `heartq_hooks::config::HookSpec`.
///
...
/// `heartq-hooks`, so a server-side test (`heartq-workspace`'s
...
/// Wire mirror of `heartq_hooks::event::HookEventName`.
///
        ...
        let json = serde_json::json!({
            "hooks": {
                "pre_tool_use": [{
                    ...
                    "url_raw": null,
                    "timeout_ms": 5000,
                    "source_dir": "/home/u/.heartq/hooks",
                    "extra_env": { "FOO": "bar" }
                }]
            }
        });
