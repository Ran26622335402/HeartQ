...
//! `heartq_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `heartq-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(role.model.as_deref(), Some("heartq-3"));
        assert_eq!(role.reasoning_effort.as_deref(), Some("high"));
        assert_eq!(
            role.prompt_file.as_deref(),
            Some(".heartq/prompts/researcher.md")
        );
...
"#;
        ...
        assert_eq!(
            ...
            Some(".heartq/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("heartq-3-fast"));
