...
//!
//! Extracted from `heartq-shell/src/agent/subagent/` `normalize_forked_context()`.
...
/// `heartq-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use heartq_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(heartq_sampling_types::synthesized_reasoning_item(text))
    }
