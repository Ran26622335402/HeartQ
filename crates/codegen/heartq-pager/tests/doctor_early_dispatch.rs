...
fn pager_binary() -> Result<std::path::PathBuf, String> {
    for key in ["PAGER_BINARY", "CARGO_BIN_EXE_heartq-pager"] {
    ...
    Err("PAGER_BINARY/CARGO_BIN_EXE_heartq-pager not set".to_owned())
}
...
fn doctor_json_bypasses_unrelated_startup_state() {
    ...
    let heartq_home = temp.path().join("heartq-home");
    std::fs::create_dir_all(&home).expect("create HOME");
    std::fs::create_dir_all(&heartq_home).expect("create HEARTQ_HOME");
    ...
    let before = directory_entries(&heartq_home);
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("HeartQ Doctor"));

    let after = directory_entries(&heartq_home);
    ...
    for absent in ["docs", "crash", "memtrace", "active_sessions.json"] {
        assert!(
            !heartq_home.join(absent).exists(),
...
fn doctor_fix_yes_writes_only_actual_home_shell_rc() {
    ...
    assert_eq!(
        ...
        "# >>> heartq doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='heartq wrap ssh'\n# <<< terminal.ssh-wrap <<<\n# <<< heartq doctor <<<"
    );
    assert!(!heartq_home.join(".bashrc").exists());
}
...
fn doctor_fix_safety_boundaries_are_process_isolated() {
    ...
    let home = temp.path().join("home");
    let heartq_home = temp.path().join("qhome");
    std::fs::create_dir_all(&home).unwrap();
    std::fs::create_dir_all(&heartq_home).unwrap();
...
fn restrictive_umask_still_preserves_exact_rc_mode() {
    ...
    let mut command = base_pager_command(&binary, &home, &heartq_home, "/bin/bash");
...
fn run_pager(
    ...
    let mut command = base_pager_command(binary, home, heartq_home, shell);
