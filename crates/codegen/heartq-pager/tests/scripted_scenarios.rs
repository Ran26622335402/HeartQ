...
//!
//! These tests exercise the real `heartq-pager` binary through a PTY using
...
/// enterprise deploy report: with `HEARTQ_GOAL=1`, `/goal` must
...
/// explicitly off (`HEARTQ_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `HEARTQ_FOLDER_TRUST=1` and a git repo that
...
/// also pins `HEARTQ_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
