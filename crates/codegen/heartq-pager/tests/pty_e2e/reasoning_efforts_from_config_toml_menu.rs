...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("heartq-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.heartq/config.toml` with a per-model reasoning-effort menu.
    let heartq_home = content.home().join(".heartq");
    std::fs::create_dir_all(&heartq_home).expect("create .heartq");
    // Quote the dotted model id: bare `[model.heartq-4.5]` is TOML key-path syntax (model.heartq-4.5), not the id "heartq-4.5".
    std::fs::write(
