...
use crate::common::*;
use heartq_pager_pty_harness::{InferenceEndpoint, InferenceRequestMatcher};
...
async fn minimal_commits_thinking_body_to_scrollback() {
    ...
    std::fs::create_dir_all(content.home().join(".heartq")).expect("mk .heartq");
    std::fs::write(
        content.home().join(".heartq/config.toml"),
