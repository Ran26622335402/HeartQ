...
use heartq_pager_pty_harness::PtyHarness;
pub(crate) use heartq_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`heartq-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `heartq-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `HEARTQ_SCROLL_SPEED`) appended to the controller's env.
