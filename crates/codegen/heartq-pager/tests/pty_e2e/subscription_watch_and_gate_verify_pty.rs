...
const PAID_TIER: &str = "SuperHeartQPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "SuperHeartQ Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `superheartq_heavy` = live
/// `/user` string `SuperHeartQPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `HEARTQ_LOCAL_AUTH` dev issuer
...
/// `HEARTQ_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let heartq_home = content.home().join(".heartq");
    std::fs::create_dir_all(&heartq_home).expect("create temp .heartq");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("HEARTQ_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("heartq-3")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("HEARTQ_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://heartq.com/superheartq?referrer=heartq-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("HEARTQ_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
