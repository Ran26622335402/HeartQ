...
HeartQ Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.heartq/hooks/`
- **Project hooks** — from `.heartq/hooks/` in your repo
...
Plugin hooks automatically receive `HEARTQ_PLUGIN_ROOT` and `HEARTQ_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.heartq/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `heartq plugin marketplace add <source>`)
...
Sources land in `~/.heartq/config.toml`:
...
Or in `~/.heartq/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../heartq-hooks/examples/README.md) — ready-to-use sample hooks
