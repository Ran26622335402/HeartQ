...
HeartQ Build is a terminal-based AI coding assistant by hypercn. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
```powershell
$env:HEARTQ_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex
```

The PowerShell installer automatically adds `%USERPROFILE%\.heartq\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
Start HeartQ by running:

```bash
heartq
```

On first launch, HeartQ opens your browser to authenticate with heartq.com. After you sign in, HeartQ stores your credentials in `~/.heartq/auth.json`, where they persist across sessions. HeartQ refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Once authenticated, HeartQ presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, HeartQ's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. HeartQ reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, HeartQ asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `heartq --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.heartq/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `heartq -c`
...
- **Agent messages** -- HeartQ's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- HeartQ's reasoning process (collapsible)
...
```
/model heartq-build                 # Switch model
...
# Experimental scrollback-native render mode. Sticky: plain `heartq` reopens in
...
Run HeartQ non-interactively for scripting, CI/CD, and automation:
...
```bash
heartq -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. HeartQ reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.heartq/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. HeartQ also reads `CLAUDE.md` files for compatibility.
