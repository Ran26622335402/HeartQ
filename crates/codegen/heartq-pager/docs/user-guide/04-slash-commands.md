...
Commands come from two places: **shell builtins**, handled by the agent backend (heartq-shell), and **pager builtins**, handled by the TUI frontend (heartq-pager). Both show up in the same menu, and any enabled skill with `user-invocable: true` appears there too.
...
Compress conversation history to reclaim context-window space. Pass a note to tell HeartQ what to keep:
...
HeartQ also auto-compacts once the context window hits 85% (tune it with `[session] auto_compact_threshold_percent`).
...
In minimal mode, open an external editor for an empty composer. HeartQ resolves `$VISUAL`, then `$EDITOR`, then `vi`; command values may include quoted arguments. Saving replaces the draft without sending it, and saving an empty file clears it. The command is hidden outside minimal mode.
...
Every copy is also written to a backup file — `~/.heartq/last-copy.txt` by default, or `HEARTQ_COPY_FILE` if set — and the toast tells you exactly where the text landed, so you can retrieve it even when the clipboard couldn't be reached or the copy went out as an OSC 52 escape this terminal couldn't confirm.
...
`/flush`, `/dream`, and `/memory` require memory to be enabled (`--experimental-memory` or `HEARTQ_MEMORY=1`); `/memory` also needs a configured memory backend. `/remember` is always available.
...
Run a prompt on a recurring interval. Give the interval as `30m`, `1 hour`, or `every 2 days`; leave it out and HeartQ will ask.
...
Set, manage, or check an autonomous goal. HeartQ works across rounds and only marks the goal complete after an independent evidence review confirms the claim; if that review can't reproduce the result or has no usable evidence, the goal stays active or pauses with concrete gaps.
...
Project workflows live in `.heartq/workflows/*.rhai`; user workflows live in `~/.heartq/workflows/*.rhai`. A same-process pause/resume continues the original immutable script, args, and `agent_budget` cap from committed host-call results — to iterate, edit the returned script copy and launch it as a new run.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter shows up as a slash command. (Turn a skill off via `/skills` and it stops being advertised.) So a skill at `~/.heartq/skills/commit/SKILL.md` runs as:
