...
Agent mode runs HeartQ as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`heartq -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `heartq agent` command and apply to every mode. Pass them before the mode name, for example `heartq agent --model heartq-build stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `heartq-build`).                    |
...
```bash
heartq agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `HEARTQ_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, HeartQ defines extension methods under the `heartq.dev/` prefix for HeartQ-specific functionality. These cover:
...
The tables here show representative methods in each category. The `heartq.dev/*` set is HeartQ-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
