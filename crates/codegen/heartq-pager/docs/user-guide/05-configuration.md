...
HeartQ reads settings from config files, environment variables, and CLI flags. This page covers the common options.
...
2. **Environment variables** (e.g. `XAI_API_KEY`, `HEARTQ_MEMORY`)
3. **config.toml** (`~/.heartq/config.toml`)
...
Location: `~/.heartq/config.toml`. If the file is missing, HeartQ uses its built-in defaults, so you only need to set the values you want to override.
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); HeartQ writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
You can also override this with `HEARTQ_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
Toggle it at runtime with `/vim-mode`, or from `/settings` → **Vim scrollback navigation**. HeartQ writes the change to `[ui] vim_mode` immediately and applies it to every future pager session, including new agents and subagents in the same process. There's no per-session override — `config.toml` is the source of truth on next launch. `vim_mode` is independent of `simple_mode`.

#### Screen mode

`[ui] screen_mode` is the **default render mode** for plain `heartq` launches. Set it from `/settings` → **Default screen mode** (restart required) or edit `config.toml` by hand — both write the file. CLI flags (`--minimal` / `--fullscreen`) and slash commands (`/minimal` / `/fullscreen`) are session-scoped and do **not** write this key; after a slash switch, the reverse command returns you for that session only.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `HEARTQ_SCROLL_SPEED`, `HEARTQ_SCROLL_MODE`, `HEARTQ_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `HEARTQ_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
MCP servers can also be set per-project in `.heartq/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.heartq/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.heartq/config.toml` (current dir) > `<repo-root>/.heartq/config.toml` > `~/.heartq/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `HEARTQ_MEMORY=1`).
...
[subagents.models]
explore = "heartq-build"               # route to different models
```
...
Background workflows — the `workflow` tool, named `.heartq/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **off by default**.

```toml
[workflows]
enabled = true                        # enable background workflows (or HEARTQ_WORKFLOWS=1)
```

Project workflows are discovered from `<repo-root>/.heartq/workflows/`; user workflows from `~/.heartq/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`heartq inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences — mostly "stop asking me" opt-outs. HeartQ writes these for you when you pick a "don't ask again" option in the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** opt-outs to your user `~/.heartq/config.toml`.
...
|-----|------|---------|-------------|
| `project_picker_disabled` | bool | `false` | When `true`, skips the picker that asks you to choose a project directory on the first prompt when HeartQ launches from a non-project directory (home, Desktop, Downloads, `/tmp`). Set automatically when you choose **"Don't ask me again"** in that picker. Teams can pin it in `managed_config.toml` or `requirements.toml`. |
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "heartq"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `heartq`. |
...
| HeartQ Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, HeartQ detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$HEARTQ_EVENT`, `$HEARTQ_MESSAGE`, and `$HEARTQ_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'HeartQ' -message '$HEARTQ_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$HEARTQ_MESSAGE' ntfy.sh/my-heartq-alerts"
...
**Focus tracking not working:** some terminals don't report focus events. If `condition = "unfocused"` never fires, try `condition = "always"`. HeartQ supports focus tracking in every detected terminal except Apple Terminal and unrecognized ones.
...
- **`[features] telemetry`** / `HEARTQ_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **`/privacy`** / Settings — coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `HEARTQ_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `HEARTQ_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by HeartQ — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= HEARTQ_EXTERNAL_OTEL)
...
Location: `~/.heartq/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `HEARTQ_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `HEARTQ_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `HEARTQ_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `HEARTQ_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `HEARTQ_OIDC_ISSUER` | OIDC issuer URL |
| `HEARTQ_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `HEARTQ_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `HEARTQ_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `HEARTQ_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default off: legacy `update_goal`; on: host-owned workflow driver) |
| `HEARTQ_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `HEARTQ_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `HEARTQ_AGENT` | Custom agent definition path or name |
| `HEARTQ_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `HEARTQ_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `HEARTQ_LOG_FILE` log and headless stderr output |
...
| `HEARTQ_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `HEARTQ_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `HEARTQ_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `HEARTQ_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `HEARTQ_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `HEARTQ_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `HEARTQ_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.heartq/config.toml` | Main configuration file |
| `~/.heartq/pager.toml` | TUI appearance configuration |
...
| `~/.heartq/lsp.json` | LSP server configuration (user-scoped) |
| `~/.heartq/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.heartq/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some settings can be set per-project by placing files in `.heartq/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.heartq/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.heartq/config.toml`) |
| `.heartq/skills/` | Project-specific skills |
| `.heartq/hooks/` | Project-specific lifecycle hooks |
| `.heartq/agents/` | Project-specific agent definitions |
| `.heartq/lsp.json` | LSP server configuration |
| `.heartq/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.heartq/lsp.json` | All projects |
| Project | `.heartq/lsp.json` | Current repository |
...
1. **Project** — `.heartq/lsp.json`
2. **User** — `~/.heartq/lsp.json`
