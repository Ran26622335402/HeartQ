...
- **`heartq dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.heartq/config.toml` under `[keybindings]` like every other shortcut.
...
```
 HeartQ Build · Dashboard — 4 agents · 2 awaiting
...
The state icon matches HeartQ Build's sibling views (
`tasks_pane`):
...
rebound via `~/.heartq/config.toml`.
...
Set `HEARTQ_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `heartq --bg`") would list
