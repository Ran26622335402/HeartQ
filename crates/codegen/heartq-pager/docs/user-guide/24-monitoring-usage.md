...
> **Status: alpha.** The schema below is versioned (`heartq_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

HeartQ CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through HeartQ.
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `HEARTQ_TELEMETRY_ENABLED` |
| `/privacy` | `/privacy opt-in` / `/privacy opt-out`, or Settings |
| Trace upload | `[telemetry] trace_upload` / `HEARTQ_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `HEARTQ_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
- **Structurally separate** by hypercn-internal telemetry: its exporters carry
  only the headers you configure, never HeartQ credentials.
- **Independent of HeartQ data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  settings govern HeartQ-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export HEARTQ_EXTERNAL_OTEL=1                  # master switch
...
`HEARTQ_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `HEARTQ_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `HEARTQ_TELEMETRY_ENABLED`, which controls HeartQ-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `heartq_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `HEARTQ_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`HEARTQ_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `heartq setup` managed config /
...
## Metrics (meter scope `ai.xai.heartq_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `heartq_code.session.count` | `{session}` | base attrs only |
| `heartq_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `heartq_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `heartq_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `heartq_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `heartq_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `heartq_code.token.usage` with your own
...
|---|---|
| `heartq_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `heartq_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `heartq_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `heartq_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `heartq_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `heartq_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `heartq_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `heartq_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `heartq_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `heartq_code.permission_mode_changed` | `to_mode`, `trigger` |
| `heartq_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `heartq_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `heartq_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `heartq_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `heartq_code.auth` | `auth_method` |
| `heartq_code.internal_error` | `error_type` (class only — no message, no location) |
| `heartq_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(heartq_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(heartq_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(heartq_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(heartq_code_tool_decision_total[1h]))
```
