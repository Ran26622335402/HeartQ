...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `HEARTQ_PLUGIN_ROOT` and `HEARTQ_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
- **MCP servers** -- a `.mcp.json` file of server configurations
- **LSP servers** -- a `.lsp.json` file of language server configurations

If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, HeartQ discovers the components above from their standard directories.
...
| `HEARTQ_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |

HeartQ sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (HeartQ also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.

---

## Plugin locations

HeartQ discovers plugins from these locations, in priority order:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session -- loaded for that session only | Trusted automatically |
| `--plugin-dir` (CLI flag, `heartq agent`) | Process -- loaded for that agent process only | Trusted automatically |
| `.heartq/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.heartq/plugins/` | User -- personal plugins for every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add in `config.toml` | Depends on location |

HeartQ also reads the `.claude/plugins/` equivalents for compatibility. When two plugins share a name, the higher-priority location wins.

The Agent SDKs load per-session plugins through `HeartQOptions.plugins`, which arrives as `_meta.pluginDirs` on `session/new` and `session/load`; because the caller controls the directory, these plugins are always trusted -- their hooks and MCP servers activate without a prompt, and they never persist beyond the session. The `--plugin-dir` flag is the process-wide equivalent for direct CLI use (repeatable: `heartq agent --no-leader --plugin-dir A --plugin-dir B stdio`); it applies to dedicated agent processes only and is ignored in leader mode (the shared leader discovers its own plugins).
...
```bash
heartq plugin list [--json] [--available]   # List installed plugins (--available requires --json)
heartq plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
heartq plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
heartq plugin update [<name>]               # Omit the name to update all plugins
heartq plugin enable <name>
heartq plugin disable <name>
heartq plugin details <name>                # Show the plugin's component inventory
heartq plugin validate [<path>]             # Validate plugin.json (default: current directory)
heartq plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `heartq plugin install <source>` without `--trust` and HeartQ prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
or `HEARTQ_MARKETPLACE_REQUIRE_SHA=1`. Both are tighten-only: either one enables
...
```bash
heartq plugin marketplace list [--json]
heartq plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
heartq plugin marketplace remove <url>      # Git URL or local path of a configured source
heartq plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
heartq plugin marketplace add my-org/team-plugins
heartq plugin marketplace list
heartq plugin install my-org/team-plugins --trust
heartq plugin list
heartq plugin update
```
...
In an interactive session, these commands open the modal on a specific tab. They take no arguments — manage plugins from the modal or with the `heartq plugin` CLI.
...
Configure plugin directories and per-plugin state in `~/.heartq/config.toml`:
...
List a plugin in `disabled` to discover it but skip loading its components. List a plugin in `enabled` to activate it — plugins are disabled by default unless a CLI override or an explicit config path enables them, so add them here to turn them on. Each entry is either a plain plugin name (as shown by `heartq plugin list`) or a full plugin ID in the form `<scope>/<hash>/<name>`.

### Hide the plugins UI

To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.heartq/pager.toml`:
...
Place this file at `~/.heartq/settings.json` or `~/.claude/settings.json`.
...
HeartQ trusts plugins from `~/.heartq/plugins/` automatically. Project plugins in `.heartq/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
heartq plugin install <source> --trust
```
...
Run `heartq inspect` to see every discovered plugin and what it provides:

```bash
heartq inspect          # Show plugins with their skills, agents, hooks, and MCP servers
heartq inspect --json   # Emit machine-readable JSON
```
