...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.heartq/agents/` or `~/.heartq/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.heartq/personas/`; applied during subagent resolution |
...
| **Examples** | `heartq-build`, `explore`, `plan` | `researcher`, `concise` |
...
HeartQ Build discovers file-based personas from these locations, in priority order:

- `.heartq/personas/*.toml` (project)
- `~/.heartq/personas/*.toml` (user)
...
> **Note:** HeartQ Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, HeartQ Build resolves the effective model and reasoning effort in this order, highest priority first:
...
HeartQ Build manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "heartq-build"               # route explore to a specific model
```
...
# instructions_file = ".heartq/personas/concise.md"  # or load from a file
```

HeartQ Build also discovers roles from `.heartq/roles/*.toml` and personas from `.heartq/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

HeartQ Build shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · heartq-3) — Thinking`
