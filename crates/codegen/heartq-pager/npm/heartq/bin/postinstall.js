#!/usr/bin/env node
// Runs once after npm install/update. Reads the heartq binary from the
// matching per-platform optional dependency (@xai-official/heartq-<platform>)
// and installs it to ~/.heartq/bin/ using versioned filenames:
//
//   Unix:    heartq-<version>  +  heartq  (symlink)
//   Windows: heartq-<version>.exe  +  heartq.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.heartq', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@xai-official/heartq: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@xai-official/heartq-${key}`;
    try {
...
if (!version) {
    console.error('@xai-official/heartq: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@xai-official/heartq: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@xai-official/heartq: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running heartq processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid heartq-* matching heartq-pager-*.
...
if (!platformDir) {
    console.error(`@xai-official/heartq: platform package @xai-official/heartq-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @xai-official/heartq');
    process.exit(0);
}

installBinary('heartq', platformDir, `heartq${EXE}`);
cleanupOldVersions('heartq');
cleanupOldVersions('heartq-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.heartq');
...
// Persist the npm registry so `heartq update` and the trampoline use the same one.
const npmRegistry = process.env.HEARTQ_NPM_REGISTRY
    || (() => {
        try {
...
// Set HEARTQ_INSTALL_COMPLETIONS=1 to auto-generate to ~/.heartq/completions.
const HEARTQ_PATH = path.join(CANONICAL_DIR, `heartq${EXE}`);
if (process.env.HEARTQ_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.heartq', 'completions');
        const bashPath = path.join(completionsDir, 'bash', 'heartq.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_heartq');
        fs.mkdirSync(path.dirname(bashPath), { recursive: true });
        fs.mkdirSync(path.dirname(zshPath), { recursive: true });
        const bashRes = spawnSync(HEARTQ_PATH, ['completions', 'bash'], { encoding: 'utf8' });
        if (bashRes.status === 0) fs.writeFileSync(bashPath, bashRes.stdout);
        const zshRes = spawnSync(HEARTQ_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.heartq/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
    console.log('Tip: heartq completions bash > ~/.local/share/bash-completion/completions/heartq');
    console.log('     heartq completions zsh  > ~/.zsh/completions/_heartq');
}
