...
//   1. Brotli-compresses the built binary into `../heartq-<platform>/bin/<bin>.br`
...
// (`@xai-official/heartq`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw heartq binary is
...
const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'heartq-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `heartq-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] heartq-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'heartq',
            envVar: 'HEARTQ_DARWIN_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'release', 'heartq-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'heartq',
            envVar: 'HEARTQ_DARWIN_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-apple-darwin', 'release', 'heartq-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'heartq',
            envVar: 'HEARTQ_LINUX_X64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'heartq-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'heartq',
            envVar: 'HEARTQ_LINUX_ARM64',
            defaultSource: path.join(xaiRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'heartq-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'heartq.exe',
            envVar: 'HEARTQ_WIN32_X64',
            defaultSource: path.join(xaiRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'heartq-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'heartq.exe',
            envVar: 'HEARTQ_WIN32_ARM64',
            defaultSource: path.join(xaiRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'heartq-pager.exe'),
        },
    ];
