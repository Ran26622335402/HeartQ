...
use heartq_shell::auth::{AuthManager, try_ensure_fresh_auth};
use heartq_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // If the user has previously run the interactive `heartq` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.heartq_com_config).await;
    ...
    let client = heartq_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = heartq_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use heartq_shell::session::merge::REMOTE_TIMEOUT;
            use heartq_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = heartq_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
