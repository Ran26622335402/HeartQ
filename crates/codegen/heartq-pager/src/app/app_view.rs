...
/// [`heartq_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<heartq_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: heartq_shell::util::config::WorktreeHintMode) -> Self {
        use heartq_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        heartq_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            heartq_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`heartq_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `HEARTQ_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `HEARTQ_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("HEARTQ_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`heartq_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => heartq_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: heartq_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`heartq_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.heartq/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`HEARTQ_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `HEARTQ_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<heartq_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<heartq_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `HEARTQ_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `heartq dashboard`, the
    ...
    /// non-selectable headers. Gated by `HEARTQ_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `HEARTQ_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `heartq-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<heartq_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        heartq_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `heartq_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `HEARTQ_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: heartq_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<heartq_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `heartq wrap` tip: set after the first
    ...
    /// Login button label from `AuthMethod.name` (e.g., "heartq.com", "Acme Corp").
    ...
    /// `heartq_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, heartq_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `heartq_build_access_gate`. `Some` = blocked.
    pub gate: Option<heartq_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperHeartQ", "Free").
    ...
    pub pending_gate_verification: Option<heartq_shell::auth::GateInfo>,
    ...
    /// `~/.heartq/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `HEARTQ_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &heartq_shell::util::config::RemoteSettings,
    ) -> Option<heartq_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(heartq_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &heartq_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: heartq_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: heartq_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: heartq_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<heartq_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && heartq_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: heartq_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(heartq_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(heartq_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `HEARTQ_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<heartq_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    heartq_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSuperheartqUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        heartq_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSuperheartqUrl),
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                heartq_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            heartq_telemetry::session_ctx::log_event(
                                heartq_telemetry::events::SuperHeartQUpsellShown {
                                    source:
                                        heartq_telemetry::events::SuperHeartQUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`heartq_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use heartq_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = heartq_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                heartq_telemetry::session_ctx::log_event(
                    heartq_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: heartq_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `heartq wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::ContextualTip {
                tip: heartq_telemetry::events::ContextualTipKind::ImageInput,
                action: heartq_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![heartq_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![heartq_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: heartq_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_disables_billing_surface_for_team_users() {
        ...
        let meta = heartq_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_enables_billing_surface_for_personal_users() {
        ...
        let meta = heartq_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_restores_billing_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&heartq_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&heartq_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = heartq_shell::auth::AuthMeta {
            subscription_tier: Some("SuperHeartQ".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&heartq_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperHeartQ".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperHeartQ")));
        assert!(!is_restricted_tier(Some("SuperHeartQ Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(heartq_shell::auth::GateInfo {
            message: "Subscribe to use HeartQ Build".into(),
            url: Some("https://heartq.com/superheartq?referrer=heartq-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = heartq_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(heartq_workspace::foreign_sessions::RecentForeignSession {
                tool: heartq_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![heartq_workspace::file_system::FuzzyMatchResult {
