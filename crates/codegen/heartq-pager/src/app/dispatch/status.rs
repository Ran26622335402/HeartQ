...
pub(super) fn dispatch_show_privacy_info(app: &mut AppView) -> Vec<Effect> {
    ...
    if app.is_zdr {
        ...
        lines.push("  Usage and code data stay on-device; HeartQ (by HyperAI) does not phone home.");
        lines.push("");
    ...
    lines.push("  - [features] telemetry / HEARTQ_TELEMETRY_ENABLED");
    lines.push("  - [telemetry] trace_upload / HEARTQ_TELEMETRY_TRACE_UPLOAD");
    lines.push("  - HEARTQ_EXTERNAL_OTEL / OTEL_*");
    lines.push("");
...
/// (persists via ACP ext-request, NOT `~/.heartq/config.toml`).
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`heartq_build_usage_redirect_url`): link out
...
pub(super) fn dispatch_manage_billing(app: &mut AppView) -> Vec<Effect> {
    ...
    super::router::dispatch(
        crate::app::actions::Action::OpenUrl("https://heartq.com/?_s=usage".to_string()),
        app,
    )
}
