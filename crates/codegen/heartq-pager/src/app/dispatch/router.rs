...
use super::billing::dispatch_open_superheartq_url;
use super::ctx::{
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(heartq_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, heartq_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(heartq_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(heartq_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::OpenSuperheartqUrl => dispatch_open_superheartq_url(app),
        Action::OpenUrl(url) => {
        ...
        // Render-layer wiring is intentionally deferred. Day 3 lands the
        // slash command + Action variants so the command palette parses
        // and the dispatch is type-correct; the actual UI render is a
        // separate change in `views/slash_dropdown.rs` / `dashboard.rs`.
        // For now we no-op so existing flows are unaffected.
        Action::MetaList(_) | Action::MetaRunsList(_) | Action::MetaRunShow(_) | Action::MetaRunSteps(_) => vec![],
    };
    app.reconcile_foreign_resume_launch();
