...
use heartq_telemetry::events::{SuperHeartQUpsell, SuperHeartQUpsellClicked};
...
/// Whether the user is at the highest subscription tier (SuperHeartQ Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("superheartq_heavy") and CCP display names ("SuperHeartQ Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "superheartq_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://heartq.com/superheartq?referrer=heartq-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://heartq.com?_s=usage";
...
///
/// **`max_tier = true`** (positively identified as SuperHeartQ Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        heartq_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using HeartQ Build",
            CreditLimitCardAction::PurchaseCredits,
            heartq_telemetry::events::CreditLimitChoice::PurchaseCredits,
            false,
        ),
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            heartq_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(heartq_telemetry::events::CreditLimitUpsellShown {
            surface: heartq_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(heartq_telemetry::events::CreditLimitUpsellShown {
        surface: heartq_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            heartq_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperHeartQUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_superheartq_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperHeartQ upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_superheartq_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperHeartQ upsell modal. Controls the heading
...
fn open_superheartq_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperHeartQUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperHeartQ.",
            SuperHeartQUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(heartq_telemetry::events::SuperHeartQUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperHeartQ".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperHeartQ Heavy".into(),
            description: "Get the most out of HeartQ Build. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /superheartq page lists
...
pub(super) fn handle_gate_refreshed(
    app: &mut AppView,
    settings: Option<heartq_shell::util::config::RemoteSettings>,
) -> Vec<Effect> {
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<heartq_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<heartq_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_superheartq_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SuperHeartQUpsellClicked {
        source: SuperHeartQUpsell::WelcomeScreen,
    ...
    let url = app
        ...
        .unwrap_or("https://heartq.com/superheartq?referrer=heartq-build");
    // Funnel attribution: tag CLI-originated SuperHeartQ upsell clicks
    // with `referrer=heartq-build`, matching the OAuth consent flow and
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "heartq-build");
