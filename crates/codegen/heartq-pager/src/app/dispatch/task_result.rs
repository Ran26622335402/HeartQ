...
/// Whether a completed clipboard probe should fall through to the `heartq wrap`
/// host-image request. A clean `FullMiss` always qualifies; a remote read
/// *error* (`AttachmentRead`) also qualifies because inside `heartq wrap` the
...
/// `osc52_sink_active()`, so this is inert outside `heartq wrap`.
    ...
    match result {
        ...
        TaskResult::ForeignResumeCwdCanonicalized {
                ...
                && let Some(canonical_cwd) = accepted_cwd
            {
                vec![Effect::DetectForeignResumeHint {
                    ...
                    heartq_home: heartq_tools::util::heartq_home::heartq_home(),
                    launch_token,
                }]
            } else {
                vec![]
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets =
                heartq_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
