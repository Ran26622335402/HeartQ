...
fn show_privacy_info_zdr() {
    ...
    assert!(
        text.contains("HEARTQ_TELEMETRY_ENABLED") && text.contains("HEARTQ_EXTERNAL_OTEL"),
...
fn show_privacy_info_opted_out() {
    ...
    assert!(
        text.contains("Other settings (not changed by /privacy)")
            && text.contains("HEARTQ_TELEMETRY_ENABLED")
            && text.contains("trace_upload")
            && text.contains("HEARTQ_EXTERNAL_OTEL"),
...
fn dispatch_confirm_reset_setting_reset_dispatches_typed_setter_for_shared_enum() {
    ...
    with_theme_test_env(|| {
        ...
        // Reset → SetTheme("heartqnight") (the registered default).
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting { key, value, .. } => {
                assert_eq!(*key, "theme");
        ...
        assert_eq!(app.current_ui.theme.as_deref(), Some("heartqnight"));
    });
}
