#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.heartq` cannot leak in.
    #[test]
...
if hidden_ids == &expected
    ...
    /// through the layer-injected seam (no real `~/.heartq` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.heartq` reads).
    #[test]
