...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: heartq_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open heartq.com in the browser for SuperHeartQ subscription upsell.
    OpenSuperheartqUrl,
    ...
    /// Open heartq.com managed connectors, appending session teamId when set.
    ...
    OpenTranscriptPager,
    /// Minimal mode (`heartq --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: heartq_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    AnnouncementsOpenCta(heartq_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"heartqnight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `heartq dashboard`).
    OpenDashboard,
    ...
    // ─── Meta-skill framework (Phase opensquilla-import P0.1) ───────────
    //
    // Payloads live in `crate::slash::commands::meta` and are rendered by
    // the slash_dropdown / dashboard view layer. The slash command lives
    // in `slash/commands/meta.rs`.

    /// `/meta list` — show available meta-skills in
    /// `${HEARTQ_HOME}/meta_skills/`.
    MetaList(crate::slash::commands::meta::MetaListPayload),
    /// `/meta runs list` — show recent meta-skill runs.
    MetaRunsList(crate::slash::commands::meta::MetaRunsListPayload),
    /// `/meta runs show <id>` — full JSON dump of one run.
    MetaRunShow(crate::slash::commands::meta::MetaRunShowPayload),
    /// `/meta runs steps <id>` — per-step table for one run.
    MetaRunSteps(crate::slash::commands::meta::MetaRunStepsPayload),
}
/// Persist-and-notify semantics for [`Effect::PersistPermissionMode`].
///
/// Both variants write to `~/.heartq/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: heartq_workspace::foreign_sessions::EnabledForeignSessionSources,
        heartq_home: std::path::PathBuf,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.heartq/config.toml`.
    ...
    /// Persist a typed setting to `~/.heartq/config.toml`. On failure,
    ...
    /// registry (`~/.heartq/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`heartq-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<heartq_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionListLoaded {
        ...
        scope: heartq_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<heartq_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<heartq_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<heartq_shell::util::changelog::ChangelogEntry>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<heartq_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<heartq_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<heartq_shell::extensions::notification::PromptUsage>,
    },
    ...
    DeepSearchResults {
        results: Vec<heartq_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
