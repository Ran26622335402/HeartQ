...
/// [`decide`](heartq_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("HEARTQ_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = heartq_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(heartq_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = heartq_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match heartq_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = heartq_config::env_bool("HEARTQ_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("HEARTQ_SESSION_PICKER_GROUPED")
        .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the heartq.com method from the advertised list
            let heartq_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "heartq.com");
            if let Some(method) = heartq_com {
                ...
                // No heartq.com method available, use the first method as fallback
        ...
        // fail-closed: do not invent heartq.com / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    heartq_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<heartq_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == heartq_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = heartq_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = heartq_shell::config::load_merged_requirements();
    let user_config = heartq_shell::config::load_from_disk().ok();
    let managed_config = heartq_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match heartq_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = heartq_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > HEARTQ_XAI_API_BASE_URL env > default).
            let endpoints_base =
                heartq_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                heartq_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to heartq-cli server-side (mirrors sampler / imagine). Done after
    ...
    app.zdr_access_enabled = heartq_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        heartq_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        heartq_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        heartq_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use heartq_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = heartq_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = heartq_shell::util::tips::pick_and_advance(&app.tips, &heartq_home);
        }
    }

    let hints = heartq_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.heartq/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        heartq_shell::util::config::clamped_display_permission_mode(
            heartq_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        heartq_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = heartq_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = heartq_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // HEARTQ_SCROLL_SPEED/_MODE/_LINES + HEARTQ_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<heartq_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`heartq "fix the bug"`). When
    ...
    // `heartq dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `HEARTQ_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("HEARTQ_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(heartq_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart heartq");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.heartq/config.toml [ui]`
                // rather than `~/.heartq/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = heartq_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> heartq_shell::agent::config::UiConfig {
    use heartq_shell::agent::config::UiConfig;
    let Ok(root) = heartq_shell::config::load_effective_config() else {
