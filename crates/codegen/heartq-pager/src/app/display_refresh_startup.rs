...
/// Inclusive bounds for motion cadence env knobs (`HEARTQ_MIN_DRAW_MS`,
/// `HEARTQ_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("HEARTQ_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("HEARTQ_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        heartq_telemetry::session_ctx::log_event(t.clone());
        ...
        heartq_telemetry::session_ctx::log_event(
            heartq_telemetry::events::DisplayRefreshProbe {
                terminal: t,
