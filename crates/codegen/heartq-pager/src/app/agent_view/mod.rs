...
pub struct AgentView {
    ...
    pub context_state: Option<heartq_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<heartq_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(heartq_telemetry::events::CreditLimitChoice::PayAsYouGo);
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::SuperHeartQUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use heartq_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        heartq_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = heartq_tools::util::heartq_home::heartq_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &heartq_tools::implementations::heartq_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
