...
thread_local! {
    static REPLAY_HEARTQ_HOME : std::cell::RefCell < Option < std::path::PathBuf >> = const
    { std::cell::RefCell::new(None) };
}
/// Override heartq home for disk-replay unit tests (thread-local; production never sets this).
#[cfg(test)]
pub(crate) fn set_replay_heartq_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_HEARTQ_HOME.with(|h| *h.borrow_mut() = home);
}
fn effective_heartq_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_HEARTQ_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_heartq_home(), parent_cwd, parent_session_id);
}
...
pub(crate) fn replay_inherited_updates(
    ...
    let home = effective_heartq_home();
    let updates =
        match heartq_shell::session::storage::load_updates_for_replay_at(child_session_id, &home)
        {
            Ok(Some(u)) => u,
            Ok(None) => return,
            Err(e) => {
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_heartq_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_heartq_home_for_tests(None);
    }
    #[test]
    ...
    fn subagent_meta_all_fields() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), Some("analyst"), Some("heartq-3")),
            " (researcher \u{00b7} analyst \u{00b7} heartq-3)"
        );
    }
    #[test]
    fn subagent_meta_partial_skips_nones() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), None, Some("heartq-3")),
            " (researcher \u{00b7} heartq-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_collapses_duplicate_persona_role() {
        assert_eq!(
            format_subagent_meta(Some("reviewer"), Some("reviewer"), Some("heartq-3")),
            " (reviewer \u{00b7} heartq-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_drops_both_empty_persona_role() {
        assert_eq!(
            format_subagent_meta(Some(""), Some(" "), Some("heartq-3")),
            " (heartq-3)"
        );
    }
    #[test]
