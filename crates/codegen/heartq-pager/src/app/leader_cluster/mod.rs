...
//!
//! Env sandboxing follows this crate's `serial(HEARTQ_HOME)` idiom; note
//! `heartq_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_heartq_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `HEARTQ_LEADER_SOCKET`, `HEARTQ_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `heartq_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p heartq-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The heartq home the agent actually persisted under: `heartq_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_heartq_home() -> PathBuf {
    heartq_config::heartq_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same HEARTQ_HOME (two agents on one updates.jsonl is the
    ...
    _heartq_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(HEARTQ_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = heartq_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("HEARTQ_HOME", heartq_home.path()),
            crate::test_util::EnvVarGuard::set("HEARTQ_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("HEARTQ_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("HEARTQ_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("HEARTQ_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("HEARTQ_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (heartq_version), not
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                heartq_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(heartq_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        // heartq-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: heartq-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same HEARTQ_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `HEARTQ_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    heartq_ws_url: String::new(),
                    heartq_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
