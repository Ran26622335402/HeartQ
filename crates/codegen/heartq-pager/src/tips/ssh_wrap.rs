//! SSH wrap tip: over SSH without `heartq wrap`, advertise that wrapping the
...
/// Ephemeral-tip dedup key for the SSH `heartq wrap` hint.
...
/// Build "Over SSH? Run `heartq wrap ssh <host>` locally for clipboard +
    ...
    EphemeralTip {
        ...
        ..EphemeralTip::new(
            ...
            Line::from(vec![
                ...
                Span::styled("heartq wrap ssh <host>", command),
...
mod tests {
    ...
    fn ssh_wrap_tip_advertises_local_wrap() {
        ...
        assert_eq!(
            text,
            "Over SSH? Run heartq wrap ssh <host> locally for clipboard + terminal restore"
        );
    }
