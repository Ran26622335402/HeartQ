...
use heartq_shell::agent::auth_method::AuthMethodKind;
use heartq_shell::agent::config::Config as AgentConfig;
use heartq_shell::sampling::types::ReasoningEffort;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a heartq-shell instance.
    pub is_heartq_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<heartq_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `heartq.com`).
    ...
    pub auth_manager: std::sync::Arc<heartq_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<heartq_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_heartq_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(heartq_shell::auth::AuthManager::new(
        ...
        agent_config.heartq_com_config.clone(),
    ));

    // Leader has no in-process agent; init this process's product telemetry client.
    heartq_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = heartq_shell::util::heartq_home::heartq_home().join("config.toml");
...
async fn initialize(
    ...
    // Check if this is a heartq-shell agent
    let is_heartq_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("heartqShell"))
...
///
/// Matches TUI startup behavior: if the first method is `heartq.com`, defer auth
...
/// for a `heartq.com` or `oidc` method — these are the ones that can trigger
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "heartqShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "heartqShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_heartq_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("heartq.com", "heartq.com", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("heartq.com"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "heartq.com");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_heartq_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("heartq.com", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_heartq_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.heartq/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use heartq_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `heartq.com`
    ...
    /// `startup_auth_heartq_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use heartq_shell::agent::auth_method::{HEARTQ_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(HEARTQ_COM_METHOD_ID, "HeartQ", None),
            make_auth_method(XAI_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with heartq.com first, the pager must require login -- pinning \
    ...
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("heartq.com", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("heartq.com", "heartq.com", Some(meta))];
