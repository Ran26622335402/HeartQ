...
//! the generic [`CodeBlockSpan`](heartq_markdown::CodeBlockSpan) API) and
...
use heartq_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](heartq_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](heartq_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a heartq_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `HeartQDay` is the only light theme; every other concrete theme (and the
/// `HeartQNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `heartq_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::HeartQDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_heartqday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::HeartQDay),
            "HeartQDay is the light theme"
        );
        for dark in [
            ThemeKind::HeartQNight,
