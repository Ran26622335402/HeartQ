...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "heartq-4.5".into(),
            new_model_id: "heartq-build".into(),
            reason: "Model \"heartq-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"heartq-4.5\" is no longer available. Switched to \"heartq-build\"."
        );
    }
