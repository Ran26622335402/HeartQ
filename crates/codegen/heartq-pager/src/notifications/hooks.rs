...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("HEARTQ_EVENT", event_str)
        .env("HEARTQ_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("HEARTQ_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'HEARTQ_EVENT=%s\\nHEARTQ_MESSAGE=%s\\nHEARTQ_SESSION_ID=%s\\n' \
             \"$HEARTQ_EVENT\" \"$HEARTQ_MESSAGE\" \"$HEARTQ_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("HEARTQ_EVENT=Turn complete"),
            "missing HEARTQ_EVENT: {content}"
        );
        assert!(
            content.contains("HEARTQ_MESSAGE=hello world"),
            "missing HEARTQ_MESSAGE: {content}"
        );
        assert!(
            content.contains("HEARTQ_SESSION_ID=sess-42"),
            "missing HEARTQ_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("HEARTQ_SESSION_ID"),
            "HEARTQ_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("HEARTQ_EVENT=Turn complete"));
        assert!(content.contains("HEARTQ_MESSAGE=test body payload"));
        assert!(content.contains("HEARTQ_SESSION_ID=test-session-123"));
    }
}
