...
//! render mode.
//!
//! `heartq-pager` cannot depend on `heartq-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`heartq-pager-bin`) wires it up once at
//! startup by calling `heartq_pager_minimal::install()`. When nothing has
