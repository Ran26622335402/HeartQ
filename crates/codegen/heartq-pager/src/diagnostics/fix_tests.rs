...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "heartq doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("heartq doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn bash_zsh_and_fish_plans_use_exact_paths_and_aliases() {
    ...
    for (shell, relative, alias) in [
        ("/bin/bash", ".bashrc", "alias ssh='heartq wrap ssh'"),
        ("/bin/zsh", ".zshrc", "alias ssh='heartq wrap ssh'"),
        (
            "/usr/local/bin/fish",
...
fn conflict_scan_uses_the_exact_validated_source_snapshot() {
    ...
    assert!(matches!(
        ...
        Err(FixError::Managed(
            heartq_config::managed_text::ManagedConfigError::StalePlan(_)
        ))
    ));
    assert_eq!(
...
fn non_utf8_source_fails_closed_before_conflict_policy() {
    ...
    assert!(matches!(
        ...
        Err(FixError::Managed(
            heartq_config::managed_text::ManagedConfigError::UnsafePath { .. }
        ))
    ));
}
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    let heartq = temp.path().join("heartq");
    std::fs::write(
        &heartq,
        format!(
            "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
    ...
    .unwrap();
    use std::os::unix::fs::PermissionsExt as _;
    std::fs::set_permissions(&heartq, std::fs::Permissions::from_mode(0o755)).unwrap();

    if let Some(bash) = find_on_path("bash") {
        let rc = temp.path().join("bashrc");
        std::fs::write(&rc, "alias ssh='heartq wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='heartq wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
    ...
    if let Some(fish) = find_on_path("fish") {
        ...
        let fish_heartq = temp.path().join("fish-heartq");
        std::fs::write(
            &fish_heartq,
            format!(
                "#!/bin/sh\nprintf '%s\\n' \"$@\" > '{}'\n",
        ...
        .unwrap();
        std::fs::set_permissions(&fish_heartq, std::fs::Permissions::from_mode(0o755)).unwrap();
        let rc = temp.path().join("config.fish");
        std::fs::write(&rc, "alias ssh 'fish-heartq wrap ssh'\n").unwrap();
        let command = format!(
