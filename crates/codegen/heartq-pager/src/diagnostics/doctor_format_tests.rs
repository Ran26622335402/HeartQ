...
fn limited_color_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: Persist in ~/.zshrc / ~/.bashrc and restart HeartQ.\n",
        )
    );
}
...
fn unwrapped_ssh_recommendation_with_no_issues_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "Recommendation\n",
            "\n",
            "  Running over SSH without `heartq wrap` -- clipboard copies depend on the terminal's escape-sequence support, and a dropped connection can leave your local terminal in a bad state\n",
