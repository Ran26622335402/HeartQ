...
use heartq_config::managed_text::{
...
pub const SSH_WRAP_FIX_COMMAND: &str = "heartq doctor fix terminal.ssh-wrap";
pub const SSH_WRAP_ONE_OFF: &str = "heartq wrap ssh <host>";

const SSH_WRAP_FIX_HANDLE: &str = "ssh-wrap";

const MANAGED_NAMESPACE: &str = "heartq doctor";
const SSH_WRAP_ALIAS_POSIX: &str = "alias ssh='heartq wrap ssh'";
const SSH_WRAP_ALIAS_FISH: &str = "alias ssh 'heartq wrap ssh'";
...
pub enum FixError {
    ...
    Managed(heartq_config::managed_text::ManagedConfigError),
    PostconditionFailed,
}
...
impl From<heartq_config::managed_text::ManagedConfigError> for FixError {
    fn from(error: heartq_config::managed_text::ManagedConfigError) -> Self {
        Self::Managed(error)
    }
}
...
pub(crate) fn human_fix_command(id: DiagnosticId) -> Option<String> {
    fix_handle(id).map(|handle| format!("heartq doctor fix {handle}"))
}
...
pub fn plan_fix(
    ...
    Ok(FixPlan {
        ...
        caveats: vec![
            ...
            "`heartq wrap` spawns the real SSH process directly, so the alias does not recurse.",
