...
pub fn apply_voice_probe(report: &mut DiagnosticReport, emit_missing_issue: bool) {
    if !heartq_voice::AUDIO_SUPPORTED {
        return;
    }
    match heartq_voice::input_device_info() {
        Ok(device) => {
        ...
        Err(err) => {
            let error = match err {
                heartq_voice::VoiceError::Config(message) => message,
                ...
                report.findings.push(DiagnosticFinding {
                    ...
                    note: Some(
                        "Connect or select an input device in your system sound settings, then \
                         re-run /terminal-setup or heartq doctor."
                            .to_owned(),
                    ),
                });
            }
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `heartq wrap` on the local end, so
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(heartq_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        message: format!(
            "Your project sandbox profile conflicts with user config.\nProfile: {profiles}\nProject config: .heartq/sandbox.toml\nUser config: ~/.heartq/sandbox.toml"
        ),
...
/// Pure SSH `heartq wrap` recommendation — suggests launching the session
/// through `heartq wrap ssh <host>` on the user's local machine, which gives a
...
/// - `!osc52_sink_active` — no wrap is already capturing our output. `heartq
...
pub fn format_clipboard_diagnostics(input: ClipboardDiagnosticsInput<'_>) -> ClipboardDiagnostics {
    ...
    let fix = match delivery {
        ...
        ClipboardDelivery::Unverified if input.is_ssh => {
            Some("heartq wrap <ssh command> or /minimal")
        }
        ClipboardDelivery::Unverified if input.container_no_display => {
            Some("heartq wrap <command> or /minimal")
        }
        ClipboardDelivery::Unverified => Some("heartq wrap or /minimal"),
        ClipboardDelivery::Failed if input.is_ssh => Some("heartq wrap <ssh command> or /minimal"),
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset NO_COLOR and restart HeartQ.".to_string());
        return Some(warning);
    }
    ...
    warning.note = Some("Persist in ~/.zshrc / ~/.bashrc and restart HeartQ.".to_string());
    Some(warning)
}
...
mod tests {
    ...
    fn clipboard_diagnostics_unknown_ssh_is_unverified() {
        ...
        for expected in [
            ...
            "wrap         off",
            "status       unverified",
            "fix          heartq wrap <ssh command> or /minimal",
        ] {
            assert!(
    ...
    fn clipboard_diagnostics_known_terminal_status() {
        ...
        assert!(
            unsupported
                .text
                .contains("fix          heartq wrap <ssh command> or /minimal")
        );
    ...
    fn clipboard_diagnostics_tmux_wrap_and_container() {
        ...
        assert!(
            container
                .text
                .contains("fix          heartq wrap <command> or /minimal")
        );
    ...
    // -- ssh_wrap_hint: `heartq wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("heartq wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        ...
        // `heartq wrap` — adoption silences the hint by itself.
    ...
    fn notification_none_protocol_no_warnings() {
        let ctx = TerminalContext {
            brand: TerminalName::HeartQDesktop,
            ..Default::default()
        };
        let query = FakeTmuxQuery::healthy_modern();
        let w = collect_notification_warnings(
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::HeartQDesktop));
