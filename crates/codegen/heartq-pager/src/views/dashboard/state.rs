...
/// `~/.heartq/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<heartq_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    heartq_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.heartq/config.toml`.
///
...
/// `~/.heartq/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no heartq home"))?;
...
fn config_path() -> Option<PathBuf> {
    let home = heartq_shell::util::heartq_home::heartq_home();
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/heartq");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/heartq");
    }
    ...
    /// per-key serial lock. The `HEARTQ_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(HEARTQ_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `HEARTQ_AGENT_DASHBOARD` key reads it concurrently.
