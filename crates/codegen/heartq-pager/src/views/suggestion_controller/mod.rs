...
// (e.g. heartq-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
pub struct SuggestionController {
    ...
    /// `HEARTQ_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `HEARTQ_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `x.ai/suggest` request.
    /// Resolved at construction from `HEARTQ_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: heartq_config::env_bool("HEARTQ_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: heartq_config::env_bool("HEARTQ_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("HEARTQ_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
