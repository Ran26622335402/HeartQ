//! Headless single-turn mode (`heartq -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_heartq_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use heartq_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use heartq_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_heartq_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<heartq_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use heartq_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<heartq_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = heartq_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut heartq_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    heartq_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `heartq login` to authenticate \
         (or `heartq login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         heartq login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `heartq login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !heartq_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !heartq_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &heartq_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'heartq models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    heartq_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = heartq_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `heartq workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = heartq_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        heartq_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_heartq_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("HEARTQ_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = heartq_shell::active_sessions::register(
            heartq_shell::active_sessions::ActiveSession {
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.heartq can't hang exit.
        let _ = heartq_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = heartq_shell::sampling::error::prompt_usage_from_error(&err)
...
mod tests {
    ...
    use super::*;
    use heartq_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            heartq_workspace::permission::types::PatternMode::Domain
        );
