...
use heartq_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("HeartQ 4.5"); if we split on the last token
        // first, a shorter catalog entry ("HeartQ") would steal the prefix and
        ...
        // level (e.g. `none` on heartq-4.5) surfaces the effort error with the
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("heartq-4.5", "HeartQ 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "HeartQ 4.5").unwrap();
        assert_eq!(plain.insert_text, "HeartQ 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "HeartQ" (reasoning) and "HeartQ 4.5". `/model HeartQ 4.5`
        // must select the full name, not treat "4.5" as an effort on "HeartQ".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("heartq", "HeartQ");
        let (long_id, long_info) = model_with_reasoning("heartq-4.5", "HeartQ 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "HeartQ 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(HeartQ 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("heartq-4.5", "HeartQ 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "HeartQ 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model heartq 4.5`
    /// resolves to the same `ModelId` as `/model HeartQ 4.5`.
    #[test]
