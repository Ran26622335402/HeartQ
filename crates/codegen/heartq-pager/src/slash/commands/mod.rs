...
pub fn builtin_commands() -> Vec<Arc<dyn SlashCommand>> {
    vec![
        ...
        Arc::new(meta::MetaCommand),
        Arc::new(proposals::ProposalsCommand),
...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        let id_fast = acp::ModelId::new(Arc::from("heartq-4.5"));
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "HeartQ 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("heartq-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "HeartQ 4.3".to_string()),
        );
    ...
    fn model_resolves_by_display_name() {
        ...
        let result = cmd.run(&mut ctx, "HeartQ 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "heartq-4.5");
            }
    ...
    fn model_resolves_by_model_id() {
        ...
        let mut ctx = make_ctx(&models);
        let cmd = model::ModelCommand;
        let result = cmd.run(&mut ctx, "heartq-4.3");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "heartq-4.3");
            }
            other => panic!("expected Action(SetDefaultModel), got {other:?}"),
        }
    }
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("HeartQ 4.5") && i.insert_text == "HeartQ 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "HeartQ 4.3" && i.insert_text == "HeartQ 4.3")
        );
    }
    #[test]
    ...
    #[test]
    fn proposals_registered_in_builtin_commands() {
        let reg = CommandRegistry::new(builtin_commands());
        assert!(
            reg.get("proposals").is_some(),
            "/proposals should be registered in builtins"
        );
        assert!(
            reg.get("meta").is_some(),
            "/meta should remain registered alongside /proposals"
        );
    }
