#!/bin/bash
#
# HeartQ CLI installer (enterprise channel) — https://x.ai/cli/enterprise-install.sh
#
...
# Env: HEARTQ_BIN_DIR, HEARTQ_PROXY_URL
#
# Usage:
...
#   HEARTQ_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/enterprise-install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/heartq"
