#!/bin/bash
#
# HeartQ CLI installer — https://x.ai/cli/install.sh
#
# Auth: HEARTQ_DEPLOYMENT_KEY (takes precedence) or ~/.heartq/auth.json from `heartq login`.
# Env: HEARTQ_CHANNEL (stable|alpha|enterprise, default: stable), HEARTQ_BIN_DIR, HEARTQ_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   HEARTQ_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.heartq/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_heartq_token() {
    local auth_file="$HOME/.heartq/auth.json"
    local scope="$1"
...
# Resolve auth: HEARTQ_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$HEARTQ_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_heartq_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_heartq_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.heartq/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.heartq/auth.json." >&2
    fi
fi
...
BASE_URL_FALLBACK="https://storage.googleapis.com/heartq-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.heartq/downloads"
BIN_DIR="${HEARTQ_BIN_DIR:-$HOME/.heartq/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${HEARTQ_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing HeartQ $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing HeartQ $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/heartq-$platform"
artifact_base="${BASE_URL}/heartq-${version}-${platform}"
...
echo "  Downloading heartq ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: HeartQ is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in heartq.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/heartq.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded heartq failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.heartq/bin and ~/.heartq/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/heartq"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/heartq and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.heartq/completions/bash" "$HOME/.heartq/completions/zsh"
"$BIN_DIR/heartq" completions bash > "$HOME/.heartq/completions/bash/heartq.bash" 2>/dev/null || true
"$BIN_DIR/heartq" completions zsh  > "$HOME/.heartq/completions/zsh/_heartq"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/heartq" completions fish > "$HOME/.config/fish/completions/heartq.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.heartq/config.toml"
...
if [ -n "$HEARTQ_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${HEARTQ_PROXY_URL:-https://cli-chat-proxy.heartq.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$HEARTQ_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.heartq/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.heartq/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.heartq/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.heartq/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "HeartQ $version installed to $BIN_DIR/heartq.exe" >&2
else
    echo "HeartQ $version installed to $BIN_DIR/heartq" >&2
fi

# --- Ensure heartq is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so heartq works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/heartq" "$candidate/heartq"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/heartq -> $BIN_DIR/heartq" >&2
...
# Also update shell config so ~/.heartq/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> heartq installer >>>
fish_add_path $HOME/.heartq/bin
# <<< heartq installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> heartq installer >>>
export PATH="$HOME/.heartq/bin:$PATH"
fpath=(~/.heartq/completions/zsh $fpath)
...
# <<< heartq installer <<<'
    ...
    if grep -qs "heartq installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> heartq installer >>>/ { skip=1; next }
            /# <<< heartq installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'heartq' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'heartq' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'heartq' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.heartq/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use heartq from cmd.exe or PowerShell, add %USERPROFILE%\\.heartq\\bin to your PATH." >&2
fi
