...
impl ChatStateHandle {
    ...
    /// Late-bind a `MemoryManager` to the chat-state actor.
    ///
    /// Phase A.5 wiring: the memory backend is constructed after the actor
    /// is spawned (the embedding pipeline + auth credentials are not ready
    /// at actor-spawn time), so the shell calls this method right after the
    /// backend is initialized. Safe to call multiple times — the last
    /// `MemoryManager` wins. Returns `true` if the command was queued.
    pub fn attach_memory_manager(&self, memory_manager: std::sync::Arc<crate::MemoryManager>) -> bool {
        self.cmd_tx
            .send(ChatStateCommand::SetMemoryManager { memory_manager })
            .is_ok()
    }
    ...
    /// Sync a turn to memory storage.
    pub async fn sync_turn_to_memory(
        &self,
        user_content: String,
        assistant_content: String,
    ) -> Result<(), String> {
        self.query("SyncTurnToMemory", |reply| {
            ChatStateCommand::SyncTurnToMemory {
                user_content,
                assistant_content,
                reply,
            }
        ...
        .await
        .unwrap_or_else(|| Err("ChatStateActor dead".to_string()))
    }

    /// Get prefetch memory content for a query.
    pub async fn get_prefetch_memory(&self, query: String) -> Result<String, String> {
        self.query("GetPrefetchMemory", |reply| {
