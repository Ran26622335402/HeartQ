...
/// emitter in `heartq-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (heartq login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `heartq_sampling_types::SamplingConfig` (which is secret-free).
