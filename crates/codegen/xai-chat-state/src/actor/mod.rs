...
use state::ChatState;
use heartq_memory::MemoryManager;
...
pub struct ChatStateActor {
    ...
    /// Optional memory manager — when `Some`, the actor dispatches
    /// `SyncTurnToMemory` / `GetPrefetchMemory` / `ExtractMemoryInsights`
    /// commands to it; when `None`, those commands return placeholders
...
impl ChatStateActor {
    ...
    /// Spawn the actor with an attached `MemoryManager`.
    ///
    /// When the manager is `Some`, the actor forwards
    /// `SyncTurnToMemory` / `GetPrefetchMemory` / `ExtractMemoryInsights`
    /// commands to it; when `None` (default `spawn_*`), those commands
    /// return placeholder results — preserving backward compatibility.
    pub fn spawn_with_memory_manager(
        initial_conversation: Vec<ConversationItem>,
        sampling_config: SamplingConfig,
        pruning_config: PruningConfig,
        persistence: Box<dyn ChatPersistence>,
    ...
    async fn handle_command(&mut self, cmd: ChatStateCommand) {
        match cmd {
            ...
            // ═══ Memory integration ═══
            //
            // When a `MemoryManager` is attached, dispatch the command to it.
            // Otherwise return a harmless placeholder (backward compatibility
            // for callers that never wired memory into the session).
            ChatStateCommand::SetMemoryManager { memory_manager } => {
                self.memory_manager = Some(memory_manager);
                tracing::debug!("ChatStateActor: MemoryManager attached");
            }
            ChatStateCommand::SyncTurnToMemory { user_content, assistant_content, reply } => {
                let result = match &self.memory_manager {
                    Some(mm) => mm
            ...
            ChatStateCommand::GetPrefetchMemory { query, reply } => {
                let result = match &self.memory_manager {
                    Some(mm) => mm
                        .build_prefetch(&query, None)
                        .await
                        .map_err(|e| e.to_string()),
                    None => Ok(String::new()),
                };
            ...
            ChatStateCommand::ExtractMemoryInsights { user_content, assistant_content, reply } => {
                let result = match &self.memory_manager {
                    Some(mm) => mm
                        .extract_and_sync_insights(&user_content, &assistant_content)
                        .await
                        .map(|insights| {
                            insights
                                .into_iter()
