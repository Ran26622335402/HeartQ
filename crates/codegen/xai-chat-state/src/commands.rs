...
pub enum ChatStateCommand {
    ...
    /// Late-bind a `MemoryManager` to the actor after construction.
    ///
    /// This is the Phase A.5 wiring: the memory backend is only available
    /// after the agent loop has authenticated + spun up the embedding
    /// pipeline, so we construct the `MemoryManager` in `spawn.rs` *after*
    /// the backend is ready and then attach it here. Without this command
    /// the actor would never see a `MemoryManager` and `GetPrefetchMemory`
    /// / `SyncTurnToMemory` commands would return placeholders.
    SetMemoryManager {
        memory_manager: std::sync::Arc<crate::MemoryManager>,
    },
    ...
    /// Extract insights from user/assistant content for memory.
