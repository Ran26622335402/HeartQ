...
//!
//! This crate extracts conversation state management from `heartq-shell`'s
...
// Re-export the MemoryManager type so command enums can name it via
// `crate::MemoryManager` without depending on `heartq-memory` directly.
pub use heartq_memory::MemoryManager;
pub use actor::state::{
