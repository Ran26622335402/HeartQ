...
//! Backend environment presets for the HeartQ CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `HEARTQ_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct HeartQBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: HeartQBuildEndpoints = HeartQBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.heartq.com/v1",
    asset_server_url: "https://assets.heartq.com",
    relay_ws_url: "wss://code.heartq.com/ws/code-agent",
    gateway_ws_url: "wss://heartq.com/ws/gw/",
    ws_origin: "https://heartq.com",
};
...
pub enum HeartQBuildEnvironment {
    #[default]
    Production,
}
impl HeartQBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        HeartQBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            HeartQBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, HeartQBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            HeartQBuildEnvironment::Production => "HEARTQ_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> HeartQBuildEndpoints {
        match self {
            HeartQBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `heartq.com/code` driving a
    ...
    /// `HEARTQ_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for HeartQBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            HeartQBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            HeartQBuildEnvironment::Production.env_prefix(),
            "HEARTQ_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_HEARTQ_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    /// loop mistakenly connecting to `wss://heartq.com/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            HeartQBuildEnvironment::Production.relay_ws_url(),
            HeartQBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            HeartQBuildEnvironment::from_flags(false, false),
            HeartQBuildEnvironment::Production
        );
    }
}
