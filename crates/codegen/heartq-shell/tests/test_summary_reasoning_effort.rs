...
//!
//! Each test spawns a real `heartq agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p heartq-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.heartq/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".heartq").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let heartq_dir = home.path().join(".heartq");
        std::fs::create_dir_all(&heartq_dir).expect("create .heartq dir");
        std::fs::write(
...
"#,
        ...
        let client = HeartQStdioClient::spawn_with_home(&server, workdir.path(), home).await;
