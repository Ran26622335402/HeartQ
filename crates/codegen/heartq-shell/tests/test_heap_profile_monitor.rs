...
use heartq_shell::auth::{AuthManager, AuthMode, HeartQAuth, HeartQComConfig};
use heartq_shell::heap_profile::{
...
use heartq_test_support::{EnvGuard, MockInferenceServer};
use heartq_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = HeartQComConfig::default().auth_scope();
    let auth = HeartQAuth {
        key: token.to_owned(),
...
fn assert_storage_auth(uploads: &[heartq_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), HeartQComConfig::default()));
