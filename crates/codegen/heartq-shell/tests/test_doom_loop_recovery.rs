...
use heartq_sampler::RetryPolicy;
use heartq_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use heartq_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// `x-heartq-turn-idx`) carries the opt-in header. Aux side-queries the binary
...
/// ```bash
/// cargo test -p heartq-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = heartq_test_support::git_workdir();
    let home = tempfile::TempDir::new().unwrap();

    let heartq_home = home.path().join(".heartq");
    std::fs::create_dir_all(&heartq_home).expect("create .heartq home");
    std::fs::write(
    ...
    let mut cmd = tokio::process::Command::new(heartq_test_support::heartq_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    heartq_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("HEARTQ_HOME", heartq_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("HEARTQ_LEADER_SOCKET");

    let result = heartq_test_support::run_headless_with_cmd(cmd).await;
    heartq_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
    ...
    let responses_posts: Vec<_> = requests
        ...
        .filter(|e| e.method == "POST" && e.path.contains("/responses"))
        .collect();
    // The session turn carries `x-heartq-turn-idx`; aux side-queries (session
    // title, etc.) do not.
    let (turns, aux): (Vec<_>, Vec<_>) = responses_posts
        .into_iter()
        .partition(|e| e.header("x-heartq-turn-idx").is_some());
    assert!(
        !turns.is_empty(),
    ...
    for turn in turns {
        assert_eq!(
            turn.header("x-heartq-doom-loop-check"),
            Some("true"),
    ...
    for side_query in aux {
        assert_eq!(
            side_query.header("x-heartq-doom-loop-check"),
            None::<&str>,
