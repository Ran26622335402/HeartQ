//! Built-binary end-to-end tests for the heartq (heartq-pager) binary.
//!
//! These tests verify that the built heartq binary works end-to-end against a mock
...
//!
//! The tests exercise:
//! - **Smoke** (`heartq --version`): binary loads without crashing
//! - **ACP stdio** (`heartq agent stdio`): full protocol lifecycle via ClientSideConnection
//!
//! Tests are `#[ignore]`d by default — they require a pre-built binary.
//!
//! Run locally (auto-builds the binary if not already present):
//! ```bash
//! cargo test -p heartq-shell --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `HEARTQ_BINARY` to point at the release artifact:
//! ```bash
//! HEARTQ_BINARY=./artifacts/heartq-0.1.159-linux-x86_64 \
//!   cargo test -p heartq-shell --test test_built_binary_e2e -- --ignored
//! ```
...
async fn heartq_build_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("heartq-4.5", "heartq-build")
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("HEARTQ_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "heartq --version with HEARTQ_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "heartq -p in git repo", Some(&server));
...
/// Verify heartq works in a non-git directory (exercises the fallback codepath
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "heartq -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    let server = heartq_build_server().await;
    ...
    assert_headless_success(&result, "heartq -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "heartq -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "heartq -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free HeartQ Build usage limit"),
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "heartq -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use heartq_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        heartq_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "heartq -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        heartq_test_support::scripted::ScriptedResponse::sse(
            heartq_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "heartq -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    let server = single_model_server("heartq-4.5", "responses").await;
    server.set_response(r#"{"name":"Alice","age":30}"#);
    ...
    assert_headless_success(&result, "heartq -p --json-schema (responses)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "heartq -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "heartq -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "heartq -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "heartq -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (heartq agent stdio)
//
// These test the agent as a server: spawn `heartq agent stdio`, speak the full
...
async fn test_stdio_full_session_lifecycle() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let client = HeartQStdioClient::spawn(&server, workdir.path()).await;
...
async fn test_stdio_prompt_then_immediate_load_session() {
    with_local_set(|| async {
        let server = MockInferenceServer::start().await.expect("start mock server");
        let workdir = git_workdir();
        let mut writer = HeartQStdioClient::spawn(&server, workdir.path()).await;
        ...
        let reader = HeartQStdioClient::spawn_with_home(&server, workdir.path(), shared_home).await;
...
/// Isolated headless run with a custom `~/.heartq/`. Clean env (no leaked
/// host credentials). Write config files into `heartq_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".heartq")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("HEARTQ_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("HEARTQ_TELEMETRY_ENABLED".into(), "false".into()),
                ("HEARTQ_FEEDBACK_ENABLED".into(), "false".into()),
                ("HEARTQ_TRACE_UPLOAD".into(), "false".into()),
                ("HEARTQ_INSTRUMENTATION".into(), "disabled".into()),
                ("HEARTQ_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn heartq_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".heartq")
    }
    ...
    async fn run(&self) -> HeadlessResult {
        let mut cmd = tokio::process::Command::new(heartq_binary());
        cmd.args(["-p", "say hello", "--yolo"])
            ...
            // Windows resolves `~` via USERPROFILE, not HOME — pin the heartq
            // home explicitly so the sandbox holds on all platforms (see
            // `test_env_cmd_tokio`).
            .env("HEARTQ_HOME", self.heartq_dir())
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    let server = MockInferenceServer::start_with_required_auth(
        vec![MockModelEntry::new("heartq-4.5")],
    ...
    std::fs::write(
        h.heartq_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
...
env_key = "HEARTQ_TEST_BYOK_TOKEN"
model = "heartq-4.5"
...
"#,
    ...
    .unwrap();
    h.env("HEARTQ_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "heartq -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "heartq -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "heartq -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_waits_for_short_background_task_and_exits_clean() {
    ...
    assert_headless_success(&result, "heartq -p with short background task", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        marker.exists(),
