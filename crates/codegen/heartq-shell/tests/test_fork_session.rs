...
use heartq_shell::sampling::ConversationItem;
...
async fn create_test_session(storage: &JsonlStorageAdapter, session_id: &str, cwd: &str) -> Info {
    ...
    let model_id = acp::ModelId::new("heartq-code-fast-1");
    ...
    storage
        .append_update(
            &info,
            &heartq_shell::session::storage::SessionUpdate::Acp(Box::new(notification)),
        )
        .await
        .unwrap();
...
async fn test_fork_session_creates_new_session_with_parent_tracking() {
    ...
    let options = heartq_shell::session::storage::CopySessionOptions {
    ...
    assert_eq!(loaded.summary.current_model_id, acp::ModelId::new("heartq-3"));
    assert_eq!(
    ...
    match &loaded.updates[0] {
        heartq_shell::session::storage::SessionUpdate::Acp(notification) => {
