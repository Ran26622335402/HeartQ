...
//! - Session resume
//!
//! Each test spawns a real `heartq agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p heartq-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".heartq").join("models_cache.json");
    if cache.exists() {
        std::fs::remove_file(&cache).expect("failed to delete models_cache.json");
    }
}
/// Start a mock server with two models:
/// - `default-model`: no agent_type (→ defaults to "heartq-build")
...
/// - `model-a`: no agent_type (→ "heartq-build")
/// - `model-b`: no agent_type (→ "heartq-build")
...
/// `heartq-build` harness. The system prompt sent to the LLM should contain
/// the heartq-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_heartq_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("HeartQ") || sys_prompt.contains("heartq"),
            "default model should use heartq-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_heartq =
            original_sys_prompt.contains("HeartQ") || original_sys_prompt.contains("heartq");
        let resumed_has_heartq =
            resumed_sys_prompt.contains("HeartQ") || resumed_sys_prompt.contains("heartq");
        assert_eq!(
            original_has_heartq,
            resumed_has_heartq,
            "resumed session should use the same harness as the original\n\
             original identity markers: heartq={original_has_heartq}\n\
             resumed identity markers: heartq={resumed_has_heartq}\n\
...
/// default to `"heartq-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_heartq_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("HeartQ") || sys_prompt.contains("heartq"),
                "model without agent_type should default to heartq-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `HEARTQ_AGENT` escape hatch should override the model's agent_type.
/// Setting `HEARTQ_AGENT=heartq-build` with an alternate-agent model should use
/// heartq-build harness.
#[tokio::test]
#[ignore]
async fn test_heartq_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            heartq_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("HEARTQ_AGENT", "heartq-build");
            let mut child = cmd.spawn().expect("spawn heartq");
            ...
            assert!(
                sys_prompt.contains("HeartQ") || sys_prompt.contains("heartq"),
                "HEARTQ_AGENT=heartq-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
