...
//! 4. Smoke-validate session JSON under `$HEARTQ_HOME/sessions/` after exit.
//!
//! Requires a built `heartq` binary (`HEARTQ_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p heartq-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p heartq-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use heartq_agent::plugins::SharedPluginRegistryHandle;
use heartq_agent::plugins::discovery::DiscoveryConfig;
use heartq_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _heartq_guard = EnvVarGuard::set("HEARTQ_HOME", &heartq_home);
    ...
    let mut registry = InstallRegistry::empty(heartq_home.join("installed-plugins"));
    ...
    // heartq_home() caches the first HEARTQ_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        heartq_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built heartq binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/HEARTQ_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via heartq_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _heartq_guard = EnvVarGuard::set("HEARTQ_HOME", &heartq_home);
    ...
    cmd.env("HOME", &home);
    cmd.env("HEARTQ_HOME", &heartq_home);
    ...
    // Smoke: session storage under HEARTQ_HOME/sessions has JSON artifacts.
