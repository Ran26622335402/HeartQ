...
//! default since they need the heartq binary (`HEARTQ_BINARY` or a local debug build):
//! ```bash
//! cargo test -p heartq-shell --test test_stop_hook_e2e -- --ignored
//! ```

use heartq_test_support::env::test_env_cmd_tokio;
...
async fn run_with_stop_hook(respond: &str) -> StopHookRun {
    ...
    let hooks_dir = home.path().join(".heartq").join("hooks");
...
async fn stop_block_loop_ends_at_continuation_cap() {
    ...
    assert_eq!(
        run.invocations(),
        heartq_shell::session::MAX_STOP_HOOK_CONTINUATIONS_PER_TURN,
