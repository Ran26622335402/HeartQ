...
//!
//! This test spawns a real `heartq agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p heartq-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // HeartQStdioClient sets HOME=<temp>; the binary uses <HOME>/.heartq as HEARTQ_HOME.
        let heartq_home = shared_home.path().join(".heartq");
        let session_dir = locate_session_dir(&heartq_home, session_id.0.as_ref());
