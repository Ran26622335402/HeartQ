...
- **Using HeartQ**
  ...
  - [SSH Passthrough](#ssh-passthrough-heartq-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`heartq inspect`)](#introspection)
...
- [Building with HeartQ](#building-with-heartq) — headless API, ACP SDK integration
...
On first launch, HeartQ opens your browser to authenticate with heartq.com:

```bash
heartq
```

Credentials are stored in `~/.heartq/auth.json` and persist across sessions. Tokens expire after 7 days; HeartQ will prompt you to re-authenticate when needed.
...
export HEARTQ_OIDC_ISSUER="https://acme.okta.com"
export HEARTQ_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export HEARTQ_CLI_CHAT_PROXY_BASE_URL="https://heartq-proxy.acme.com/v1"
```

**3. Run `heartq`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.heartq/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
HeartQ is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     HeartQ     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. HeartQ runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by HeartQ and saved to `~/.heartq/auth.json` as the access token
5. exit 0 → success; exit non-zero → HeartQ falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | HeartQ (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. HeartQ reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want HeartQ to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells HeartQ when to proactively refresh. Without it, HeartQ assumes tokens last 30 days.
...
# Print ONLY the token to stdout (HeartQ captures this)
...
```bash
# Or via environment variables
export HEARTQ_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export HEARTQ_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export HEARTQ_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, HeartQ cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with heartq.com". In headless mode (`heartq -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=heartq-cli")
...
When HeartQ needs to refresh an expired token, it re-runs your binary with `HEARTQ_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$HEARTQ_AUTH_EXPIRED" = "1" ]; then
...
`HEARTQ_AUTH_EXPIRED` is optional — if your binary ignores it, HeartQ still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

HeartQ supports automatic credential refresh for external auth providers and OIDC. When HeartQ detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. HeartQ handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, HeartQ re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), HeartQ re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, HeartQ silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# HeartQ refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export HEARTQ_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `heartq login` before starting — HeartQ runs your binary automatically on first launch. You _can_ run `heartq login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, HeartQ tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
heartq --debug-file /tmp/heartq-auth.log -p "hello"
tail -f /tmp/heartq-auth.log
```
...
`auth_provider_command` above replaces HeartQ's *session* auth: it mints the token sent to xAI's backend. If you instead want xAI models on normal xAI login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `HEARTQ_AUTH_EXPIRED=1` is set whenever HeartQ re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `HEARTQ_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `HEARTQ_AUTH_PROVIDER_REFRESH_TOKEN` / `HEARTQ_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and HeartQ re-runs the command on restart to re-mint.
- HeartQ runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.heartq/config.toml`, managed config, requirements). A project's `.heartq/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits HeartQ's environment (so it sees `PATH`, `HOME`, and any other secrets there), but HeartQ's own first-party credentials (`XAI_API_KEY`, `HEARTQ_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `HEARTQ_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `heartq login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the heartq CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.heartq.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.heartq/auth.json)" \
  -H "X-XAI-Token-Auth: heartq-cli" \
  -H "x-heartq-model-override: heartq-build" \
  -d '{
    "model": "heartq-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.heartq/auth.json` (set by `heartq login`)                                                                                                                              |
| `X-XAI-Token-Auth: heartq-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-heartq-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `heartq-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `heartq-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `heartq login` to refresh.
...
heartq --cwd ~/projects/my-app

# Start with an initial task
heartq --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
heartq --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
heartq --always-approve --prompt "Format all files"
```
...
Run HeartQ non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `heartq-build`)               |
...
heartq -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
heartq -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
heartq -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
heartq -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
heartq -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
heartq -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
heartq -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
heartq -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
heartq -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
heartq --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
heartq -p "What does this project do?"

# Use a specific model
heartq -p "Optimize this function" -m heartq-build

# Get JSON output for parsing
heartq -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
heartq -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
heartq -p "Remember: the secret number is 42" --output-format json
heartq -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
heartq -p "Continue where we left off" -c

# Run in a different directory
heartq -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
heartq -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
heartq -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
heartq -p "Generate a README" > README.md

# Parse JSON output with jq
heartq -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
heartq -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | heartq -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  heartq -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
heartq -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run HeartQ as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `heartq-build`)                           |
...
```bash
heartq agent headless --heartq-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`heartq ssh`)

Use `heartq ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
heartq ssh user@host

# With SSH flags
heartq ssh -t user@host
heartq ssh -L 8080:localhost:8080 user@host

# With remote command
heartq ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `heartq ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with HeartQ

HeartQ can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `heartq -p` and parses JSON output.
...
class HeartQChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["heartq", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="heartq-build", stream=False):
...
class HeartQChat {
  ...
  async create(
    ...
    { model = "heartq-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("heartq", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await HeartQACPChat(cwd=".").init()
...
class HeartQACPChat {
  ...
  async init() {
    this.proc = spawn("heartq", ["agent", "stdio"]);
...
// Usage
const client = await new HeartQACPChat(".").init();
...
HeartQ implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
HeartQ reads configuration from `~/.heartq/config.toml`. If the file doesn't exist, HeartQ uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "heartq-build"           # model used for new sessions
web_search = "heartq-4.20-multi-agent"   # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: HEARTQ_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: HEARTQ_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: HEARTQ_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: HEARTQ_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: HEARTQ_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: HEARTQ_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: HEARTQ_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: HEARTQ_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `HEARTQ_TELEMETRY_BUILD_EVENTS_URL`, `HEARTQ_TELEMETRY_BUILD_EVENTS_API_KEY`, and `HEARTQ_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

HeartQ can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives HeartQ language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, HeartQ can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — HeartQ can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

HeartQ looks for server definitions in:

- project config: `<repo>/.heartq/lsp.json`
- user config: `~/.heartq/lsp.json`
...
- LSP tools are enabled (`HEARTQ_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
HEARTQ_LSP_TOOLS=1 heartq
```
...
If LSP tools are enabled but no usable server config is found, HeartQ emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
HeartQ does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `HEARTQ_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-heartq"

[model.company-heartq]
model = "heartq-build"
base_url = "https://heartq-proxy.acme.com/"
name = "HeartQ Build Latest (Proxy)"
...
With this config, `heartq` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). HeartQ reads these files and appends their contents to the system prompt.

HeartQ scans for agent rules in this order:

1. `~/.heartq/` (global rules)
...
Within each directory, HeartQ checks for these filenames:
...
Skills are reusable prompt packages that extend HeartQ with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

HeartQ discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.heartq/skills/`           | Local | Highest  |
| `<repo_root>/.heartq/skills/` | Repo  | Medium   |
| `~/.heartq/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.heartq/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how HeartQ decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, HeartQ invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when HeartQ automatically invokes the skill. Be specific about trigger phrases and use cases.
...
HeartQ discovers agent definitions from `.heartq/agents/` (project), `~/.heartq/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `HEARTQ_AGENT` env var
4. Default `heartq-build` agent
...
```bash
heartq --agent-profile ./my-agent.md
# or
export HEARTQ_AGENT="my-custom-agent"
```
...
```bash
export HEARTQ_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "heartq-build"              # route explore to a lighter model
```
...
model = "heartq-build"
prompt_file = ".heartq/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".heartq/personas/concise.md"  # or load from file
```

Both are also discovered from `.heartq/roles/*.toml` and `.heartq/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend HeartQ with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.heartq/plugins/`            | Project |
| `~/.heartq/plugins/`          | User    |
...
HeartQ discovers hooks from `.heartq/hooks/` in the project directory. Manage them with:
...
The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, HeartQ falls back to built-in defaults for known models.
...
[model.heartq-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, HeartQ starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `HEARTQ_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `heartq models` output). You also need a `[model.*]` entry so HeartQ knows
...
>
> [model.my-custom-model]              # 2. tell HeartQ how to reach it
...
Point HeartQ at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `HEARTQ_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `HEARTQ_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export HEARTQ_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
heartq
```

HeartQ fetches the model list from `{HEARTQ_MODELS_BASE_URL}/models` on startup and sends inference requests to `HEARTQ_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `HEARTQ_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.heartq/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, HeartQ uses API key auth (`Authorization: Bearer`) instead of session auth. `heartq login` is not required — only the API key.

---

## MCP Servers

Extend HeartQ's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.heartq/config.toml`:
...
MCP servers can also be configured per-project in `.heartq/config.toml`. HeartQ walks from the current directory up to the git repo root, loading `.heartq/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.heartq/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.heartq/config.toml` | This repository   | ↑        |
| `<cwd>/.heartq/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.heartq/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.heartq/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.heartq/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.heartq/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `HEARTQ_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets HeartQ remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.heartq/memory/`:
- **Global** (`~/.heartq/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.heartq/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.heartq/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
At the end of each session, HeartQ saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.heartq/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
HeartQ searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.heartq/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.heartq/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, HeartQ emits the `heartq-shell-memory_injection`
...
HeartQ can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.heartq/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.heartq/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.heartq/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.heartq/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.heartq/sandbox.toml` (global) or `.heartq/sandbox.toml` (per-project):
...
```bash
heartq --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire heartq process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), HeartQ logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.heartq/sandbox-events.jsonl`
...
Use `heartq inspect` to see everything HeartQ discovers in the current directory:

```bash
heartq inspect          # human-readable output
heartq inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.heartq/skills/`, `~/.heartq/skills/`, plugins, and config paths
...
HeartQ automatically discovers configuration from Claude Code directories alongside native `.heartq/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How HeartQ uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.heartq/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by HeartQ at runtime.

---

## Built-in Tools

HeartQ includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `HEARTQ_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (HeartQ docs, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `x.ai/company`).

---

## Session Persistence

HeartQ automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.heartq/sessions/`, organized by URL-encoded working directory:

```
~/.heartq/sessions/<encoded-cwd>/<session-id>/
...
heartq -p "Remember: X=42" -s my-session
heartq -p "What is X?" -s my-session

# Resume existing session (errors if not found)
heartq -p "Continue" -r my-session

# Continue most recent session in current directory
heartq -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
heartq -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.heartq/config.toml` | Configuration file                                  |
| `~/.heartq/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.heartq/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.heartq/memory/`     | Cross-session memory files and index                |
| `~/.heartq/skills/`     | User-scoped skill definitions                       |
| `~/.heartq/plugins/`    | User-scoped plugins                                 |
| `~/.heartq/agents/`     | User-scoped agent definitions                       |
| `.heartq/config.toml`   | Project-scoped config (MCP servers)                 |
| `.heartq/skills/`       | Project-scoped skill definitions                    |
| `.heartq/plugins/`      | Project-scoped plugins                              |
| `.heartq/agents/`       | Project-scoped agent definitions                    |
| `.heartq/hooks/`        | Project-scoped hooks                                |
| `.heartq/lsp.json`      | LSP server configuration                            |
...
| `HEARTQ_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.heartq.com/v1`)                          |
| `HEARTQ_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `HEARTQ_MODELS_LIST_URL`          | Override the model list URL if it differs from `{HEARTQ_MODELS_BASE_URL}/models`                                              |
| `HEARTQ_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `HEARTQ_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `HEARTQ_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `HEARTQ_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `HEARTQ_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `HEARTQ_HOME`                     | Override config directory (default: `~/.heartq`)                                                           |
| `HEARTQ_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `HEARTQ_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `HEARTQ_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `HEARTQ_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `HEARTQ_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `HEARTQ_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `HEARTQ_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `HEARTQ_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `HEARTQ_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `HEARTQ_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.heartq/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `HEARTQ_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `heartq` commands and flags.
...
heartq completions bash > ~/.local/share/bash-completion/completions/heartq
```

Reload your shell or run `source ~/.bashrc`.

Alternative (HeartQ-managed location):

```bash
mkdir -p ~/.heartq/completions/bash
heartq completions bash > ~/.heartq/completions/bash/heartq.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.heartq/completions/bash/heartq.bash" ]] && source "$HOME/.heartq/completions/bash/heartq.bash"
```
...
heartq completions zsh > ~/.zsh/completions/_heartq
```
...
```bash
mkdir -p ~/.heartq/completions/zsh
heartq completions zsh > ~/.heartq/completions/zsh/_heartq
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.heartq/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `heartq` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `heartq --debug` or `HEARTQ_LOG_FILE` instead:

```bash
# Per-session debug log (~/.heartq/debug/<sessionId>.txt)
heartq --debug

# Log to a custom path
HEARTQ_LOG_FILE=/tmp/heartq-debug.log heartq

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.heartq/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `HEARTQ_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
HEARTQ_LOG_FILE=/tmp/heartq-debug.log RUST_LOG="info,heartq_shell::auth=debug" heartq
```
...
cat ~/.heartq/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.heartq/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.heartq/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
