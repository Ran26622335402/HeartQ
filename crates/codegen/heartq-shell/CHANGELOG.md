...
- **heartq doctor fix terminal.ssh-wrap** can install the recommended SSH wrapper alias.
...
- **heartq doctor** now shows standalone terminal, tmux, clipboard, and keyboard diagnostics without starting the TUI.

## Bug Fixes

- **Image paste** over heartq wrap now works on headless remotes.
...
- **Added HEARTQ_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `heartq --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now HeartQ 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.heartq/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `heartq wrap ssh <host>` for clipboard and terminal restore.
...
- **heartq** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **heartq wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `heartq login`.
...
- **heartq login** now requests HeartQ Projects scopes so workspace listing works after consent.
...
- **`heartq plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using heartq -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **heartq inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct heartq --minimal --resume command.
...
- **HeartQ no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **heartq setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask HeartQ above the footer shortcuts.
...
- **`heartq worktree ls`** now works as a short alias for `heartq worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.heartq/config.toml`.
...
- **Resumed heartq.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.heartq/worktrees/.
...
- **Terminal command cards** on heartq.com now correctly settle after foreground bash tasks.
...
- **heartq inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain heartq launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive heartq agent stdio without silent parse drops on session/* calls.
...
- **heartq --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by heartq.com" link in the Extensions modal is now clickable and underlined.
...
- **heartq --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **heartq update** on Windows no longer fails when a previous .old executable is still running.
...
- **heartq models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `heartq plugin marketplace add`.
...
- **Plan files** now default to .heartq/plan.md to match HeartQ conventions.
...
- **--minimal** flag now shows in `heartq --help`.
...
- **Tool result previews** no longer paint opaque panels in `heartq --minimal`.
- **heartq wrap** now correctly handles quoted strings and shell aliases.
...
- **HeartQ.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the heartq.com model catalog** in the picker.
...
- **`heartq update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `heartq wrap`.
...
- **heartq agent stdio** now checks for updates in the background like other modes.
...
- **heartq wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `heartq agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `heartq wrap`** to run any command with local clipboard support.
...
- **Fixed `heartq agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or heartq-desktop.
...
- You can now pass --json-schema to heartq -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `HEARTQ_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **heartq -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in heartq -p and heartq agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `heartq` after running `heartq update`.
- Headless (`heartq -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`heartq plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **heartq sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **heartq quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, heartq sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, HEARTQ_HUNK_TRACKER, or config.
...
- **HeartQ** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`heartq dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`heartq mcp add`** now accepts positional arguments (e.g. `heartq mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`heartq update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.heartq.
...
- **MCP servers menu** and slash commands now work when starting heartq outside a project directory.
...
- **heartq --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `heartq --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `heartq update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`heartq --debug`** now produces per-session log files under ~/.heartq/debug/ even with a leader process.
...
- **`heartq update`** now restarts any older running leader so all clients get the new binary.
...
- **`heartq login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `heartq agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the HEARTQ_IMAGE_GEN_HARNESS env var.
...
- **heartq inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via HEARTQ_CURSOR_MCPS_ENABLED / HEARTQ_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside HeartQ and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the heartq-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality heartq-imagine-image-quality model.
