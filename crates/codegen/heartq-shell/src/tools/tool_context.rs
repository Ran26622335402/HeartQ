...
//! heartq-tools. This struct is NOT used for tool execution — it's session infrastructure.
//!
...
use heartq_workspace::file_system::{AsyncFileSystem, AsyncFsWrapper};
use heartq_workspace::session::file_state::FileStateHandle;
...
pub struct ToolContext {
    ...
    pub lsp: Option<Arc<dyn heartq_tools::implementations::lsp::LspBackend>>,
    ...
    pub monitor_event_buffer:
        Option<heartq_tools::implementations::heartq_build::task::types::MonitorEventBuffer>,
    ...
    pub task_wake_suppressed:
        Option<heartq_tools::reminders::task_completion::TaskWakeSuppressed>,
...
impl ToolContext {
    ...
    pub fn new(
        ...
        let session_env = heartq_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
