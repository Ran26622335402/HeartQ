...
use heartq_sampler::SamplerConfig;
use heartq_tools::implementations::heartq_build;
use heartq_tools::registry::types::ToolConfig;

/// Production heartq-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production heartq-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `HEARTQ_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `HEARTQ_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> heartq_tools::implementations::heartq_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("HEARTQ_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| heartq_config::env_bool("HEARTQ_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `heartq_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `HeartQBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `HeartQBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "HeartQBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "HeartQBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"HeartQBuild:read_file"));
        assert!(ids.contains(&"HeartQBuild:search_replace"));
        assert!(ids.contains(&"HeartQBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"HeartQBuildHashline:hashline_read"));
        assert!(ids.contains(&"HeartQBuildHashline:hashline_edit"));
        assert!(ids.contains(&"HeartQBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            heartq_agent::config::AgentDefinition::plan(),
            heartq_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "HeartQBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"HeartQBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"HeartQBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"HeartQBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production heartq-build must set the 10h foreground ceiling"
        );
    }
