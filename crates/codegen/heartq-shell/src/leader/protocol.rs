...
pub enum ClientMode {
    /// Headless mode (heartq agent, heartq agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (heartq agent stdio, heartq -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `heartq update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `heartq update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `heartq update` just installed; the leader uses
...
if request_id == "ws-1"
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["heartq-a".into(), "heartq-b".into()],
            pid: 4242,
        };
