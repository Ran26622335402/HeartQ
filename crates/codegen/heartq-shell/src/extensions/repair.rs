...
/// back atomically. `heartq_root` is injectable for tests.
async fn repair_on_disk(heartq_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &heartq_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(heartq_root.to_path_buf());
