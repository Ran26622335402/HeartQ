...
//! Delegates to `heartq-plugin-marketplace` crate for scanning and install logic.
...
fn load_filtered_marketplace_sources() -> Vec<heartq_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                heartq_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                heartq_plugin_marketplace::SourceKind::Local { path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        heartq_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let heartq_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = heartq_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = heartq_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
...
async fn handle_update(
    ...
    use heartq_plugin_marketplace::installer;
    use xai_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &heartq_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            heartq_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            heartq_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match heartq_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        heartq_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = heartq_plugin_marketplace::git::default_cache_root();
            match heartq_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                heartq_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = heartq_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = heartq_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = heartq_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        ...
        Err(heartq_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            heartq_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match heartq_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    heartq_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &heartq_plugin_marketplace::MarketplaceSource,
    ...
    let scan = heartq_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = heartq_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match heartq_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: heartq_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.heartq/config.toml`.
async fn handle_add_source(url: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &heartq_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if heartq_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        heartq_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
...
/// Remove a marketplace source from `~/.heartq/config.toml` and uninstall all
...
fn remove_source_locked(source_url_or_path: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let config_path = heartq_home.join("config.toml");
    let is_official = heartq_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<heartq_home>/.config-init.lock`,
...
fn acquire_init_lock(heartq_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(heartq_home);
    let lock_path = heartq_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a heartq_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub fn purge_default_skills_installs(heartq_home: &std::path::Path) {
    purge_default_skills_installs_impl(heartq_home, || {
        heartq_agent::plugins::install_registry::InstallRegistry::try_load_from(
            heartq_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}

fn purge_default_skills_installs_impl(
    heartq_home: &std::path::Path,
    load_registry: impl FnOnce() -> Result<
        heartq_agent::plugins::install_registry::InstallRegistry,
        heartq_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = heartq_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(heartq_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %heartq_home.join(".config-init.lock").display(),
...
pub fn ensure_official_marketplace_source(heartq_home: &std::path::Path) {
    let config_path = heartq_home.join("config.toml");
    ...
    // (settings.json / known_marketplaces.json) under heartq_home. Scoped to
    // heartq_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = heartq_plugin_marketplace::load_sources(&parsed);
    let json_sources = heartq_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&heartq_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, heartq_plugin_marketplace::SourceKind::Git { url, .. }
            if heartq_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            heartq_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                heartq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = heartq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<heartq_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        heartq_plugin_marketplace::load_sources(&parsed)
    }
...
if path == &dir
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                heartq_plugin_marketplace::SourceKind::Git { url, .. }
                    if heartq_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            heartq_plugin_marketplace::SourceKind::Git { url, .. }
                if url == heartq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                heartq_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                heartq_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use heartq_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = heartq_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
