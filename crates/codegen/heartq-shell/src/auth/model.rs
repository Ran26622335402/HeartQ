...
/// auth.json scope key for plain API key auth (desktop login, `heartq login --api-key`).
...
/// server enrichment opts in. Single source for `HeartQAuth`, `AuthMeta`, and
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from heartq-desktop login or `heartq login --api-key`)
    ApiKey,
}
...
pub struct HeartQAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_heartq_code_access: Option<bool>,
...
impl std::fmt::Debug for HeartQAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HeartQAuth")
...
impl HeartQAuth {
    ...
    /// `true` when this auth can access heartq.com managed MCP connectors.
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &HeartQAuth) {
...
#[cfg(test)]
impl HeartQAuth {
    /// Returns a `HeartQAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// HeartQAuth { key: "my-key".into(), ..HeartQAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, HeartQAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `heartq login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<HeartQAuth> {
...
/// Early-invalidation buffer. Override with `HEARTQ_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("HEARTQ_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &HeartQAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &HeartQAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> HeartQAuth {
        HeartQAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    fn is_xai_auth_matrix() {
        ...
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| HeartQAuth {
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "SuperHeartQPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("SuperHeartQPro"));
    }
