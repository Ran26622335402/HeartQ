...
use crate::auth::{AuthManager, HeartQAuth, HeartQComConfig, parse_output};
use crate::util::heartq_home;
...
/// session.
fn is_cached_credential_compatible(auth: &HeartQAuth, heartq_com_config: &HeartQComConfig) -> bool {
    let expected_issuer = heartq_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| heartq_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(heartq_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("HEARTQ_LOGIN_DEVICE_FLOW")
...
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `HEARTQ_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `heartq_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("HEARTQ_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.heartq_com_config());
...
pub async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    heartq_com_config: &HeartQComConfig,
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = heartq_com_config.oidc.is_some(),
        has_oauth2 = heartq_com_config.oauth2.is_some(),
        has_external_auth = heartq_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, heartq_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, heartq_com_config)
        }) {
            heartq_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = heartq_com_config.auth_provider_command {
    ...
    // `heartq login --devbox` uses run_devbox_login and is not gated here.
    if !heartq_com_config.blocks_automatic_oidc()
    ...
    // --device-auth / HEARTQ_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(heartq_com_config) {
        return crate::auth::oidc::run_login_flow(heartq_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = heartq_com_config.oauth2 {
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `heartq login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(heartq_com_config: &HeartQComConfig) -> Option<HeartQAuth> {
    let heartq_home = heartq_home::heartq_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(&heartq_home, heartq_com_config.clone()));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(heartq_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_ensure_session_noninteractive(
    ...
    if let Some(auth) = try_ensure_fresh_auth(heartq_com_config).await {
        return Some(auth);
    }
    let heartq_home = heartq_home::heartq_home();
    let auth_manager = Arc::new(AuthManager::new(&heartq_home, heartq_com_config.clone()));
    ...
    mint_session_noninteractive(&auth_manager, heartq_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<HeartQAuth> {
    auth_manager
        .current_or_expired()
...
async fn mint_session_noninteractive(
    ...
    if let Some(cmd) = heartq_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: HeartQAuth) -> HeartQAuth {
...
fn report_signed_in(auth: &HeartQAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<HeartQAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(heartq_com_config).await)
    } else {
        ensure_authenticated(heartq_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `heartq login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `HEARTQ_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `heartq login` on an
    ...
    let authenticated = if devbox {
    ...
    } else if cli_should_use_device(&config.heartq_com_config, login_override).await {
        if config.heartq_com_config.oauth2.is_none() {
        ...
        let heartq_home = heartq_home::heartq_home();
        let auth_manager = Arc::new(AuthManager::new(&heartq_home, config.heartq_com_config.clone()));
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.heartq_com_config) {
            eprintln!(
    ...
    // config was actually applied; `heartq setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `HEARTQ_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        heartq_telemetry::external::set_identity(
            heartq_telemetry::external::IdentityAttrs::default(),
        );
        heartq_telemetry::external::flush();
...
/// `heartq logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let heartq_home = heartq_home::heartq_home();
    let auth_manager = AuthManager::new(&heartq_home, config.heartq_com_config.clone());
...
mod tests {
    ...
    /// Run `f` with `HEARTQ_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("HEARTQ_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("HEARTQ_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("HEARTQ_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A heartq.com first-party (x.ai-issuer) OIDC session — `is_xai_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> HeartQAuth {
        HeartQAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
            refresh_token: refresh.map(str::to_string),
            ..HeartQAuth::test_default()
        }
    }
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), HeartQComConfig::default());
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only heartq_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), HeartQComConfig::default()));
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), HeartQComConfig::default())
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("HEARTQ_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("HEARTQ_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `heartq login` must use loopback, not device —
        ...
        let xai = HeartQComConfig::default();
    ...
    fn oidc_auth(issuer: &str) -> HeartQAuth {
        HeartQAuth {
    ...
    fn weblogin_cred_is_never_compatible() {
        let cfg = HeartQComConfig::default();
    ...
    fn pinned_cfg(team: &str) -> HeartQComConfig {
        HeartQComConfig {
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = HeartQAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = HeartQComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = HeartQAuth {
        ...
        // HEARTQ_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into HeartQ: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `heartq login` passes `on_stderr=None`; stderr must be inherited so
