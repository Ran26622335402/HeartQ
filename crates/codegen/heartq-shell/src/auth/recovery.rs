...
use crate::auth::model::HeartQAuth;
use crate::auth::token_type::TokenType;
use heartq_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&HeartQAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        heartq_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<HeartQAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<HeartQAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<HeartQAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.heartq_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<HeartQAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            heartq_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<HeartQAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<HeartQAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &HeartQAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, HeartQAuth};
    ...
    fn rejected_cred() -> Option<HeartQAuth> {
        Some(HeartQAuth {
    ...
    impl TokenRefresher for OkRefresher {
        async fn refresh(&self, _reason: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            ...
            RefreshOutcome::Success(Box::new(HeartQAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), HeartQComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(m: &Arc<AuthManager>) -> (Result<HeartQAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.heartq_com_config().auth_scope();
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.heartq_com_config().auth_scope();
        let expired_different = HeartQAuth {
