...
//!
//! 1. [`heartq_telemetry::unified_log::warn`] for the local
//!    `~/.heartq/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`heartq_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`heartq_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`heartq_sampler::SamplerConfig::attribution_callback`] at every
...
use heartq_sampler::{Auth401AttributionCallback, SamplingConsumer};
use heartq_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`heartq_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `heartq_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn heartq_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `heartq_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `heartq-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `heartq-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.heartq/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `HeartQAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `HeartQAuth` (or
    ...
    // `heartq_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.heartq/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> HeartQAuth {
        HeartQAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `heartq_tools::Auth401AttributionCallback`
