...
/// Parse stdout into a session-credential `HeartQAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<HeartQAuth> {
...
pub(crate) async fn run_external_refresh(command: &str) -> Option<HeartQAuth> {
...
pub(crate) async fn refresh_with_command(command: &str, prev_auth: &HeartQAuth) -> Option<HeartQAuth> {
...
mod tests {
    ...
    #[tokio::test]
    async fn sets_heartq_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $HEARTQ_AUTH_EXPIRED")
            .await
            .unwrap();
