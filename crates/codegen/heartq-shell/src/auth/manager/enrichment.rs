...
use crate::auth::model::{HeartQAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: HeartQAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.heartq_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut HeartQAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: HeartQAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut HeartQAuth, user_info: UserInfo) {
