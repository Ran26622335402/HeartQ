...
use crate::util::heartq_auth_credentials::HeartQAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::HeartQAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| heartq_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: HeartQAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = HeartQAuthCredentials::new(None);
...
pub(crate) fn embedding_session_credentials(
...
) -> heartq_memory::EndpointScopedCredentials {
    ...
    heartq_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
///
/// - `"heartq-shell"`   — classic HeartQ CLI / TUI (heartq-shell)
/// - `"heartq-pager"`   — new HeartQ Pager / TUI (heartq-pager)
/// - `"heartq-desktop"` — HeartQ Desktop app
/// - `"heartq-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-heartq-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(heartq_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = HeartQAuthCredentials::new(user_token);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = HeartQAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        heartq_telemetry::external::set_identity(
            heartq_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `heartq-pager` and `heartq-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> heartq_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let heartq_com_config = crate::auth::HeartQComConfig::default();
    let exporter = heartq_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = heartq_com_config.token_header.clone();
    let heartq_home = crate::util::heartq_home::heartq_home();
    let bootstrap = Arc::new(AuthManager::new(&heartq_home, heartq_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    heartq_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::HeartQComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use heartq_auth::AuthCredentialProvider;
    /// Serializes tests that pin `HEARTQ_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `HEARTQ_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("HEARTQ_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("HEARTQ_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("HEARTQ_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("HEARTQ_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> HeartQAuth {
        HeartQAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<HeartQAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::HeartQComConfig::default(),
        ));
        ...
        impl crate::auth::refresh::TokenRefresher for OkRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(HeartQAuth {
                    key: "fresh".into(),
                    auth_mode: crate::auth::AuthMode::Oidc,
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: heartq_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use heartq_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    /// user token, matching the precedence in `HeartQAuthCredentials::apply`.
