...
//! connection to the heartq.com relay server with automatic reconnection.
...
/// Config for the heartq.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a heartq.com first-party session
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.heartq_ws_url.clone(),
            ws_origin: ctx.heartq_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to heartq.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `heartq login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from HeartQ WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(heartq_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                heartq_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test HeartQAuth with the given key.
    fn test_auth(key: &str) -> HeartQAuth {
        HeartQAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let builds = |a: &HeartQAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = HeartQAuth {
    ...
    /// Helper: write a HeartQAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &HeartQAuth) {
