...
pub(super) fn strip_task_tools_at_max_depth(
    ...
    use heartq_tools::implementations::heartq_build::task::MAX_SUBAGENT_DEPTH;
...
pub(crate) async fn handle_subagent_request(
    ...
    }
    {
        ...
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(heartq_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = heartq_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    } else if effective_runtime.isolation != xai_tool_types::SubagentIsolationMode::None
    {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(
            ...
            Err(e) => {
                ...
                std::env::temp_dir().join("heartq-subagent-worktrees").join(&request.id)
            }
        };
    ...
    effective_runtime.capability_mode = heartq_subagent_resolution::intersect_capability_modes(
    ...
    {
        let reminder = heartq_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = heartq_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            heartq_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<heartq_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = heartq_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<heartq_tools::registry::types::ToolConfig> = vec![
            (& heartq_build::ReadFileTool).into(), (& heartq_build::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = heartq_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == heartq_hooks::event::HookEventName::Stop {
                            s.event = heartq_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    heartq_agent::config::McpServerRef::Named(name) => {
                    ...
                    heartq_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            heartq_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            heartq_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    heartq_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                heartq_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    heartq_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            Vec::new(),
            heartq_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        heartq_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        heartq_telemetry::events::Outcome::Cancelled
    } else {
        heartq_telemetry::events::Outcome::Error
    };
    heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/heartq/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
