...
async fn subagent_inherits_session_cli_overrides() {
    use heartq_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = heartq_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
        resume_worktree_action(true, Some("refs/heartq/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
    assert_eq!(
        resume_worktree_action(false, Some("refs/heartq/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn heartq_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = heartq_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn dummy_tracker(
    ...
    let cwd = heartq_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn heartq_workspace::file_system::AsyncFileSystem> = Arc::new(
        heartq_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
...
fn explicit_override_takes_precedence_over_role() {
    ...
    let role = heartq_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            heartq_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = heartq_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = heartq_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(heartq_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use heartq_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(heartq_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use heartq_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
...
async fn bootstrap_fork_live_parent_chat_state_is_forked_with_marker() {
    ...
    let req = bootstrap_test_request(true);
    let mut ctx = ctx_with_toggle(HashMap::new());
    let chat = spawn_test_parent_chat_state("heartq-4.5");
...
async fn handle_subagent_request_rejects_file_as_cwd() {
    ...
    let tmp_file = tmp_dir.path().join("heartq-test-cwd-file");
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("HEARTQ_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("HEARTQ_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("HEARTQ_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("HEARTQ_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("HEARTQ_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("HeartQBuild:read_file");
...
/// Regression: on the DEFAULT heartq-build host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    use heartq_agent::config::BuiltinAgentName;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.parent_agent_name = Some("ai-oncall-bot".to_string());
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::HeartQBuildPlan.as_ref().to_string(),
    );
...
fn subagent_auth_type_rule() {
    ...
    let byok = byok_model_entry("heartq-byok");
    let plain = test_model_entry("heartq-plain");
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("heartq-3".to_string(), test_model_entry("heartq-3-2025-02-15"));
    assert!(
        super::handle_request::task_model_override_error(Some("heartq-3"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
        "key lookup should succeed"
    );
    assert!(
        super::handle_request::task_model_override_error(Some("heartq-3-2025-02-15"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(! error.contains("heartq models"));
    }
    assert!(
...
fn test_sampling_config(model_slug: &str) -> heartq_sampling_types::SamplingConfig {
