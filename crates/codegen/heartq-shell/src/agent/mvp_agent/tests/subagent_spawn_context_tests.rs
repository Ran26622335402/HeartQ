...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = heartq_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                heartq_workspace::permission::spawn_permission_manager(
                    sid.clone(),
                    gateway,
                    cwd,
                    heartq_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                heartq_workspace::permission::AccessKind::Read(Some(".env".into())),
                heartq_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        heartq_workspace::permission::Decision::PolicyDeny(_)
                    ),
