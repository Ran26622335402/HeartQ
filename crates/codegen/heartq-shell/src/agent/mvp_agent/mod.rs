...
use heartq_telemetry::id::{agent_id, agent_instance_id};
use heartq_telemetry::session_ctx::log_event;
use heartq_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use heartq_workspace::permission::{ClientType, PermissionEvent};
...
use heartq_workspace::session::git::GitDiscoveryResult;
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::HeartQAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperHeartQPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "HeartQPro" => jwt_claim == "superheartq",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperHeartQPro" => jwt_claim == "superheartq_heavy",
        "SuperHeartQLite" => jwt_claim == "superheartq_lite",
        _ => false,
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a heartq_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[heartq_announcements::RemoteAnnouncement]>,
    last_emitted: &[heartq_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<heartq_announcements::RemoteAnnouncement>> {
    let current = heartq_announcements::filter_expired_at(
...
/// Override with `HEARTQ_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("HEARTQ_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// heartq.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// list. Set by `enforce_heartq_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.heartq/`).
    ///
    ...
    /// Local session-registry override: `HEARTQ_SESSION_REGISTRY` env, else
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        heartq_tools::implementations::heartq_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: heartq_tools::implementations::heartq_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: heartq_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled heartq-desktop's first
    ...
    workspace_ops: RefCell<Option<heartq_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<heartq_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &heartq_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<heartq_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !heartq_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = heartq_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `heartq-build` →
/// `heartq-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `heartq-build` →
/// `heartq-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use heartq_agent::config::is_strict_harness_agent_type;
    match (
...
///
/// Headers injected:
///  - `x-heartq-client-version` -- required by the proxy's version-gate check.
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-heartq-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
    ...
    headers
        .entry("x-heartq-client-identifier".to_string())
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = heartq_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_heartq_code_access(&self, auth: &crate::auth::HeartQAuth) {
        if !auth.is_xai_auth() {
        ...
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings heartq_build_access_gate)"
            );
            ...
            .map(|auth| {
                let gate = if !self.tier_allowed.get() && gate.is_none() {
                    ...
                    Some(crate::auth::GateInfo {
                        message,
                        url: Some(
                            "https://heartq.com/superheartq?referrer=heartq-build".to_string(),
                        ),
    ...
    /// cached_token, heartq.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.heartq/bundled/` and the manifest.
...
/// absent — matching the `heartq_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_heartq_code_access` (initial login gate) and
