...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        heartq_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::heartq_home::heartq_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    heartq_workspace::permission::cleanup_stale_permission_state(
        ...
        heartq_workspace::trust::migrate_legacy_hook_trust();
        ...
        if !self.tier_allowed.get() && let Some(auth) = self.auth_manager.current() {
            self.enforce_heartq_code_access(&auth).await;
        }
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("heartq-web") => client_type = ClientType::HeartQWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("heartq-code-extension") => client_type = ClientType::Extension,
                Some("heartq-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.heartq_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.heartq_com_config.auth_provider_label.clone(),
                cfg.heartq_com_config.auth_provider_command.is_some(),
                cfg.heartq_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                ...
                "auth: advertising heartq.com auth method",
            );
        }
        let preferred_method = self.cfg.borrow().heartq_com_config.preferred_method;
        ...
        heartq_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "heartq_home" : crate ::util::heartq_home::heartq_home().display()
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("HEARTQ_AGENT_METADATA");
                    serde_json::json!(
                        { "heartqShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (heartq_mcp::wire::MCP_SDK) :
                        ...
                        heartq_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().heartq_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(heartq_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().heartq_com_config.preferred_method, Some(crate
            ...
            auth_method::HEARTQ_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let heartq_ctx = self.auth_manager.heartq_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_heartq_oauth(cli_oauth);
                tracing::debug!(
                ...
                let auth_result = if !auth_meta.headless {
                    ...
                    tokio::select! {
                        ...
                        heartq_ctx, crate ::auth::AuthChannels { url_tx : Some(url_tx),
                    ...
                    tokio::select! {
                        biased; _ = cancel.cancelled() => { cancelled = true;
                        Err(anyhow::anyhow!("Authentication cancelled")) } r = crate
                        ::auth::run_auth_flow(& self.auth_manager, heartq_ctx, auth_meta
                    ...
                    })?;
                {
                    ...
                    tracing::debug!(
                        "auth: heartq.com/oidc handler set api_key (SessionToken)"
                    );
                    heartq_telemetry::unified_log::debug(
                        "auth: heartq.com/oidc handler set api_key (SessionToken)",
                        None,
    ...
    async fn new_session(
        ...
        heartq_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || heartq_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = heartq_telemetry::context::collect_git_context(&cwd_str);
                let ev = heartq_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                heartq_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match heartq_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
    ...
    async fn load_session(
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        heartq_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        heartq_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        heartq_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => heartq_telemetry::events::PlanModeState::Inactive,
        };
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            heartq_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = heartq_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : heartq_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a heartq worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            heartq_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use heartq_workspace::session::git::RestoreKind;
            let outcome = heartq_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let preloaded_envrc = heartq_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let is_heartq_build = persisted_model.0.starts_with("heartq-build");
        let same_family_fallback = if is_heartq_build {
            available.keys().find(|id| id.0.starts_with("heartq-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("heartq-build")).cloned()
        };
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    heartq_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = heartq_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = heartq_workspace::session::git::detect_head_divergence(
        ...
        if self.product_analytics_enabled() {
            log_event(heartq_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = heartq_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = heartq_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
        ...
        let result = match method.as_ref() {
            ...
            "x.ai/cloud/terminate" => {
                crate::extensions::auth_gate::require_xai_auth(
                    ...
                    "Run `heartq login` to authenticate.",
                )?;
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| heartq_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| heartq_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == heartq_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                heartq_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            heartq_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
