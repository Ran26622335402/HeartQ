...
impl MvpAgent {
    ...
    pub(crate) fn sync_process_static_api_key(&self, preferred_model_id: Option<&str>) {
        if self.cfg.borrow().heartq_com_config.api_key_auth_disabled() {
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::HeartQAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// must not block the ACP response (heartq-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled heartq-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            heartq_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                heartq_version::VERSION.to_owned(),
                subscription_tier,
    ...
    /// Return the current HeartQAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::HeartQAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &heartq_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on heartq-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<heartq_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    heartq_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    heartq_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match heartq_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => heartq_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    ///     `heartq.com`, `oidc`) -- even if the in-memory token is currently
    ///     expired or missing.
    ///
    ...
    /// iff `should_advertise_xai_api_key`; otherwise `heartq.com`. Returns `None`
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().heartq_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_xai_api_key(
                self.cfg.borrow().heartq_com_config.api_key_auth_disabled(),
    ...
    /// `use_oauth` only when the target is interactive `heartq.com`. When
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let Some(method_id) = self.cached_token_fallthrough_method_id() else {
            ...
            heartq_telemetry::unified_log::warn(
        ...
        let meta = if method_id.0.as_ref() == auth_method::HEARTQ_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::HeartQAuth) {
        ...
        let (
            ...
            let heartq_user_id = is_xai.then(|| user_id.clone());
            let heartq_team_id = is_xai.then(|| team_id.clone()).flatten();
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<heartq_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    /// Whether the current session is a personal heartq.com account on a gated
    ...
    /// SuperHeartQ upsell prose (see `ImageGenConfig`/`VideoGenConfig`'s
    /// `tier_restricted`).
    ///
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> heartq_tools::implementations::heartq_build::image_gen::ImageGenConfig {
        ...
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| heartq_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("heartq-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use heartq_tools::implementations::heartq_build::deploy_app::AppBuilderDeployerConfig;
    ...
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> heartq_tools::implementations::heartq_build::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `HEARTQ_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `HEARTQ_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `HEARTQ_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> heartq_tools::implementations::heartq_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        if !cfg.heartq_com_config.api_key_auth_disabled() {
        ...
        if relay_sync_enabled {
            tracing::info!("[heartq] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_xai_auth {
            tracing::info!(
                "[heartq] Relay sync: DISABLED (no auth - run 'heartq login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: heartq_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: heartq_tools::implementations::heartq_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().heartq_com_config.auth_provider_command.clone(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<heartq_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> heartq_tools::implementations::heartq_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<heartq_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or HEARTQ_RELAY_SYNC_ENABLED)
    ...
    pub async fn execute_hooks_action(
            ...
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&heartq_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<heartq_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::HeartQSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<heartq_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(heartq_version::VERSION.to_string()),
                workspace_type: None,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                heartq_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// 5. `HEARTQ_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `HEARTQ_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`heartq_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<heartq_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> heartq_agent::AgentDefinition {
        use heartq_agent::AgentDefinition;
        let heartq_agent_env_set = std::env::var("HEARTQ_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(heartq_agent::config::is_strict_harness_agent_type);
        if !heartq_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = heartq_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = heartq_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("HEARTQ_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("heartq-build-concise") | Some("heartq_build_concise") => {
                AgentDefinition::heartq_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::heartq_build_plan()
                    }
                }
            }
            Some(name) => {
                heartq_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::heartq_build_plan)
            }
            None => AgentDefinition::heartq_build_plan(),
        };
        if !heartq_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = heartq_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn heartq_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let loc_tracking_enabled = hunk_tracking_enabled && has_xai_auth
            && (self
                ...
                .unwrap_or(false)
                || std::env::var("HEARTQ_LOC_TRACKING")
        ...
        let mut session_env = heartq_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                heartq_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            heartq_agent::config::ModelOverride::Override(id) => {
            ...
            heartq_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = heartq_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = heartq_tools::util::heartq_home::heartq_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".heartq").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let attribution_callback: Option<
                heartq_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = heartq_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = heartq_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    subagent_toggle,
                    Vec::new(),
                    heartq_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
