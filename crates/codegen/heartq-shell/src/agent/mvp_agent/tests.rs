...
fn jwt_tier_claim_maps_free_and_paid() {
    ...
    assert_eq!(
        ...
        Some("superheartq_heavy")
    );
    assert_eq!(
        jwt_tier_claim(&jwt_with_tier(6)).as_deref(),
        Some("superheartq_lite")
    );
    assert_eq!(jwt_tier_claim(&jwt_with_tier(99)).as_deref(), Some("99"));
}
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::HeartQAuth {
    crate::auth::HeartQAuth {
        key: key.into(),
        auth_mode: mode,
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ("superheartq", "HeartQPro"),
        ("x_basic", "XBasic"),
        ("x_premium", "XPremium"),
        ("x_premium_plus", "XPremiumPlus"),
        ("superheartq_heavy", "SuperHeartQPro"),
        ("superheartq_lite", "SuperHeartQLite"),
    ];
...
fn jwt_claim_matches_user_subscription_tier_rejects_stale_and_unknown() {
    ...
    assert!(!jwt_claim_matches_user_subscription_tier("free", "HeartQPro"));
...
fn harness_pair(id: &str) -> Vec<heartq_sampling_types::conversation::ConversationItem> {
    use heartq_sampling_types::ToolCall;
    use heartq_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_heartq_build() {
    let prev = std::env::var("HEARTQ_AGENT").ok();
    unsafe {
        std::env::remove_var("HEARTQ_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("HEARTQ_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return heartq-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = heartq_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `heartq-build-plan`, models in the catalog that still declare
/// `agent_type = "heartq-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `heartq-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_heartq_build_family() {
    let prev = std::env::var("HEARTQ_AGENT").ok();
    unsafe {
        std::env::remove_var("HEARTQ_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["heartq-build", "heartq-build-plan", "heartq-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for heartq-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("HEARTQ_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use heartq_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".heartq").join("agents");
    ...
    assert_eq!(
        def.model,
        heartq_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("HEARTQ_AGENT", v) },
        None => unsafe { std::env::remove_var("HEARTQ_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `heartq-build` (a client profile name) to
/// `heartq-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("heartq-build", "heartq-build-plan"));
    assert!(harnesses_are_compatible("heartq-build-plan", "heartq-build"));
    assert!(harnesses_are_compatible("heartq-build", "heartq-build"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("heartq-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "heartq-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_heartq_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "heartq-build-plan"),
        "heartq-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_heartq_build() {
    assert_eq!(
        resolve_required_agent_type(None, "heartq-build"),
        "heartq-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "heartq-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "heartq-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "heartq-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use heartq_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use heartq_tools::notification::ToolNotificationHandle;
    use heartq_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = heartq_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: heartq_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: heartq_tools::implementations::heartq_build::image_gen::ImageGenConfig::default(),
        video_gen_config: heartq_tools::implementations::heartq_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: heartq_tools::implementations::heartq_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: heartq_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            heartq_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(heartq_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: heartq_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "heartq-build".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: heartq_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("heartq-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("heartq-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("heartq-3", false, None)),
        (sid_b.clone(), make_test_handle("heartq-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use heartq_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use heartq_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("heartq-3", false, Some("heartq-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("heartq-3", false, Some("heartq-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("heartq-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("heartq-3", true, Some("heartq-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("heartq-3", true, Some("heartq-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("heartq-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("heartq-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("heartq-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::HeartQWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::HeartQAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::HeartQAuth::test_default()
            });
            use acp::Agent as _;
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, HeartQComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), HeartQComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::HeartQAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, HeartQComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let heartq_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("HEARTQ_HOME", heartq_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), HeartQComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("heartq-3", false, None);
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("heartq-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("heartq-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("heartq-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("heartq-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("heartq-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("heartq-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::HeartQWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::HeartQTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::HeartQWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::HEARTQ_COM_METHOD_ID,
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::HeartQAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(HeartQAuth::test_default());
...
/// Minimal agent whose `heartq_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.heartq_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use heartq_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("HEARTQ_DISABLE_API_KEY_AUTH");
...
/// switch keeps the fallthrough on interactive `heartq.com` (api-key auth is
/// neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{HEARTQ_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use heartq_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("HEARTQ_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(HEARTQ_COM_METHOD_ID),
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive heartq.com so XAI_API_KEY can't bypass forced IdP login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to log in, so the fallthrough is interactive `heartq.com`.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_heartq_com_without_credentials() {
    use crate::agent::auth_method::{
        HEARTQ_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use heartq_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("HEARTQ_DISABLE_API_KEY_AUTH");
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(HEARTQ_COM_METHOD_ID),
        "no API-key creds and no kill switch -> interactive heartq.com login",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use heartq_tools::implementations::heartq_build::video_gen::{
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use heartq_tools::implementations::heartq_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
/// Build traffic via the `x-heartq-client-identifier` header. If this header is
...
async fn prepare_image_gen_config_sends_client_identifier_header() {
    ...
    assert_eq!(
        extra_headers
            .get("x-heartq-client-identifier")
            .map(String::as_str),
...
async fn prepare_video_gen_config_sends_client_identifier_header() {
    use heartq_tools::implementations::heartq_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::HeartQAuth::test_default());
    assert!(
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("heartq-4.5"), Some("heartq-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("heartq-4.5"), None),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = heartq_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = heartq_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(heartq_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<heartq_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::HEARTQ_DISABLE_CUSTOM_BRIDGE_ENV);
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (heartq-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("heartq-tui"));
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".heartq/roles");
    let personas = cwd.join(".heartq/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.sessions.borrow_mut().insert(sid.clone(), handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                heartq_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.heartq/roles` / `personas` alone must drive
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _env = EnvGuard::set("HEARTQ_HOME", home.path());
    let _sim = EnvGuard::set(heartq_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("HEARTQ_FOLDER_TRUST");
    let repo = tempfile::tempdir().unwrap();
    git2::Repository::init(repo.path()).unwrap();
    write_project_subagent_definitions(repo.path());
    run_local_for_bridge_test(|| async {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use heartq_test_support::EnvGuard;
    use heartq_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("HEARTQ_HOME", home.path());
    let _sim = EnvGuard::set(heartq_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("HEARTQ_FOLDER_TRUST");
...
fn ann(id: &str) -> heartq_announcements::RemoteAnnouncement {
    heartq_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<heartq_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = heartq_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = heartq_announcements::RemoteAnnouncement {
