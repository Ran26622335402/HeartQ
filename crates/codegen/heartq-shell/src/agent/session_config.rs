...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct HeartQSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("heartq-build", "HeartQ Build"),
            model("heartq-4.5", "HeartQ 4.5"),
        ];
        let current = acp::ModelId::from("heartq-build");
        ...
        assert_eq!(selected_models[0].id, "heartq-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("heartq-build", "HeartQ Build")];
        let current = acp::ModelId::from("heartq-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("heartq-build", "")];
        let current = acp::ModelId::from("heartq-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "heartq-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "heartq-build".to_string(),
            category: "model".to_string(),
            label: "HeartQ Build".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "heartq-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "HeartQ Build");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn heartq_session_detail_serializes_camel_case() {
        let detail = HeartQSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "heartq-build");
