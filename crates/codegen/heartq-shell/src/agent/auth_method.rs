...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "HEARTQ_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `HEARTQ_CODE_XAI_API_KEY` for backward compatibility.
...
/// Returns `true` if either `XAI_API_KEY` or `HEARTQ_CODE_XAI_API_KEY` is set.
...
/// `HEARTQ_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// Optional display label for the login method (`heartq.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `heartq_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `heartq.com` method).
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.heartq_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.heartq_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(heartq_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            ...
            HEARTQ_COM_METHOD_ID => Self::HeartQCom,
    ...
    /// `true` for session-based methods (cached_token, heartq.com, oidc).
    pub fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::HeartQCom | Self::Oidc)
    }

    /// Requires user interaction (browser, OIDC redirect, or external auth command).
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::HeartQCom | Self::Oidc)
    }
...
/// `true` for session-based ACP methods (cached_token, heartq.com, oidc).
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `heartq login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `heartq login`, set XAI_API_KEY, or add api_key to ~/.heartq/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `heartq login` to authenticate.";
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.heartq/auth.json".to_string())),
    )
}

pub const HEARTQ_COM_METHOD_ID: &str = "heartq.com";

/// xAI OAuth2/OIDC auth. Method id `"heartq.com"` kept for ACP wire-compat.
pub fn heartq_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("HeartQ");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(HEARTQ_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// No advertiseable API-key credentials → interactive `heartq.com`.
    #[test]
    fn after_cached_token_unavailable_falls_to_heartq_com_without_api_key() {
        assert_eq!(
    ...
    /// `heartq.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `heartq.com` is
    ...
    #[test]
    fn fresh_user_only_advertises_heartq_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::HeartQCom));
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `heartq.com` (mutually exclusive). xai.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_heartq_com_but_xai_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::HeartQCom),
            "heartq.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `heartq.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let heartq = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::HeartQCom)
            .expect("heartq.com must be advertised");
        assert_eq!(heartq.name(), "Acme Corp");
        let meta = heartq.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.heartq/config.toml` skeleton from the bug report, walks it through
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // heartq.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
    ...
    /// Legacy `HEARTQ_CODE_XAI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `HEARTQ_CODE_XAI_API_KEY` are set,
    ...
    // -- heartq login --legacy regression coverage ------------------------
    //
    // `heartq login --legacy` produces a HeartQAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `HEARTQ_AUTH` env var (or via legacy
    ...
    // to-end. A regression in HEARTQ_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `HEARTQ_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn heartq_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, HeartQAuth, HeartQComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("HEARTQ_AUTH_PATH");
        let _g2 = EnvGuard::unset(XAI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `heartq login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = HeartQAuth {
        ...
        // Provide it via HEARTQ_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("HEARTQ_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in HEARTQ_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("HEARTQ_AUTH");
        let _g2 = EnvGuard::unset("HEARTQ_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::HeartQCom),
            "no cached token AND no api key: pager must show login (heartq.com first)",
        );
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, HEARTQ_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![HEARTQ_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
