...
use crate::auth::{AuthManager, HeartQAuth, HeartQComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use heartq_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&heartq_home, HeartQComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<heartq_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<heartq_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `heartq-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-heartq-build`
    /// → slug `heartq-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.heartq/models_cache.json` after an
    ...
    /// When another heartq process sharing `~/.heartq` (a `--no-leader` run, a
    /// newer client, heartq-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `heartq_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::heartq_home::heartq_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.heartq_version.as_deref() != Some(heartq_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            heartq_version: Some(heartq_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "HeartQ Build" both route to `heartq-build`)
...
pub(crate) fn prefetch_models_and_settings_blocking(
    ...
    // Settings need a heartq.com session; skip for BYOK.
...
fn resolve_prefetch_env_with_auth(auth: Option<HeartQAuth>) -> Option<PrefetchEnv> {
...
fn resolve_prefetch_env(heartq_com_config: Option<HeartQComConfig>) -> Option<PrefetchEnv> {
    let heartq_home = crate::util::heartq_home::heartq_home();
    let auth_manager = AuthManager::new(&heartq_home, heartq_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<HeartQAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(heartq_com_config: Option<HeartQComConfig>) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(heartq_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `heartq-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-heartq-build`).
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto heartq-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("heartq-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, HeartQComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("heartq-4"));
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("heartq-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("heartq-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, HeartQComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use heartq_sampling_types::ReasoningEffortOption;
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("heartq-4.5"),
            api_key: None,
            env_key: None,
            auth_provider: None,
            api_base_url: None,
        ...
        prefetched.insert("heartq-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["heartq-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("heartq-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, HeartQComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn first_apply_refresh_reselects_default_model() {
        let mgr = test_manager();
        let mut cfg = config::Config::default();
        cfg.models.default = Some("heartq-3".to_string());

        assert!(!mgr.has_fetched_real_catalog());

        let prefetched = make_prefetched(&["heartq-3", "heartq-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);

        assert!(mgr.has_fetched_real_catalog());
        assert_eq!(mgr.current_model_id().0.as_ref(), "heartq-3");
    }
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with heartq-4 removed.
        let prefetched = make_prefetched(&["heartq-3", "heartq-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn apply_config_honors_new_preferred_model() {
        ...
        *mgr.inner.cfg.write() = stale_cfg;
        ...
        new_cfg.models.default = Some("heartq-3".to_string());
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("heartq-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn auth_refresh_then_config_reload_preserves_user_model() {
        ...
        *mgr.inner.etag.write() = None;
        ...
        let prefetched = make_prefetched(&["heartq-3", "heartq-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        assert_eq!(mgr.current_model_id().0.as_ref(), "heartq-4");

        // Config reload with persisted preference.
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("heartq-4".to_string());
        mgr.apply_config(new_cfg);
        assert_eq!(mgr.current_model_id().0.as_ref(), "heartq-4");
    }
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["heartq-4.5", "heartq-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("heartq-4.5"));
        assert!(mgr.models().contains_key("heartq-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["heartq-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("heartq-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["heartq-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("heartq-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["heartq-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("heartq-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["heartq-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("heartq-legacy"));
    }
    ...
    fn clear_resets_has_fetched_real_catalog() {
        ...
        let prefetched = make_prefetched(&["heartq-4.5", "heartq-4.3"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        let first_available = mgr.available().keys().next().unwrap().clone();
        assert_eq!(
            mgr.current_model_id().0.as_ref(),
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("HEARTQ_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "heartq-build", Some("Auto")),
            make_entry_config_with_id(Some("heartq-build"), "heartq-build", Some("HeartQ Build")),
            make_entry_config_with_id(
                Some("heartq-composer-2.5-fast"),
                "heartq-composer-2.5-fast",
                Some("HeartQ Fast"),
            ),
        ];
        ...
        assert!(map.contains_key("heartq-build"));
        assert!(map.contains_key("heartq-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "heartq-build",
            "auto entry should still route to heartq-build"
        );
        assert_eq!(map["heartq-build"].info.model, "heartq-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("heartq-build"), "heartq-build", Some("First")),
            make_entry_config_with_id(Some("heartq-build"), "heartq-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["heartq-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-heartq-build".to_string(),
            make_model_entry("heartq-build"),
        );
        catalog.insert("heartq-build".to_string(), make_model_entry("heartq-build"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("heartq-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "heartq-build", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-heartq-build".to_string(),
            make_model_entry("heartq-4.5"),
        );
        models.insert("heartq-4.3".to_string(), make_model_entry("heartq-4.3"));

        let persisted = acp::ModelId::new("heartq-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "enterprise-heartq-build");
    }

    #[test]
    fn resolve_catalog_key_prefers_exact_key_match() {
        let mut models = IndexMap::new();
        models.insert("heartq-4.5".to_string(), make_model_entry("heartq-4.5"));

        let persisted = acp::ModelId::new("heartq-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
        assert_eq!(key.0.as_ref(), "heartq-4.5");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-heartq-build".to_string(),
            make_model_entry("heartq-4.5"),
        );
        models.insert("user-heartq-build".to_string(), make_model_entry("heartq-4.5"));

        let persisted = acp::ModelId::new("heartq-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-heartq-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("heartq-build".to_string(), make_model_entry("heartq-build"));
        models.insert(
            "enterprise-heartq-build".to_string(),
            make_model_entry("heartq-build"),
        );
        ...
        let available = test_available_keys(&["enterprise-heartq-build", "heartq-4.3"]);

        let persisted = acp::ModelId::new("heartq-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("heartq-build".to_string(), make_model_entry("heartq-4.5"));
        models.insert("other".to_string(), make_model_entry("heartq-build"));

        let available = test_available_keys(&["heartq-build", "other"]);

        let persisted = acp::ModelId::new("heartq-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "heartq-build");
    }
