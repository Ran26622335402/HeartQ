...
use crate::auth::{AuthManager, AuthMode, HeartQAuth, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (heartq.com
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = heartq_agent::prompt::workspace_user::optional_workspace_user_dir();
...
pub async fn run_stdio_agent(
    ...
    heartq_telemetry::unified_log::set_version(heartq_version::VERSION);
    ...
    xai_file_utils::queue::cleanup_orphaned_uploads(
        &heartq_home::heartq_home(),
        xai_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by heartq-desktop when spawning `heartq agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("HEARTQ_CLIENT_VERSION") {
        crate::unified_log::info(
            "HEARTQ_CLIENT_VERSION",
            None,
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `heartq agent stdio` is a local/interactive entrypoint (spawned by
            // heartq-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    heartq_telemetry::unified_log::set_version(heartq_version::VERSION);
    // `heartq agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `heartq agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a heartq.com session. \
        Run `heartq login` to sign in, or use `heartq agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.heartq_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'heartq login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `heartq login`."),
        }
    } else if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&heartq_home::heartq_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
    ...
    // Capture the heartq build URL for the first-connection callback
    let heartq_code_url = format!("{}/build", ctx.heartq_ws_origin);
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open HeartQ Build: {} (press Enter to open in browser)",
                heartq_code_url
            );
            eprintln!();
            let url_for_open = heartq_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&HeartQAuth>, session: &HeartQAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's heartq.com relay connection according to the start policy,
/// returning the slot where the [`RelayHandle`](crate::agent::relay::RelayHandle)
/// is parked once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `heartq agent leader`
...
///   duplication of mirroring every agent message to heartq.com.
///
...
/// When a heartq.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the heartq.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: heartq_version::VERSION.to_string(),
    })
    ...
    let auth: Option<HeartQAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    // ── Phase 6b: Legacy devbox auth migration ─────────────────────────────
    let auth: Option<HeartQAuth> = migrate_devbox_auth_if_legacy(auth, &agent_config).await;

    let auth_for_prefetch: Option<HeartQAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            // Bridge websocket messages to agent (from heartq.com relay)
            ...
            // Start (or arm) the heartq.com relay. Eager by default — a bare
            // `heartq agent leader` (devbox / systemd) has no local IPC clients
            ...
            let relay_handle_slot = if let Some(relay_config) = relay_config {
                ...
                info!("Relay disabled: no heartq.com session token (BYOK / local-only leader)");
                Rc::new(std::cell::RefCell::new(None))
            };
            ...
            let auth_scope = agent_config.heartq_com_config.auth_scope();
            // Gated on user_heartq_home() so a cwd-relative .heartq/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = heartq_config::user_heartq_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.heartq/models_cache.json
                            // (another heartq process fetched a fresher /v1/models
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> HeartQAuth {
        HeartQAuth {
            key: key.into(),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::HeartQComConfig {
            heartq_ws_url: format!("ws://{addr}"),
            heartq_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    /// `heartq agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the heartq.com relay eagerly.
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `heartq agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
