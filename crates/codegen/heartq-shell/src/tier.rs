...
//! (SuperHeartQ, SuperHeartQ Heavy/Lite, X Premium/+, and any unknown future name)
...
mod tests {
    ...
    fn unrestricted_names() {
        assert!(!is_restricted_tier_name("SuperHeartQ"));
        assert!(!is_restricted_tier_name("SuperHeartQ Heavy"));
        assert!(!is_restricted_tier_name("superheartq_lite"));
