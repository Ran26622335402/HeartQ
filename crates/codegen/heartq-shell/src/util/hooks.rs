...
use heartq_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .heartq/hooks/ directories are still scanned (they hold
    ...
    // user_heartq_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .heartq.
    ...
    if let Some(ref heartq) = heartq {
        global.push(heartq.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = heartq
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".heartq").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    Vec<heartq_hooks::error::HookError>,
) {
