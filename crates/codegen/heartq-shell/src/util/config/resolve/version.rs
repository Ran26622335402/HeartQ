...
///
/// Reads `stable_version` from `~/.heartq/version.json` (written by the
...
///
/// This is a lightweight duplicate of `heartq_update::channel_name()` for
/// use in `heartq-shell` which cannot depend on `heartq-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    use std::sync::OnceLock;
    static NAME: OnceLock<Option<&'static str>> = OnceLock::new();
    *NAME.get_or_init(|| {
        let version_path = crate::util::heartq_home::heartq_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(heartq_version::VERSION).ok()?;
