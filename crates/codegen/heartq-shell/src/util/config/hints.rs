...
/// env master `HEARTQ_CONTEXTUAL_HINTS` (all-on/off) > user config
...
pub fn resolve_contextual_hints(
    ...
    let resolve_tip = |user: Option<bool>, feature_flag: Option<bool>| -> bool {
        BoolFlag::env("HEARTQ_CONTEXTUAL_HINTS")
            .config(user)
...
fn merge_hints_config_layers(
    ...
    if let Some(m) = managed {
        heartq_config::deep_merge_toml(&mut merged, m);
    }
    if let Some(u) = user {
        heartq_config::deep_merge_toml(&mut merged, u);
    }
    if let Some(r) = requirements {
        heartq_config::deep_merge_toml(&mut merged, r);
    }
    merged
}
...
mod tests {
    ...
    const ENV_CONTEXTUAL_HINTS: &str = "HEARTQ_CONTEXTUAL_HINTS";

    // `HEARTQ_CONTEXTUAL_HINTS` is process-global; serialize the tests reading it
