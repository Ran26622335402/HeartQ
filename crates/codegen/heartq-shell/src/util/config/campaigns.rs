...
pub fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_heartq_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several heartq processes share `$HEARTQ_HOME`; the in-process mutex alone would
...
/// `HEARTQ_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` = none; beats
...
pub fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("HEARTQ_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid HEARTQ_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `HEARTQ_CAMPAIGNS_OVERRIDE` (replaces all
...
/// `HEARTQ_CAMPAIGNS_OVERRIDE`. For one-shot CLI entrypoints that never fetch
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<heartq_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    use heartq_config::ConfigLayers;
    ...
    /// `HEARTQ_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the
    ...
    fn override_beats_kill_switch() {
        ...
        let _env = EnvGuard::unset("HEARTQ_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `HEARTQ_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*: the
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("HEARTQ_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// made while `HEARTQ_CAMPAIGNS=0` must still record the dismissal, or a
    /// later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::unset("HEARTQ_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::set("HEARTQ_CAMPAIGNS", "0");
    ...
    /// `HEARTQ_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing — even
    /// layer + remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("HEARTQ_CAMPAIGNS_OVERRIDE", "[]");
