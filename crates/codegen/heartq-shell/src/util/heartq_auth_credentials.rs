...
use std::sync::Arc;
/// Credentials for authenticating with heartq backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: heartq-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct HeartQAuthCredentials {
...
impl std::fmt::Debug for HeartQAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HeartQAuthCredentials")
            .field(
                "user_token",
...
impl HeartQAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your HEARTQ_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `heartq login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> HeartQAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> HeartQAuthCredentials {
...
impl heartq_auth::HttpAuth for HeartQAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        HeartQAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = HeartQAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = HeartQAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = HeartQAuthCredentials::new(Some("static-token".into()));
