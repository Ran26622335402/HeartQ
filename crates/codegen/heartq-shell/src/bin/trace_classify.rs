...
//!       [--api-key <key> | $XAI_API_KEY | <heartq-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use heartq_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "heartq-4.5")]
    model: String,
    ...
    /// `$XAI_API_KEY`, then `<heartq-home>/auth.json` (`xai::api_key`
    ...
    /// shell uses (`$HEARTQ_HOME` or `~/.heartq`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    heartq_home: Option<PathBuf>,
}
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert!(cli.heartq_home.is_none());
    }
    ...
    #[test]
    fn cli_heartq_home_override_parses() {
        ...
        .expect("parse");
        assert_eq!(cli.heartq_home, Some(PathBuf::from("/tmp/scratch-heartq")));
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["heartq-4.5"]);
