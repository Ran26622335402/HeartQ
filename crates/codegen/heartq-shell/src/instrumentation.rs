//! Shim — see `heartq_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `heartq_shell::instrumentation_timer!`). Keeping the macro here
...
pub use heartq_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `heartq_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `heartq_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `heartq_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `heartq_telemetry::instrumentation`.
#[macro_export]
