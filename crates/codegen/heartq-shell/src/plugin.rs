...
use heartq_agent::plugins::discovery::PluginScope;
use heartq_agent::plugins::git_install::{self, UpdateStatus};
use heartq_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use heartq_plugin_marketplace::git::{self, SourceCacheLease};
use heartq_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.heartq/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &heartq_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `heartq plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `heartq plugin install ./{name}`, or add a \
                     source with `heartq plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `heartq plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `heartq plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
pub fn marketplace_require_sha() -> bool {
    heartq_config::load_effective_config_disk_only()
        .map(|c| heartq_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| heartq_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &heartq_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.heartq/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user heartq via user_heartq_home() (None when no home resolves) and
    // home separately, so removal still runs from $HEARTQ_HOME when no home dir
    // exists, and never touches a cwd-relative .heartq.
    let home = dirs::home_dir();
    let heartq = heartq_config::user_heartq_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref heartq) = heartq {
        settings_candidates.push(heartq.join("settings.local.json"));
        settings_candidates.push(heartq.join("settings.json"));
    }
    ...
    if let Some(ref heartq) = heartq {
        known_candidates.push(heartq.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("heartq plugin install ./sentry"), "{msg}");
        assert!(msg.contains("heartq plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("heartq plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> heartq_workspace::permission::resolution::MarketplaceAllowlist {
        heartq_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = heartq_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
