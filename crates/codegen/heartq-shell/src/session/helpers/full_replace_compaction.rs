//! heartq-build's L5 wiring onto the shared full-replace engine
//! (`heartq_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](heartq_compaction::sample_full_replace_summary);
//! this module adapts heartq-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](heartq_compaction::FullReplaceError).
...
use heartq_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use heartq_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for heartq-build's full-replace pass.
///
...
/// else the structured heartq-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the heartq-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured heartq-build), ignoring the shared
...
/// Map heartq-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces heartq-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    heartq_telemetry::session_ctx::log_event(CompactionRetryDegraded {
