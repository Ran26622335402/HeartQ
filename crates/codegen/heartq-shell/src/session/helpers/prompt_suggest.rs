...
/// prediction.
pub(crate) const DEFAULT_SUGGEST_MODEL: &str = "heartq-build-0.1";
...
/// (`heartq-build-0.1`) is API-key-only and excluded from OAuth catalogs, so
...
/// `HEARTQ_PROMPT_SUGGESTIONS_MODEL` keeps working for models the catalog does
...
mod tests {
    ...
    fn effective_model_default_requires_catalog() {
        ...
        // OAuth catalogs exclude heartq-build-0.1 → skip the request entirely,
    ...
    fn effective_model_env_pin_bypasses_catalog_guard() {
        // HEARTQ_PROMPT_SUGGESTIONS_MODEL is the explicit escape hatch: used
