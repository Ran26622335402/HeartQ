...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for heartq-build in remote settings).
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `heartq-build`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "heartq-build" | "heartq-4.5")
}
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "heartq-build"));
        assert!(should_show_model_fingerprint(false, "heartq-4.5"));
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.to_submission(Some("heartq-3".into()), None, None, Some(5));
        assert_eq!(
