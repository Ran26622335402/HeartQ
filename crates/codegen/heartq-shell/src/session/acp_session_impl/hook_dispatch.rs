...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> heartq_hooks::runner::RunContext<'_> {
        heartq_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[heartq_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, heartq_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: heartq_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            heartq_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                heartq_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    heartq_telemetry::events::HookOutcome::Success,
                ),
                heartq_hooks::result::HookRunResult::Blocked {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    heartq_telemetry::events::HookOutcome::Blocked,
                ),
                heartq_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    heartq_telemetry::events::HookOutcome::Error,
                ),
                heartq_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: heartq_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
