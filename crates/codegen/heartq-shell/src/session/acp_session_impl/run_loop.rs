...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<heartq_mcp::servers::McpClientEvent>();
        {
    ...
    // ─── Curator idle scheduler ─────────────────────────────────────────
    // A deterministic maintenance pass (stale → archive → reactivate)
    // that runs periodically based on `CuratorConfig` and persisted
    // `CuratorState`.
    let curator_cfg = {
        let user_cfg = crate::config::load_effective_config().ok();
        let section = user_cfg.as_ref().and_then(|v| v.get("curator"));

        // CuratorConfig default is enabled=false; override to align with
        // the "startup auto-enable" requirement.
        let mut cfg = heartq_agent::curator::CuratorConfig::default();
        cfg.enabled = section
        ...
        if let Some(s) = section {
            if let Some(v) = s.get("interval_hours").and_then(|v| v.as_integer()) {
                cfg.interval_hours = v.max(1) as u32;
            }
            if let Some(v) = s.get("min_idle_hours").and_then(|v| v.as_float()) {
                cfg.min_idle_hours = v.max(0.0);
            } else if let Some(v) = s.get("min_idle_hours").and_then(|v| v.as_integer()) {
                cfg.min_idle_hours = (v as f64).max(0.0);
            }
            if let Some(v) = s.get("stale_after_days").and_then(|v| v.as_integer()) {
                cfg.stale_after_days = v.max(0) as u32;
            }
            if let Some(v) = s.get("archive_after_days").and_then(|v| v.as_integer()) {
                cfg.archive_after_days = v.max(0) as u32;
            }
        }
        cfg
    };
    if curator_cfg.enabled {
        let curator_check_interval = std::time::Duration::from_secs(60);
        let mut interval = tokio::time::interval(curator_check_interval);
        interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
        let session_for_curator = session.clone();
        let curator_cfg_for_task = curator_cfg.clone();
        tokio::task::spawn_local(async move {
            loop {
                interval.tick().await;
                let now = std::time::SystemTime::now();
                let state_path = curator_cfg_for_task
                    .skills_root
                    .join(".curator_state.json");
                let state = heartq_agent::curator::CuratorState::load(&state_path);
                if matches!(
                    curator_cfg_for_task.should_run_now(&state, None, now),
                    heartq_agent::curator::ShouldRun::Yes
                ) {
                    let report = heartq_agent::curator::run_curator_review(
                        &curator_cfg_for_task,
                    );
                    let msg = format!("Curator idle pass: {}", report.summary);
                    let _ = session_for_curator
    ...
    loop {
        tokio::select! {
            ...
            heartq_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : heartq_telemetry::memory_log::TARGET,
...
if let Some(timeout) = session.idle_flush_timeout { idle_flush_sleep
            ...
            heartq_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            body_bytes_after, }) => {
            heartq_telemetry::unified_log::info("shell.image_budget", Some(session
...
if let Some(tx) = respond_to { let _ =
            ...
            .fire_hook(heartq_hooks::event::HookEventName::SessionEnd, None,
            heartq_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            heartq_hooks::dispatcher::dispatch_non_blocking(& registry,
            heartq_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            ...
            heartq_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            // ─── T4: LLM curator consolidation hook (best-effort) ────────
            // Runs on session end, then maps parseable directives into
            // `skill_manage` operations (delete + edit).
            if curator_cfg.enabled {
                let candidates = heartq_agent::curator::collect_consolidation_candidates(
                    &curator_cfg.skills_root,
                );
                if !candidates.is_empty() {
                    let session_for_llm = session.clone();
                    let llm_out = heartq_agent::curator::run_llm_curator_review(
                        &candidates,
                        |prompt| {
                            let session = session_for_llm.clone();
                            async move {
                                let sampling_client = session
                                ...
                                let request = ConversationRequest {
                                    items: vec![
                                        ConversationItem::system(
                                            "You are the HeartQ skill curator.",
                                        ),
                                        ConversationItem::user(prompt),
                                    ],
                                    model: Some(model),
                                    x_heartq_conv_id: Some(format!(
                                        "llm-curator-{}",
                                        uuid::Uuid::new_v4()
                                    )),
                                    x_heartq_req_id: Some(format!(
                                        "xai-llm-curator-{}",
                                        uuid::Uuid::new_v4()
                                    )),
                                    x_heartq_session_id: Some(session_id),
                                    x_heartq_agent_id: Some(
                                        heartq_telemetry::id::agent_id(),
                                    ),
                                    ..Default::default()
                                };
                    ...
                    if let Ok(text) = llm_out {
                        ...
                        for line in text.lines() {
                            ...
                            if let Some(rest) = lower
                                ...
                                .map(|r| trimmed["- archive:".len()..].trim())
                            {
                                let token = rest
                                    .trim_matches('`')
                                    .trim_matches('"')
                            ...
                            if let Some(rest) = lower
                                .strip_prefix("- merge:")
                                .map(|r| trimmed["- merge:".len()..].trim())
                            {
                                // Expected: `<from1>,<from2> into <target>`
                                let lower_rest = rest.to_lowercase();
                                if let Some(into_pos) = lower_rest.find(" into ") {
                                    let from_part = &rest[..into_pos];
                                    let target_part =
                                        rest[into_pos + " into ".len()..].trim();
                        ...
                        if !archives.is_empty() || !merges.is_empty() {
                            tracing::info!(
                                llm_curator_archives = archives.len(),
                                llm_curator_merges = merges.len(),
                                "LLM curator directives parsed"
                            );
                        }

                        if curator_cfg.dry_run {
                            tracing::info!(
                                "LLM curator dry_run=true; skipping skill_manage mutations"
                            );
                        } else {
                            // Apply archives (delete skills).
                            for skill_name in archives {
                                let _ =
                                    heartq_tools::skill_manager::skill_manage(
                                        "delete",
                                        &skill_name,
                                        None,
                                        None,
                            ...
                            // Apply merges:
                            // - edit target's SKILL.md by appending a merge marker
                            // - delete source skills
                            for (froms, target) in merges {
                                let target_md = curator_cfg
                                    .skills_root
                                    .join(&target)
                                    .join("SKILL.md");
                                if let Ok(mut existing) =
                                    std::fs::read_to_string(&target_md)
                                {
                                    existing =
                        ...
                        tracing::warn!(
                            error = ?llm_out,
                            "LLM curator review failed; skipping directives"
                        );
                    }
                }
            }

            // ─── T7: Learning graph snapshot (best-effort) ───────────────
            if !session.startup_hints.is_subagent {
                let mut memory_paths = Vec::new();
                if let Some(storage) = session.memory.storage() {
                    memory_paths.push(storage.workspace_memory_file());
                    memory_paths.push(storage.global_memory_file());
                }
                match crate::session::helpers::learning_graph::write_learning_graph(
                    &curator_cfg.skills_root,
                    memory_paths,
                ) {
                    Ok(path) => {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            target: heartq_telemetry::memory_log::TARGET,
                            error = %e,
                            "LEARNING_GRAPH: write failed"
                        );
                    }
                }
            }

            let telem = session.memory
...
{ let
            ...
            heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::SessionEnded
            ...
            false; heartq_telemetry::unified_log::info("shell.task_wake.gate_cleared",
...
if let Some(ref tp)
            ...
            .push_capped(heartq_tools::implementations::heartq_build::task::types::MonitorEventNotification
            ...
            heartq_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(heartq_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
...
if let
            ...
            session.fire_hook(heartq_hooks::event::HookEventName::SessionStart, None,
            heartq_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            heartq_hooks::dispatcher::dispatch_non_blocking(& registry,
            heartq_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(heartq_hooks::event::HookEventName::SessionEnd, None,
            heartq_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            heartq_hooks::dispatcher::dispatch_non_blocking(& registry,
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[heartq_sampling_types::ConversationItem],
    turn_number: usize,
