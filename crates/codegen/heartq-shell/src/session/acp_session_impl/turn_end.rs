...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            heartq_workspace::session::git::get_worktree_info(cwd),
            heartq_workspace::session::git::get_branch(cwd),
        );
        ...
        let params = heartq_workspace::session::git::GitHeadChanged {
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<heartq_tools::implementations::heartq_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use heartq_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> heartq_hooks::event::StopFailureKind {
        use heartq_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &heartq_agent::plugins::install_registry::InstallError,
    ) -> String {
