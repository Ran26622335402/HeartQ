...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// HeartQ-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as heartq edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &heartq_tools::implementations::heartq_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        heartq_tools::normalization::merge_tool_meta(
            &toolset,
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => heartq_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && heartq_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(heartq_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(heartq_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            heartq_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            heartq_hooks::event::HookEventName::PostToolUseFailure,
                            heartq_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    heartq_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    heartq_hooks::event::HookEventName::PostToolUse,
                    heartq_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                ...
                .await;
            let (ext_file_path, ext_parameters) = if heartq_telemetry::external::is_active() {
            ...
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(heartq_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                heartq_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                heartq_hooks::event::HookEventName::PreToolUse,
                None,
                heartq_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    heartq_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let heartq_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                heartq_workspace::permission::AccessKind::Read(p) => (
                    heartq_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                heartq_workspace::permission::AccessKind::Edit(p) => {
                    (heartq_telemetry::events::AccessKind::Edit, p.clone())
                }
                heartq_workspace::permission::AccessKind::Bash(cmd) => {
                    (heartq_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                heartq_workspace::permission::AccessKind::Grep { path, glob } => (
                    heartq_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                heartq_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (heartq_telemetry::events::AccessKind::Mcp, name.clone())
                }
                heartq_workspace::permission::AccessKind::WebFetch(u) => {
                    (heartq_telemetry::events::AccessKind::Web, u.clone())
                }
                heartq_workspace::permission::AccessKind::WebSearch(q) => {
                    (heartq_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let edit_path_context = matches!(&access_kind, AccessKind::Edit(_)).then(|| {
                heartq_workspace::permission::types::EditPathContext {
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (heartq_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    heartq_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    heartq_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    heartq_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        heartq_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        heartq_hooks::event::HookEventName::PermissionDenied,
                        heartq_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    pub(super) async fn request_plan_approval(
        ...
        heartq_tools::implementations::heartq_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use heartq_tools::implementations::heartq_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                heartq_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        heartq_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
    ...
    /// `heartq_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use heartq_telemetry::enums::PrCreationSource;
        use heartq_tools::util::git_detect;
        match &result.output {
            heartq_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    heartq_telemetry::session_ctx::log_event(
                        heartq_telemetry::events::PrMerged {},
                    );
                }
            }
            heartq_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: heartq_tools::util::git_detect::PrRef,
        source: heartq_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            heartq_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                heartq_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let heartq_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                heartq_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | heartq_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            heartq_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            heartq_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`heartq_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: heartq_sampler::SamplingEvent,
    ) {
        use heartq_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == heartq_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                heartq_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = heartq_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use heartq_tools::types::ToolInput;
    use heartq_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use heartq_tools::implementations::heartq_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use heartq_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// HeartQ edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn heartq_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "heartq tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use heartq_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use heartq_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use heartq_tools::implementations::heartq_build::exit_plan_mode::ExitPlanModeExtResponse;
...
mod wait_interrupt_tests {
    ...
    use heartq_tools::types::output::ToolOutput;
