...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &heartq_tools::implementations::heartq_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        // Meta-skill auto-trigger (and clarify resume) before slash rewrite so
        // matching runs against the raw user text.
        if !self.startup_hints.is_subagent && !origin.is_synthetic() {
            let pending = self.pending_meta_run.borrow_mut().take();
            let outcome = crate::session::helpers::meta_auto_trigger::run_meta_auto_trigger(
                &original_prompt_text,
                pending,
                self.meta_skill_auto_trigger_enabled,
            );
            *self.pending_meta_run.borrow_mut() = outcome.pending_run_id;
            for msg in outcome.notifications {
                self.send_xai_notification(XaiSessionUpdate::HookAnnotation { message: msg })
                    .await;
            }
        }
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = heartq_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        heartq_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    heartq_telemetry::session_ctx::log_event(
                        heartq_telemetry::events::SlashCommandUsed {
                            command: sk.name.clone(),
                            args_provided: !sk.args.is_empty(),
                        },
                    );
                    heartq_telemetry::session_ctx::log_event(
                        heartq_telemetry::events::SkillDispatched {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        heartq_telemetry::session_ctx::log_event(
                            heartq_telemetry::events::PluginUsed {
        ...
        heartq_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        heartq_telemetry::session_ctx::begin_prompt_id();
        let origin = super::super::PromptOrigin::from_prompt_id(prompt_id);
        let mut chunk_meta = serde_json::Map::new();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = heartq_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || heartq_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = heartq_telemetry::events::PromptSubmitted {
            ...
            heartq_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(heartq_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        let prompt_text_for_hook = user_message.clone();
        {
            ...
            // Phase A.5: snapshot user_message before the match arm below
            // moves it into `ConversationItem::*`. We use the snapshot for the
            // memory prefetch query (raw text is the best semantic signal;
            // we deliberately avoid user_chat.text() which mixes in images
            // and tool_result attachments).
            let prefetch_query = user_message.clone();
            ...
            // ─── Phase A.5: prefetch persisted memory into this turn ───
            //
            // Before pushing the user's message, ask the MemoryManager
            // (if attached) for context that's relevant to this query. The
            // result is a fenced `<memory-context>` block injected as a
            // system item ahead of the user message so the model sees
            // relevant prior-session knowledge without us mutating the
            // user's text.
            //
            // We deliberately use the raw user text rather than
            // `user_chat.text()` so attachments/images don't pollute the
            // semantic query. The block is empty-skipped when memory is
            // disabled or no results match.
            if !prefetch_query.trim().is_empty() {
                match self
                    .chat_state_handle
                    .get_prefetch_memory(prefetch_query.clone())
                    .await
                {
                    Ok(ctx) if !ctx.trim().is_empty() => {
                        let bytes = ctx.len();
                        let memory_item = ConversationItem::system(ctx);
                        self.chat_state_handle.push_user_message(memory_item);
                        tracing::debug!(
                            session_id = % self.session_info.id.0,
                            bytes,
                            "TURN_PREFETCH: injected memory context"
                        );
                    }
                    Ok(_) => {
                        tracing::trace!(
                            session_id = % self.session_info.id.0,
                            "TURN_PREFETCH: empty result (no relevant memory)"
                        );
                    }
                    Err(e) => {
                        tracing::warn!(
                            session_id = % self.session_info.id.0,
                            error = % e,
                            "TURN_PREFETCH: failed (continuing without)"
                        );
                    }
                }
            }
        ...
        self.dispatch_hook(
            heartq_hooks::event::HookEventName::UserPromptSubmit,
            heartq_hooks::event::HookPayload::UserPromptSubmit {
        ...
        match &result {
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    ...
                    self.dispatch_hook(
                        heartq_hooks::event::HookEventName::StopFailure,
                        heartq_hooks::event::HookPayload::StopFailure {
                            error: heartq_hooks::event::StopFailureKind::InvalidRequest,
                ...
                .await;
                heartq_telemetry::session_ctx::log_event(
                    heartq_telemetry::events::TurnCompleted {
                        outcome: heartq_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                heartq_telemetry::session_ctx::log_event(
                    heartq_telemetry::events::TurnCompleted {
                        outcome: heartq_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                heartq_telemetry::session_ctx::log_session_event(
                    heartq_telemetry::events::ApiError {
                ...
                heartq_telemetry::session_ctx::log_event(
                    heartq_telemetry::events::TurnCompleted {
                        outcome: heartq_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
    ...
    pub(super) async fn mark_subagent_usage_not_applied(&self, prompt_id: Option<&str>) -> bool {
        ...
        use heartq_tools::implementations::heartq_build::task::types::{
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = heartq_tools::implementations::heartq_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = heartq_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = heartq_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(crate) async fn handle_turn_end(&self, turn_succeeded: bool) {
        ...
        // ─── Phase A.5: sync the just-completed turn to persistent memory ───
        //
        // Fires after every turn (success or failure). We extract the last
        // user query + final assistant text from the conversation and ship
        // them to the MemoryManager, which decides whether to write them
        // to session memory and what insights to extract. Errors are
        // logged but never propagate — memory is best-effort.
        //
        // Skipped when `GROK_MEMORY` is unset AND `memory.flush.enabled`
        // is off (no manager attached).
        let last_user = self
            .chat_state_handle
        ...
        if !last_user.is_empty() && !last_assistant.is_empty() {
            match self
                .chat_state_handle
                .sync_turn_to_memory(last_user.clone(), last_assistant.clone())
                .await
            {
                Ok(()) => {
                    tracing::debug!(
                        session_id = % self.session_info.id.0,
                        user_bytes = last_user.len(),
                        asst_bytes = last_assistant.len(),
                        "TURN_SYNC: persisted to memory"
                    );
                }
                Err(e) => {
                    tracing::warn!(
                        session_id = % self.session_info.id.0,
                        error = % e,
                        "TURN_SYNC: failed (memory not updated)"
                    );
                }
            }
        }

        // ─── Background Review (skills/memory proposals) ──────────────
        //
        // Fires after turn sync (best-effort) so background heuristics can
        // see the same "user query + final assistant text" pair.
        if !self.startup_hints.is_subagent {
            use heartq_agent::background_review::ReviewAction;
            ...
            let action = self
                .background_reviewer
                .borrow_mut()
                .review_turn(&turn_text);

            match action {
                ReviewAction::NoOp => {}
                ReviewAction::ProposeSkillPatch { skill_name, hint } => {
                    let msg = format!(
                        "后台回顾建议：考虑 patch/改进技能 `{}`。提示：{}",
                        skill_name, hint
                    );
                ...
                ReviewAction::ProposeMemoryNote { content } => {
                    if let Some(storage) = self.memory.storage() {
                        // Store as a workspace-scoped note so later dream-enhanced
                        // passes can see it.
                        let note = format!(
                            "## Background Review\n\n{}\n",
                            content.trim()
                        );
                        if let Err(e) = storage.append_to_memory(
                            ...
                            tracing::warn!(
                                session_id = %self.session_info.id.0,
                                error = %e,
                                "BACKGROUND_REVIEW: failed to append memory note"
                            );
                        } else {
                            let msg =
                                "后台回顾已写入 MEMORY.md（workspace 作用域）".to_string();
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use heartq_tools::types::memory_backend::MemoryBackend as _;
        ...
        tracing::info!(
            target : heartq_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        heartq_telemetry::session_ctx::log_event(
            heartq_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<heartq_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        self.maybe_apply_model_router().await;
        self.chat_state_handle
        ...
        loop {
            ...
            request.x_heartq_session_id = Some(self.session_info.id.to_string());
            request.x_heartq_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_heartq_agent_id = Some(heartq_telemetry::id::agent_id());
            if request.x_heartq_deployment_id.is_none() {
                request.x_heartq_deployment_id = crate::managed_config::resolve_deployment_id(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                heartq_telemetry::session_ctx::log_event(
                    heartq_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(heartq_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    heartq_sampling_types::ConversationItem::Assistant(_) => {
