...
fn drop_cli_catchall_allows(
    rules: Vec<heartq_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<heartq_workspace::permission::types::PermissionRule>,
    Vec<heartq_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if heartq_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
fn resolve_background_review_config(
    raw_cfg: Option<&toml::Value>,
) -> heartq_agent::background_review::BackgroundReviewConfig {
    let section = raw_cfg.and_then(|v| v.get("background_review"));
    let enabled = section
        .and_then(|s| s.get("enabled"))
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    let interval_turns = section
        .and_then(|s| s.get("interval_turns"))
...
fn resolve_meta_skill_auto_trigger_enabled(raw_cfg: Option<&toml::Value>) -> bool {
    raw_cfg
        .and_then(|v| v.get("meta_skill_auto_trigger"))
        .and_then(|s| s.get("enabled"))
        .and_then(|v| v.as_bool())
        .unwrap_or(true)
}

fn resolve_auto_propose_enabled(raw_cfg: Option<&toml::Value>) -> bool {
    raw_cfg
        .and_then(|v| v.get("auto_propose"))
        .and_then(|s| s.get("enabled"))
        .and_then(|v| v.as_bool())
        .unwrap_or(true)
}
...
#[cfg(test)]
mod hermes_opensquilla_autostart_config_tests {
    use super::{
        resolve_auto_propose_enabled, resolve_background_review_config,
        resolve_meta_skill_auto_trigger_enabled,
    };

    #[test]
    fn defaults_enable_background_review_meta_and_auto_propose() {
        let bg = resolve_background_review_config(None);
        assert!(bg.enabled);
        assert_eq!(bg.interval_turns, 5);
        assert!(resolve_meta_skill_auto_trigger_enabled(None));
        assert!(resolve_auto_propose_enabled(None));
    }

    #[test]
    fn toml_can_disable_autostart_flags() {
...
"#,
        ...
        .unwrap();
        let bg = resolve_background_review_config(Some(&raw));
        assert!(!bg.enabled);
        assert_eq!(bg.interval_turns, 9);
        assert!(!resolve_meta_skill_auto_trigger_enabled(Some(&raw)));
        assert!(!resolve_auto_propose_enabled(Some(&raw)));
    }
}
...
mod cli_catchall_drop_tests {
    ...
    use heartq_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use heartq_workspace::permission::rules::parse_permission_rule;
    use heartq_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    attribution_callback: Option<heartq_sampler::SharedAttributionCallback>,
    ...
    preloaded_skills: Option<Vec<heartq_tools::implementations::skills::types::SkillInfo>>,
    compat: CompatConfig,
    ...
    web_search_sampling_config: Option<heartq_sampler::SamplerConfig>,
    web_fetch_config: heartq_tools::implementations::heartq_build::web_fetch::WebFetchConfig,
    image_gen_config: heartq_tools::implementations::heartq_build::image_gen::ImageGenConfig,
    video_gen_config: heartq_tools::implementations::heartq_build::video_gen::VideoGenConfig,
    app_builder_deployer_config: heartq_tools::implementations::heartq_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: heartq_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<heartq_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<heartq_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<heartq_workspace::permission::PermissionHandle>,
    api_key_provider: Option<heartq_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<heartq_hooks::discovery::HookRegistry>>,
    workspace_ops: heartq_workspace::WorkspaceOps,
    cli_permission_rules: Vec<heartq_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn heartq_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        heartq_tools::implementations::heartq_build::scheduler::types::SchedulerHandle,
    >,
    ...
    heartq_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(heartq_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            heartq_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = heartq_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("heartq: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        heartq_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(heartq_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if heartq_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    heartq_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn heartq_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            heartq_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        heartq_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            heartq_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("HEARTQ_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "HEARTQ_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = heartq_sampling_types::SamplingConfig {
    ...
    let task_completion_reservations =
        heartq_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        heartq_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        heartq_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        heartq_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn heartq_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn heartq_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn heartq_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(heartq_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(heartq_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || heartq_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                heartq_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                heartq_telemetry::enums::PermissionMode::Auto
            } else {
                heartq_telemetry::enums::PermissionMode::Ask
            },
    ...
    let compaction_policy = heartq_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        heartq_tools::implementations::heartq_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn heartq_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : heartq_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : heartq_telemetry::memory_log::TARGET, removed,
        ...
        // ─── Phase A.5: late-bind MemoryManager to the chat-state actor ───
        //
        // The memory backend (and hence the MemoryManager) is only available
        // *after* the embedding pipeline + auth credentials have been set up.
        // The chat-state actor, however, is spawned earlier in this function
        // (before we know whether memory will succeed). We therefore attach
        // the manager via the `SetMemoryManager` late-bind command.
        //
        // Until this command lands, the actor's `GetPrefetchMemory` /
        // `SyncTurnToMemory` commands return empty placeholders. Gated by
        // `GROK_MEMORY` env var / `memory.enabled` config so the off-by-default
        // path stays zero-cost.
        let memory_enabled = std::env::var("GROK_MEMORY")
            .map(|v| matches!(v.as_str(), "1" | "true" | "TRUE" | "yes" | "on"))
            .unwrap_or(false)
            || memory_config.as_ref().is_some_and(|mc| mc.flush.enabled);
        if memory_enabled {
            let turn_sync: std::sync::Arc<dyn heartq_memory::TurnSync> =
                std::sync::Arc::new(heartq_memory::StorageTurnSync::new(
                    storage.clone(),
                    session_info.id.to_string(),
                ));
            let manager = std::sync::Arc::new(
                heartq_memory::MemoryManager::new(
                    backend.clone(),
                    heartq_memory::MemoryManagerConfig::default(),
                )
                .with_turn_sync(turn_sync),
            );
            let _ = chat_state_handle.attach_memory_manager(manager.clone());
            tracing::info!(
                target : heartq_telemetry::memory_log::TARGET,
                "MEMORY_INIT: MemoryManager attached to ChatStateActor \
                 (prefetch + StorageTurnSync now active)"
            );
        } else {
            tracing::debug!(
                target : heartq_telemetry::memory_log::TARGET,
                "MEMORY_INIT: MemoryManager NOT attached (GROK_MEMORY unset, \
                 flush disabled) — actor returns empty placeholders"
            );
        }
        let mc = memory_config.as_ref();
        let total_chunks = storage.total_chunk_count();
        heartq_telemetry::session_ctx::log_event(
            heartq_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        heartq_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let resolved_task_output =
        heartq_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        heartq_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        heartq_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<heartq_tools::implementations::heartq_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::HeartQTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::HeartQWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::HeartQPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = heartq_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = heartq_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<heartq_sampler::SamplingEvent>();
    let sampler_handle = heartq_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<heartq_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<heartq_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = heartq_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        heartq_tools::implementations::heartq_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        heartq_tools::implementations::heartq_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use heartq_tools::implementations::heartq_build::workflow::WorkflowLaunchAck;
                ...
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / HEARTQ_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
    ...
    let raw_effective_config = crate::config::load_effective_config().ok();
    let mut effective_config = raw_effective_config
        .as_ref()
        .and_then(|raw| crate::agent::config::Config::new_from_toml_cfg(raw).ok())
        .unwrap_or_default();
    effective_config.remote_settings = remote_settings.clone();
    let background_review_cfg =
        resolve_background_review_config(raw_effective_config.as_ref());
    let meta_skill_auto_trigger_enabled =
        resolve_meta_skill_auto_trigger_enabled(raw_effective_config.as_ref());
    let auto_propose_enabled =
        resolve_auto_propose_enabled(raw_effective_config.as_ref());
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        model_router: {
            let mut cfg = heartq_model_router::ModelRouterConfig::default();
            if let Ok(root) = crate::config::load_effective_config() {
                if let Some(table) = root.get("model_router") {
                    if let Ok(parsed) =
                        table.clone().try_into::<heartq_model_router::ModelRouterConfig>()
                    {
                        cfg = parsed;
                    }
                }
            ...
            if matches!(
                std::env::var("HEARTQ_MODEL_ROUTER").as_deref(),
                Ok("1") | Ok("true") | Ok("TRUE")
            ) {
                cfg.enabled = true;
            }
            heartq_model_router::ModelRouterState::new(cfg)
        },
        session_start: std::time::Instant::now(),
        inference_idle_timeout: Duration::from_secs(inference_idle_timeout_secs),
        max_turns,
        max_retries: heartq_sampler::resolve_max_retries(max_retries),
        ...
        dream_check_timeout: memory_config
            ...
            .filter(|&s| s > 0)
            .map(std::time::Duration::from_secs),
        background_reviewer: std::cell::RefCell::new(
            heartq_agent::background_review::BackgroundReviewer::new(
                background_review_cfg,
            ),
        ),
        meta_skill_auto_trigger_enabled,
        pending_meta_run: std::cell::RefCell::new(None),
        auto_propose_enabled,
        tool_result_pruning_enabled: session_pruning_config.enabled,
        ...
        vcs_kind: {
            ...
            match heartq_workspace::session::git::discover_git_root(root) {
                heartq_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    heartq_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => heartq_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(heartq_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(heartq_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            heartq_tools::implementations::heartq_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                heartq_tools::implementations::heartq_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : heartq_telemetry::memory_log::TARGET, files = files.len(),
                ...
                heartq_telemetry::session_ctx::log_event(
                    heartq_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use heartq_tools::implementations::heartq_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use heartq_tools::implementations::heartq_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = heartq_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            heartq_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        heartq_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, heartq_agent::AgentBuildError>,
    >();
