...
fn partition_rules_by_scope(
    files: Vec<heartq_agent::prompt::agents_md::AgentConfigFile>,
    heartq_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_root: Option<&std::path::Path>,
) -> (
    Vec<heartq_agent::prompt::user_message::RuleEntry>,
    Vec<heartq_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = heartq_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use heartq_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[heartq_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_heartq_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn heartq_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.heartq/rules/project.md"),
    ...
    #[test]
    fn nested_heartq_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/heartq/rules/global.md"),
            file("/custom/heartq/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/heartq/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/heartq"),
            &[],
            Some(Path::new("/custom/heartq/worktrees/repo")),
        );
        assert_eq!(paths(&user), vec!["/custom/heartq/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
...
/// end for heartq ordering), so a failed offload never
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(heartq_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: heartq_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use heartq_agent::prompt::agents_md::read_agents_config_with_paths;
        use heartq_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use heartq_workspace::file_system::{git_status_short, jj_status};
        use heartq_workspace::session::git::VcsKind;
    ...
    ) -> Vec<heartq_agent::prompt::user_message::McpServerEntry> {
        use heartq_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = heartq_sampler::SamplingClient::new(sampler_config).map_err(|e| {
