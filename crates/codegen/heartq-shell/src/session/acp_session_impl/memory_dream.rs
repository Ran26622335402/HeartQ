...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &heartq_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                heartq_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                heartq_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        heartq_telemetry::session_ctx::log_event(
            heartq_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        // Enhanced dream path: collect session snippets, rank, quarantine-filter.
        const DREAM_ENHANCED_ENABLED: bool = true;
        let enhanced_promotions = if DREAM_ENHANCED_ENABLED {
            use heartq_memory::{
                collect_candidates_from_sessions, filter_and_rank_candidates, QuarantineRules,
            };
            let evidence = collect_candidates_from_sessions(sessions_dir, &dream_msg.processed_stems);
            let rules = QuarantineRules::default();
            filter_and_rank_candidates(&evidence, &rules, 0.0, Some(10))
        } else {
            Vec::new()
        };
        ...
        let dream_path = if matches!(result.status, DreamStatus::Completed { .. }) {
            ...
            if DREAM_ENHANCED_ENABLED && !enhanced_promotions.is_empty() {
                use heartq_memory::apply_promotion_candidates;
                let applied = apply_promotion_candidates(storage, &enhanced_promotions);
                tracing::info!(
                    target: heartq_telemetry::memory_log::TARGET,
                    applied,
                    candidates = enhanced_promotions.len(),
                    "{log_prefix}: applied enhanced dream promotions"
                );
            }

            // Auto-propose meta-skills from co-occurring session skills.
            if self.auto_propose_enabled {
                let mut skill_names: Vec<String> = self
                    .tool_bridge_handle()
                    .get_announced_skill_names()
                    .await
                    .into_iter()
                    .collect();
                if let Some(active) = self.active_skill.lock().clone() {
                    if !skill_names.iter().any(|s| s == &active) {
                        skill_names.push(active);
                    }
                }
                if let Some(proposal) =
                    heartq_memory::auto_propose::propose_from_cooccurrence(&skill_names)
                {
                    match heartq_memory::auto_propose::save_proposal(&proposal) {
                        Ok(path) => {
                            tracing::info!(
                                target: heartq_telemetry::memory_log::TARGET,
                                path = %path.display(),
                                title = %proposal.title,
                                "{log_prefix}: auto-propose saved"
                            );
                        ...
                        Err(e) => {
                            tracing::warn!(
                                target: heartq_telemetry::memory_log::TARGET,
                                error = %e,
                                "{log_prefix}: auto-propose save failed"
                            );
                        }
                    }
                }
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            x_heartq_conv_id: Some(format!("dream-{}", uuid::Uuid::new_v4())),
            x_heartq_req_id: Some(format!("xai-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: heartq_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                items,
                model: Some(model),
                x_heartq_conv_id: Some(format!("flush-{}", uuid::Uuid::new_v4())),
                x_heartq_req_id: Some(format!("xai-flush-{}", uuid::Uuid::new_v4())),
                x_heartq_session_id: Some(session_id.clone()),
                x_heartq_agent_id: Some(heartq_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: heartq_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        heartq_telemetry::session_ctx::log_event(
            heartq_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => heartq_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => heartq_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => heartq_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => heartq_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::MemoryFlushed {
    ...
    /// LLM call using the `heartq-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let request = ConversationRequest {
            items,
            tools: vec![],
            model: Some("heartq-build".to_owned()),
            temperature: Some(0.3),
            max_output_tokens: Some(1024),
            ..Default::default()
        };

        let request_id = heartq_sampler::RequestId::random();
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    heartq_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                heartq_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = heartq_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = heartq_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                heartq_sampler::collect_response(events).await
            }
        };
