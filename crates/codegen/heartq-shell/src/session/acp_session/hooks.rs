...
//! both matched by the agent ([`heartq_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use heartq_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use heartq_telemetry::events::ClientHookGateOutcome;
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            heartq_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    fn client_gate_responses<'a>(
        ...
        let default_timeout =
            if envelope.hook_event_name.traits().gate == heartq_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                heartq_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    heartq_telemetry::session_ctx::log_event(
                        heartq_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> heartq_hooks::dispatcher::StopDispatchResult {
        use heartq_hooks::result::HookRunResult;

        let mut out = heartq_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed)) in ordered {
            ...
            let detail = heartq_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                heartq_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use heartq_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    heartq_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use heartq_hooks::matcher::HookMatcher;
