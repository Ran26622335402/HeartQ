...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, heartq_home: &Path) -> Result<()> {
        ...
        if session_dir_at(heartq_home, &self.source_cwd, &self.session_id)
            == session_dir_at(heartq_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(heartq_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(heartq_home);
...
pub(super) fn read(heartq_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(heartq_home, session_id);
...
pub(super) fn write(
    ...
    journal
        .validate(heartq_home)
        .map_err(WriteFailure::NotCommitted)?;
    let path = journal_path(heartq_home, &journal.session_id);
...
pub(super) fn relocation_dir(heartq_home: &Path) -> PathBuf {
    heartq_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(heartq_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(heartq_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(heartq_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    heartq_home
        .join("sessions")
        .join(heartq_config::encode_cwd_dirname(cwd))
        .join(session_id)
}

pub(super) fn sync_namespace(heartq_home: &Path) -> Result<()> {
    let dir = relocation_dir(heartq_home);
