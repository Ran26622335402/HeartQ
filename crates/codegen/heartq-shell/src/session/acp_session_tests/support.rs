...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> heartq_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> heartq_agent::Agent {
    use heartq_tools::implementations::heartq_build::update_goal::UpdateGoalTool;
    use heartq_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// HeartQ-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_heartq_build_agent_with_todo() -> heartq_agent::Agent {
    use heartq_tools::implementations::heartq_build::todo::TodoWriteTool;
    use heartq_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> heartq_agent::Agent {
    use heartq_tools::implementations::heartq_build::enter_plan_mode::EnterPlanModeTool;
    use heartq_tools::implementations::heartq_build::exit_plan_mode::ExitPlanModeTool;
    use heartq_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<heartq_tools::registry::types::ToolConfig>,
) -> heartq_agent::Agent {
    test_agent_from_config(
        heartq_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        heartq_agent::AgentDefinition::default_heartq_build(),
        std::sync::Arc::new(heartq_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: heartq_agent::AgentDefinition,
    backend: std::sync::Arc<dyn heartq_tools::computer::types::TerminalBackend>,
) -> heartq_agent::Agent {
    use heartq_tools::computer::local::LocalFs;
    use heartq_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("heartq-test"),
    ...
    heartq_agent::Agent::new(
        definition,
        heartq_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        heartq_agent::ReminderPolicy::default(),
        heartq_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    let cwd = heartq_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(heartq_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    tool_context.task_completion_reservations =
        Some(heartq_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(heartq_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        permissions: heartq_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
