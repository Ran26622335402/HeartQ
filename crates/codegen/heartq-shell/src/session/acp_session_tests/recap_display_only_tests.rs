        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_heartq_req_id.starts_with("xai-recap-"),
                        "req id: {}",
                        artifact.x_heartq_req_id
                    );
...
/// shape, so it also covers the laxer heartq ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use heartq_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use heartq_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // heartq backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = heartq_sampling_types::build_messages_request(&req);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.api_backend = heartq_sampling_types::ApiBackend::Responses;
            ...
            let conv_id = recap_req
                .header("x-heartq-conv-id")
                .expect("recap must send x-heartq-conv-id");
            assert!(
