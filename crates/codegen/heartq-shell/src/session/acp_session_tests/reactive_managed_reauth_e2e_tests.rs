...
//! handshake against rmcp 2.1 in `heartq-mcp/tests/repro_sse_flood.rs`): a
...
use heartq_mcp::servers::{ClientStateKind, HttpConfig, McpClient};

const MANAGED: &str = "heartq_com_testconnector";
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use heartq_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `heartq_com_*` tools enter entry-B.
