...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), HeartQComConfig::default()));
    am.hot_swap(HeartQAuth {
...
/// attribution.
fn auth_error() -> heartq_sampler::SamplingErrorInfo {
    heartq_sampler::SamplingErrorInfo {
        kind: heartq_sampler::SamplingErrorKind::Auth,
...
fn model_not_found_error() -> heartq_sampler::SamplingErrorInfo {
    heartq_sampler::SamplingErrorInfo {
            kind: heartq_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model heartq-build does not exist or your team does not have access".into(),
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `heartq logout`, got: {msg}"
            );
            assert!(
                msg.contains("heartq login"),
                "hint must mention `heartq login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> heartq_sampler::SamplingErrorInfo {
    heartq_sampler::SamplingErrorInfo {
            kind: heartq_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.heartq.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=heartq-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = heartq_sampler::SamplingErrorKind::Api;
