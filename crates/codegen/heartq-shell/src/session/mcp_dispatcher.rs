...
//!
//! Receives [`heartq_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`heartq_mcp::liveness`]),
//! - the [`heartq_mcp::servers::HeartQClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`heartq_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `heartq_com_` prefix)
    /// or `local` (user `.heartq/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`heartq_mcp::servers::ClientStateKind::Ready`]
...
/// [`heartq_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit heartq.com"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && heartq_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
mod tests {
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "heartq_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `heartq_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(classify_source("heartq_com_linear"), McpServerSource::Managed);
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use heartq_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("heartq_com_slack"), // managed
