...
/// sanitizer shared with `heartq-mcp` so the advertised folder matches disk.
...
mod tests {
    ...
    fn sanitize_replaces_unsafe_chars_and_never_empty() {
        assert_eq!(
            sanitize_descriptor_segment("heartq_com_linear"),
            "heartq_com_linear"
        );
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.heartq/projects/enc/mcps");
