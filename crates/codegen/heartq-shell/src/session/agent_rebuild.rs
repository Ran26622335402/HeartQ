...
//! [`heartq_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`heartq_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`heartq_agent::Agent`] owns an [`heartq_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`heartq_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use heartq_agent::error::AgentBuildError;
use heartq_agent::prompt::context::PromptAudience;
use heartq_agent::prompt::skills::SkillsConfig;
use heartq_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use heartq_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use heartq_tools::implementations::heartq_build::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<heartq_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<heartq_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use heartq_tools::implementations::heartq_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(heartq_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(heartq_tools::types::resources::SchedulerBackgroundLoops(
                *scheduler_background_loops,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(heartq_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use heartq_tools::implementations::heartq_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            heartq_tools::computer::local::LocalTerminalBackend::new_local(
                heartq_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: heartq_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(heartq_tools::types::tool::ToolKind::Task)
            .expect("HeartQBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("HeartQBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_heartq_build())
                    .await
