...
use heartq_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's HeartQ Build flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use heartq_tools::implementations::skills::skill::{
...
/// `heartq-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn listing(name: &str) -> crate::session::workflow::registry::WorkflowListing {
        crate::session::workflow::registry::WorkflowListing {
            ...
            when_to_use: None,
            source: "project",
            path: Some(format!(".heartq/workflows/{name}.rhai")),
        }
    }
