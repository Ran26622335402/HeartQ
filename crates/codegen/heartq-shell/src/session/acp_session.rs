...
use heartq_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use heartq_agent::prompt::skills::SkillsConfig;
use heartq_sampler::SamplerConfig as SamplingConfig;
use heartq_sampling_types::truncate_bytes;
...
use heartq_tools::types::compat::CompatConfig;
use heartq_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use heartq_workspace::file_system::CodebaseIndexManager;
use heartq_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use heartq_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
mod mcp;
#[path = "acp_session_impl/model_router.rs"]
mod model_router;
...
impl heartq_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        heartq_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// `heartq-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<heartq_sampler::SharedAttributionCallback>,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<heartq_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<heartq_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-heartq-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<heartq_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    /// Per-turn multi-model smart router (OpenSquilla-compatible c0–c3).
    pub(crate) model_router: heartq_model_router::ModelRouterState,
    ...
    /// BackgroundReviewer: low-frequency background proposals based on recent turns.
    /// Kept in-session so it can learn turn cadence across multiple prompts.
    pub(crate) background_reviewer: std::cell::RefCell<
        heartq_agent::background_review::BackgroundReviewer,
    >,
    /// When true, user prompts are matched against meta-skill triggers and
    /// auto-executed via `SkillManagerExecutor` (see `[meta_skill_auto_trigger]`).
    pub(crate) meta_skill_auto_trigger_enabled: bool,
    /// Pending meta-skill run awaiting user clarification (`AwaitingUser`).
    pub(crate) pending_meta_run: std::cell::RefCell<Option<uuid::Uuid>>,
    /// When true, dream-enhanced completion may emit meta-skill proposals
    /// (see `[auto_propose]`).
    pub(crate) auto_propose_enabled: bool,
    /// When true, full-replace compaction runs Hermes `ToolResultPruner`
    /// on summarizer input (see `[compaction.pruning]`).
    pub(crate) tool_result_pruning_enabled: bool,
    ...
    pub(crate) agent: std::cell::RefCell<heartq_agent::Agent>,
    ...
    /// Kill-switch (`HEARTQ_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<heartq_tools::implementations::heartq_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, heartq_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<heartq_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<heartq_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<heartq_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `heartq-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: heartq_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`heartq_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`heartq_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: heartq_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == heartq_tools::implementations::heartq_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == heartq_tools::implementations::heartq_build::workflow::WORKFLOW_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<heartq_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `heartq prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &heartq_agent::PromptContext) {
...
) -> Option<heartq_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use heartq_tools::types::output::{MCPOutput, ToolOutput};
    use heartq_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl heartq_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use heartq_tools::registry::types::ToolConfig;
    use heartq_tools::tool_taxonomy::TOOL_META_KEY;
    use heartq_workspace::permission::PermissionCommand;
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "HeartQBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                assert_eq!(t["namespace"], "heartq_build");
...
mod managed_gateway_tool_tests {
    ...
    use heartq_tools::types::output::{MCPOutput, ToolOutput};
    use heartq_tools::types::tool::{ToolKind, ToolNamespace};
    use heartq_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<heartq_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
