...
use heartq_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `HEARTQ_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("HEARTQ_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("HEARTQ_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("HEARTQ_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG HEARTQ_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                heartq_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            heartq_telemetry::session_ctx::log_event(
                heartq_telemetry::events::AutoCompactSuppressed {
    ...
    async fn run_compact_inner(
        ...
        trigger: heartq_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        if crate::session::helpers::compaction_gates::anti_thrash_blocked() {
            tracing::warn!(
                target: "heartq_compaction_gates",
                "COMPACTION_GATES: anti-thrash blocked compaction attempt"
            );
            return Err(acp::Error::internal_error()
                .data("compaction blocked: anti-thrash threshold exceeded"));
        }
        ...
        let trigger_str = match trigger {
            heartq_telemetry::events::CompactionTrigger::Manual => "manual",
            heartq_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = heartq_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            heartq_hooks::event::HookEventName::PreCompact,
            heartq_hooks::event::HookPayload::PreCompact {
        ...
        let effective_tool_defs: Vec<heartq_sampling_types::ToolDefinition> = self
        ...
        let compaction_tools: Vec<heartq_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(heartq_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<heartq_sampling_types::HostedTool> =
            if use_backend_search {
        ...
        let auto_trigger = matches!(trigger, heartq_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = heartq_compaction::FullReplaceConfig {
            max_attempts: max_retries,
            retry_delay_secs,
            sampling_timeout_secs: 0,
        };
        let mut request_turns = simplified_messages.clone();
        if self.tool_result_pruning_enabled {
            let prune_cfg = heartq_compaction::ToolPrunerConfig::default();
            let prune_stats =
                crate::session::helpers::tool_result_prune::prune_conversation_tool_results(
                    &mut request_turns,
                    &prune_cfg,
                );
            if prune_stats.deduplicated > 0
                || prune_stats.truncated > 0
                || prune_stats.summarized > 0
            {
                tracing::info!(
                    session_id = %self.session_info.id.0,
                    deduplicated = prune_stats.deduplicated,
                    truncated = prune_stats.truncated,
                    summarized = prune_stats.summarized,
                    tokens_saved = prune_stats.tokens_saved,
                    "COMPACTION: tool-result pruning applied to summarizer input"
                );
            }
        }
        ...
        while compact_summary.is_none() {
            match heartq_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(heartq_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(heartq_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(heartq_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            heartq_telemetry::session_ctx::log_event(
                                heartq_telemetry::events::CompactionRetryDegraded {
                                    trigger,
        ...
        let conversation = self.chat_state_handle.get_conversation().await;
        let gate = crate::session::helpers::compaction_gates::apply_post_summary_gates(
            &conversation,
            &generate_session_compact,
            heartq_compaction::SafetyMode::BestEffort,
        );
        if matches!(
            gate.decision,
            heartq_compaction::CompactionDecision::Disabled
                | heartq_compaction::CompactionDecision::EmergencyEphemeral
        ) {
            return Err(acp::Error::internal_error().data(format!(
                "compaction blocked by safety gate ({})",
                gate.decision
            )));
        }
        // Prefer gate-augmented summary (obligation carry-forward).
        let generate_session_compact = gate.summary;
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                heartq_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                heartq_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use heartq_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn heartq_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn heartq_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : heartq_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        heartq_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            heartq_hooks::event::HookEventName::PostCompact,
            heartq_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &heartq_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                heartq_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<heartq_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use tokio::sync::mpsc;
    use heartq_paths::AbsPathBuf;
    use heartq_workspace::file_system::MockFs;
    use heartq_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        let chat_state_handle = xai_chat_state::ChatStateActor::spawn(
            vec![],
            heartq_sampling_types::SamplingConfig {
        ...
        SessionActor {
            ...
            meta_skill_auto_trigger_enabled: false,
            pending_meta_run: std::cell::RefCell::new(None),
            auto_propose_enabled: false,
            tool_result_pruning_enabled: false,
            ...
            vcs_kind: heartq_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: heartq_sampler::SamplerHandle::noop(),
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use heartq_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): HeartQ Build usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("HeartQ Build usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> heartq_sampler::SamplingErrorInfo {
        heartq_sampler::SamplingErrorInfo {
            kind: heartq_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = heartq_sampler::SamplingErrorInfo {
                    kind: heartq_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
