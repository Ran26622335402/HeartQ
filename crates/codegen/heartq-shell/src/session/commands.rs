...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: heartq_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `heartq-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-heartq-model-override` header and merges
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<heartq_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.heartq/hooks`,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<heartq_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<heartq_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<heartq_tools::types::TaskSnapshot>>>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<heartq_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// tool-free model call (default `heartq-build-0.1` when available via
    ...
    /// `heartq-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<heartq_sampling_types::conversation::ConversationItem>>>,
    },
