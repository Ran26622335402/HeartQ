...
//!   under `~/.heartq/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.heartq/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.heartq/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.heartq/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.heartq/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.heartq/`.
...
/// `CleanAll` mode. Placed under `~/.heartq/worktree_pool/<instance_id>/<uuid>/`.
