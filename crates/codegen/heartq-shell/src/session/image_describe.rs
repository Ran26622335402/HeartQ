...
//! content blocks, the session calls a vision-capable HeartQ model
...
use heartq_tools::util::truncate::truncate_middle;
...
impl ImageDescribeCache {
    ...
    pub async fn get_or_describe(
        &self,
        client: heartq_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
mod tests {
    use super::*;
    use heartq_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![heartq_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
