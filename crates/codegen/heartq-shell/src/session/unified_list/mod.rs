...
/// Env lane (desktop `HEARTQ_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`HEARTQ_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source: "local".into(),
            model_id: Some("heartq-build".into()),
            num_messages: 7,
    ...
    fn xai_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::HeartQAuth {
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = heartq_test_support::EnvGuard::set(
            "HEARTQ_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = heartq_test_support::EnvGuard::unset("HEARTQ_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = heartq_test_support::EnvGuard::set("HEARTQ_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = heartq_test_support::EnvGuard::set("HEARTQ_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::HEARTQ_CHAT_MODE_ENV;
        let _chat_off = heartq_test_support::EnvGuard::unset(HEARTQ_CHAT_MODE_ENV);
        let _desktop_off =
            heartq_test_support::EnvGuard::unset("HEARTQ_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                heartq_test_support::EnvGuard::set("HEARTQ_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = heartq_test_support::EnvGuard::set(HEARTQ_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = heartq_test_support::EnvGuard::unset(HEARTQ_CHAT_MODE_ENV);
        ...
        {
            let _on = heartq_test_support::EnvGuard::set(HEARTQ_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = heartq_workspace::session::git::normalize_repo_url(this_repo).unwrap();
