// Re-export all types from the standalone heartq-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use heartq_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `heartq-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `heartq-sampler` crate can reference them without a circular
// dep on `heartq-shell`.

// Tests for the types now live in heartq-sampling-types crate.

use heartq_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `heartq-sampling-types` or
/// `heartq-tools`) so neither of those crates needs to depend on
