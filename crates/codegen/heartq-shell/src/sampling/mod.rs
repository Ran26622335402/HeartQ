...
pub use self::types::*;
pub use heartq_sampler::ApiBackend;
pub use heartq_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// heartq-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `heartq-sampler` crate. We re-export the public surface here so
...
// `heartq_sampler::*`. The shell-side `sampling::client::Config`
