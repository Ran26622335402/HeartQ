...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.heartq/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** heartq process
    /// sharing the same `~/.heartq` (the writer may also be this process;
...
/// Watches `~/.heartq/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.heartq/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.heartq/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let heartq_home_buf = heartq_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(heartq_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(heartq_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(heartq_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(heartq_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %heartq_home.display(),
                    error = %e,
                    "failed to watch heartq home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.heartq/config.toml` so the loop above
        // watches `<cwd>/.heartq/`), the call below installs a
        ...
        tracing::info!(
            heartq_home = %heartq_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.heartq/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.heartq/config.toml` trigger a
    ...
    /// `<cwd>/.heartq/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.heartq/` does not yet
/// exist at session-open time, the `.heartq/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.heartq/` followed by a
/// write to `<cwd>/.heartq/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let heartq_dir = cwd.join(".heartq");
    if let Err(e) = debouncer
        .watcher()
        .watch(&heartq_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .heartq directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let heartq_dir = cwd.join(".heartq");
    if let Err(e) = debouncer.watcher().unwatch(&heartq_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .heartq directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.heartq/` hasn't
...
fn discovery_change_for_path(path: &Path) -> Option<DiscoveryChange> {
    if path.file_name().is_some_and(|name| name == ".heartq") {
...
/// recursively: `heartq_home` (`~/.heartq`, or `$HEARTQ_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.heartq` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, heartq_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, heartq_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".heartq", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, heartq_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(heartq_home) {
        return true;
    }
...
impl ProjectDiscoveryWatcher {
    pub fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let project_heartq = project_root.join(".heartq");
        let workflows = project_heartq.join("workflows");
        let (tx, rx) = mpsc::unbounded_channel();
        let project_heartq_for_events = project_heartq.clone();
        let mut debouncer =
            new_filtered_debouncer(SKILLS_DEBOUNCE, move |res: DebounceEventResult| {
                ...
                for event in events
                    .iter()
                    .filter(|event| event.path.starts_with(&project_heartq_for_events))
                {
        ...
        let initial = if project_heartq.is_dir() {
            project_heartq.clone()
        } else {
            project_root.clone()
        };
        ...
        let refresh_dirs = vec![
            (project_heartq, RecursiveMode::NonRecursive),
            (
                project_root.join(".heartq").join("skills"),
                RecursiveMode::Recursive,
            ),
            (
                project_root.join(".heartq").join("commands"),
...
impl SkillsFileWatcher {
    ///
    /// Uses [`collect_skill_config_dirs`](heartq_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let heartq_home = heartq_tools::util::heartq_home::heartq_home();
        ...
        let dirs_to_watch = heartq_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &heartq_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &heartq_home) {
        ...
        let mut refresh_dirs = vec![(heartq_home.join("workflows"), RecursiveMode::NonRecursive)];
        if let Some(cwd) = cwd {
            let project_root = crate::session::workflow::registry::project_root(cwd);
            let project_heartq = project_root.join(".heartq");
            let parent_watch = if project_heartq.is_dir() {
                project_heartq.clone()
            } else {
                project_root
            };
            ...
            refresh_dirs.push((project_heartq.clone(), RecursiveMode::NonRecursive));
            refresh_dirs.push((project_heartq.join("workflows"), RecursiveMode::NonRecursive));
        }
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only heartq_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_heartq_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let heartq_home = home.join(".heartq");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &heartq_home, Some(home));

        // heartq_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&heartq_home));
        ...
        assert!(!g(&home.join("repo").join(".heartq")));
    }

    /// Regression for the ~/.heartq inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.heartq/worktrees/`) must not
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let heartq = Path::new("/tmp/project/.heartq");
        assert_eq!(
            discovery_change_for_path(heartq),
            Some(DiscoveryChange::Skills),
            "first .heartq creation must take the broader skills reload path"
        );
        assert_eq!(
            discovery_change_for_path(&heartq.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&heartq.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&heartq.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<heartq_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another heartq process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.heartq/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let heartq_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_heartq = cwd.path().join(".heartq");
        fs::create_dir_all(&project_heartq).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_heartq.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_heartq.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.heartq/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.heartq/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_heartq = new_cwd.path().join(".heartq");
        fs::create_dir_all(&project_heartq).unwrap();
        fs::write(project_heartq.join("config.toml"), "").unwrap();
