//! `heartq mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = heartq_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.heartq/config.toml".to_string(),
...
// ── Managed (heartq.com) server discovery ─────────────────────────

const MANAGED_SOURCE_LABEL: &str = "heartq.com";
...
/// Discover managed `heartq_com_*` servers if the user has xAI auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let auth_manager = Arc::new(crate::auth::AuthManager::new(&heartq_home, heartq_com_config));
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `heartq login`"),
    };
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use heartq_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `heartq mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `heartq mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `heartq mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `heartq mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
