//! AcpTerminalAdapter: implements `heartq-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates heartq-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (heartq-desktop) spawns with `shell: true`
...
/// Wraps heartq-shell's ACP gateway to satisfy heartq-tools' TerminalBackend.
