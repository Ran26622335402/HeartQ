...
/// Resolved absolute path to bash. On Unix uses the `heartq_config` shell
/// resolution cascade (`$HEARTQ_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        heartq_config::shell::unix_shell_path(heartq_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `heartq-tools`.
pub use heartq_tools::util::pager_env;
...
/// heartq-pager which renders its own UI and doesn't need raw ANSI codes).
///
