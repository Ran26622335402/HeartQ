//! heartq.com chat-product model catalog (`POST /rest/modes`) — the models
//! heartq-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const HEARTQ_WEB_URL: &str = "https://heartq.com";
...
pub enum ChatModelsError {
    #[error("no heartq.com credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("HEARTQ_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("HEARTQ_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("HEARTQ_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| HEARTQ_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid heartq.com bearer — deliberately NOT `is_xai_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.heartq_com_config().token_header.clone(),
            )
