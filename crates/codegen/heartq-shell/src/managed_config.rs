...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    heartq_config::MANAGED_CONFIG_FILENAME,
    heartq_config::REQUIREMENTS_FILENAME,
    heartq_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    heartq_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            heartq_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", heartq_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            heartq_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: HeartQAuth) -> Option<HeartQAuth> {
...
/// config is a heartq.com feature with one heartq.com auth.
fn read_active_team_auth() -> Option<HeartQAuth> {
    let home = crate::util::heartq_home::heartq_home();
...
/// files (TUI tick vs `heartq login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `heartq setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `HEARTQ_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("HEARTQ_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `heartq-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = heartq_config::signed_policy::stored_envelope_nonce(
        &crate::util::heartq_home::heartq_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            heartq_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
/// Override with `HEARTQ_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("HEARTQ_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** HeartQBuildDeployment UUID (the id
...
/// Resolve deployment key from `HEARTQ_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (heartq setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("HEARTQ_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.heartq/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<HeartQAuth>,
...
/// read-only `heartq setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if heartq_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(heartq_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        heartq_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(heartq_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            heartq_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(heartq_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &heartq_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<heartq_config::signed_policy::SignatureEnvelope> {
    use heartq_config::signed_policy::now_unix;
    ...
    let claim = match heartq_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &heartq_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `heartq login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<HeartQAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `heartq setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(HeartQAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `heartq setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !heartq_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::heartq_home::heartq_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            heartq_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `heartq setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.heartq`.
    Installed,
...
/// Result of `heartq setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `heartq setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `heartq setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `heartq setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `heartq setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && heartq_config::signed_policy::verification_active()
...
/// Run the `heartq setup` sync for the current principal. The caller must check
