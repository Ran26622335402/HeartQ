...
/// two-function shape in `heartq-tools` (F29).
...
/// `heartq-tools` — existing items keep their prior content when
...
/// binary. Wraps a `heartq_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: heartq_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: heartq_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_heartq_conv_id: Some(format!("trace-classifier-{}", uuid::Uuid::new_v4())),
        x_heartq_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_heartq_session_id: session_id_str,
        x_heartq_agent_id: Some(heartq_telemetry::id::agent_id()),
...
pub struct RunArgs {
    ...
    /// HeartQ-home directory containing the `auth.json` to consult as
    /// the third API-key fallback. Defaults to
    /// `heartq_shell::util::heartq_home::heartq_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub heartq_home: Option<PathBuf>,
}
...
/// provider command (if `HEARTQ_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, heartq_home: &Path) -> Result<String> {
    ...
    if let Some(key) = non_interactive_auth_key(heartq_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set XAI_API_KEY, or run `heartq login` to populate \
         <heartq-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `heartq_home`, install the
...
/// explicit `heartq_home` so `--heartq-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `heartq_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(heartq_home: &Path) -> Result<Option<String>> {
    use crate::auth::{AuthError, AuthManager, HeartQComConfig};
    ...
    let config = HeartQComConfig::default();
    let auth_provider_command = config.auth_provider_command.clone();
    let manager = std::sync::Arc::new(AuthManager::new(heartq_home, config));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `heartq login` to re-authenticate, \
...
) -> Result<heartq_sampler::SamplingClient> {
    let default_home;
    let heartq_home_path: &Path = match heartq_home {
        Some(p) => p,
        None => {
            default_home = crate::util::heartq_home::heartq_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, heartq_home_path).await?;
    let config = heartq_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..heartq_sampler::SamplerConfig::default()
    };
    heartq_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use heartq_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        let conv_id = req.x_heartq_conv_id.as_deref().expect("conv id");
        ...
        assert_eq!(req.x_heartq_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_heartq_req_id.as_deref().expect("req id");
        ...
        assert_eq!(
            req.x_heartq_agent_id.as_deref(),
            Some(heartq_telemetry::id::agent_id().as_str())
        );
    }
    ...
    /// OIDC scope (the same scope `heartq login` writes today and
    /// `AuthManager` reads). `auth_mode: api_key` skips the refresh
    /// path entirely — useful for "plain key, no refresh wanted"
    /// fixtures.
    fn write_auth_json(heartq_home: &Path, key: &str) {
        let scope = crate::auth::HeartQComConfig::default().auth_scope();
        ...
        std::fs::write(heartq_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(heartq_home: &Path) {
        std::fs::write(heartq_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(heartq_home: &Path, key: &str) {
        let scope = crate::auth::HeartQComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(heartq_home: &Path, key: &str) {
        let scope = crate::auth::HeartQComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        let heartq_home = tmp.path();
        // No auth.json at all → AuthManager sees nothing on disk.
        write_empty_auth_json(heartq_home);
        ...
        assert!(
            resolve_api_key(None, heartq_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), heartq_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), heartq_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), heartq_home)
                .await
                .expect("ok"),
            "from-flag",
        );
        ...
        assert_eq!(
            resolve_api_key(None, heartq_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), heartq_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), heartq_home).await.expect("ok"),
            "from-env",
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let heartq_home = tmp.path();
        write_auth_json(heartq_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, heartq_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("XAI_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, heartq_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, heartq_home)
                .await
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(heartq_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, heartq_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(heartq_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, heartq_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let heartq_home = tmp.path();
        write_fresh_oidc_auth_json(heartq_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, heartq_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let heartq_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(heartq_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, heartq_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// * `HEARTQ_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `HEARTQ_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.heartq/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `HEARTQ_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    async fn run_rejects_missing_trace_path() {
        ...
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            heartq_home: Some(tmp.path().to_path_buf()),
        };
