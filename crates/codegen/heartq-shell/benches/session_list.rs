...
//!
//! Run: `cargo bench -p heartq-shell --bench session_list`
...
use heartq_shell::session::persistence::Summary;
use heartq_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use heartq_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = heartq_shell::util::heartq_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = heartq_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = heartq_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(heartq_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("HEARTQ_HOME", home.path());
    }
    assert_eq!(heartq_shell::util::heartq_home::heartq_home(), home.path());
