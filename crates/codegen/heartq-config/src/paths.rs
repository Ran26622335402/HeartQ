//! Filesystem locations for heartq config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static HEARTQ_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user heartq directory (`~/.heartq`, canonicalized) used when
/// `HEARTQ_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`heartq_home()`] is the default without duplicating the computation.
///
...
/// `~/.heartq/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `xai_fast_worktree::db::resolve_heartq_home` (deliberately standalone crate).
pub fn default_heartq_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".heartq")
}

/// Per-user config directory: `$HEARTQ_HOME` or `~/.heartq`. Created if needed.
pub fn heartq_home() -> PathBuf {
    HEARTQ_HOME
        .get_or_init(|| {
            let heartq_home = if let Ok(v) = std::env::var("HEARTQ_HOME") {
                PathBuf::from(v)
            } else {
                default_heartq_home()
            };
...
/// The user-global heartq home, but only when one genuinely resolves: `Some` when
/// `$HEARTQ_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`heartq_home()`], this never falls back to a cwd-relative `.heartq`, so callers
/// that *scan* user-global heartq resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.heartq` tree for the user-global one when no home resolves.
pub fn user_heartq_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("HEARTQ_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(heartq_home)
}

/// Canonical heartq application path: `$HEARTQ_HOME/bin/heartq` (Unix) or `heartq.exe` (Windows).
pub fn heartq_application() -> PathBuf {
    heartq_application_in(&heartq_home())
}

/// [`heartq_application`] under an explicit home instead of `$HEARTQ_HOME`.
pub fn heartq_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "heartq.exe" } else { "heartq" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/heartq/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/heartq"))
    } else {
        None
    }
}
...
/// `heartq_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    heartq_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = heartq_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_heartq_home_has_no_verbatim_prefix() {
        ...
        let home = default_heartq_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".heartq"));
    }
