//! Config file loading for HeartQ.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/heartq/managed_config.toml`
//! 2. `$HEARTQ_HOME/managed_config.toml`
//! 3. `$HEARTQ_HOME/config.toml`
//! 4. `$HEARTQ_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/heartq/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.heartq`, admin-forced) — macOS only
//!
...
pub use paths::{
    ...
    default_heartq_home, encode_cwd_dirname, ensure_sessions_cwd_dir, heartq_application,
    heartq_application_in, heartq_home, sessions_cwd_dir, system_config_dir, user_heartq_home,
};
pub use validation::{
