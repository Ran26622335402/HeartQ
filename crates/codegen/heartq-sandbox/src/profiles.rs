...
//! `off`. Custom profiles via `~/.heartq/sandbox.toml` or `.heartq/sandbox.toml`.
...
/// Load sandbox config from `~/.heartq/sandbox.toml` and `.heartq/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.heartq/sandbox.toml
    let global_path = heartq_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.heartq/sandbox.toml (additive only)
    let project_path = workspace.join(".heartq").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&heartq_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".heartq").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.heartq/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.heartq/sandbox.toml or .heartq/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
"#;
    ...
    fn enxio_device_file_is_skipped_but_directory_is_not() {
        ...
        let dir = std::env::temp_dir().join(format!("heartq-sbx-test-{}", std::process::id()));
