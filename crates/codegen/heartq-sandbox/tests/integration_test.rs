//! Integration tests for heartq-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use heartq_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use heartq_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!heartq_sandbox::should_restrict_child_network());
}
