//! Leader-mode (`heartq agent --leader stdio`) test harness.
//!
...
/// (version-skew) test. Falls back to [`heartq_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "HEARTQ_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`heartq_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "HEARTQ_BINARY_CLIENT";
...
/// (`HEARTQ_BINARY_LEADER`, else the shared [`heartq_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`HEARTQ_BINARY_CLIENT`,
/// else the shared [`heartq_binary`] resolution).
...
/// A `heartq agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&heartq_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export HEARTQ_* vars
            // (e.g. HEARTQ_LEADER_SOCKET pointing at a REAL leader on this
            // machine). env_clear + explicit allowlist guarantees the test
            // can never touch a leader outside its sandbox home.
            .env_clear()
            .env("PATH", std::env::var("PATH").unwrap_or_default())
            .env("HOME", home)
            .env("HEARTQ_HOME", home.join(".heartq"))
            ...
            .env("HEARTQ_LEADER_SOCKET", home.join(".heartq").join("leader.sock"))
            .env("HEARTQ_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("HEARTQ_XAI_API_BASE_URL", server.url())
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("HEARTQ_TELEMETRY_ENABLED", "false")
            .env("HEARTQ_FEEDBACK_ENABLED", "false")
            .env("HEARTQ_TRACE_UPLOAD", "false")
            .env("HEARTQ_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.heartq/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "heartq_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".heartq").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".heartq").join("leader.log")).unwrap_or_default()
}
