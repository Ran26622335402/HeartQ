...
fn local_heartq_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("heartq-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_heartq_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build heartq-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build heartq-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "heartq-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve heartq binary: `HEARTQ_BINARY` env (CI) or a locally built `heartq-pager` binary.
pub fn heartq_binary() -> PathBuf {
    if let Ok(path) = std::env::var("HEARTQ_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "HEARTQ_BINARY does not exist: {}", p.display());
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_heartq-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_heartq_binary_path();
    ensure_local_heartq_binary(&binary);
    binary
}
...
/// Point heartq at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox heartq on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit HEARTQ_HOME every spawned child shares the real
        // `%USERPROFILE%\.heartq` — test 1's models_cache.json (which embeds
        ...
        .env("HEARTQ_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("HEARTQ_XAI_API_BASE_URL", mock_url)
        ...
        .env("HEARTQ_DISABLE_AUTOUPDATER", "1");
}
