//! ACP stdio clients for testing heartq sessions end-to-end: the typed
//! [`HeartQStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// Spawn `heartq agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`HeartQStdioClient`])
...
/// kill-list so a test can still set e.g. `HEARTQ_DEBUG_LOG=1` explicitly.
...
/// Drives `heartq agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`HeartQStdioClient`] (`ClientSideConnection`,
