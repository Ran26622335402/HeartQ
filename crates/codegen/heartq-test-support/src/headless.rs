//! Headless mode (`heartq -p`) test runner.
//!
//! Runs the heartq binary as a subprocess with the mock server, captures output.
...
use crate::env::{heartq_binary, test_env_cmd_tokio};
...
pub struct HeadlessResult {
    ...
    pub stderr: String,
    pub timed_out: bool,
    /// Wall time of the heartq invocation; logged so CI timeout budgets can be
    /// tuned against observed durations.
    pub elapsed: Duration,
}

/// Timeout for one headless heartq invocation: 60 seconds, multiplied by
/// [`crate::scaled`]'s `HEARTQ_TEST_TIMEOUT_SCALE`.
fn headless_timeout() -> Duration {
    crate::scaled(Duration::from_secs(60))
}

/// Run `heartq` with the given args against the mock server, bounded by
...
pub async fn run_headless_with_cmd(mut cmd: tokio::process::Command) -> HeadlessResult {
    ...
    let mut child = cmd
        .spawn()
        .unwrap_or_else(|e| panic!("failed to spawn heartq binary at {}: {e}", binary.display()));
    ...
    let (status, timed_out) = match tokio::time::timeout(headless_timeout(), child.wait()).await {
        Ok(result) => (
            result.unwrap_or_else(|e| {
                panic!("failed to wait for heartq binary {}: {e}", binary.display())
            }),
            false,
        ),
        Err(_) => {
            ...
            let status = child.wait().await.unwrap_or_else(|e| {
                panic!(
                    "failed to kill timed out heartq binary {}: {e}",
                    binary.display()
                )
            });
            (status, true)
    ...
    eprintln!("[harness-timing] headless heartq run: {elapsed:?} (timed_out={timed_out})");
