//! Memory-system configuration value types, extracted from heartq-shell
...
//! `heartq-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.heartq/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.heartq/memory/` are scanned:
