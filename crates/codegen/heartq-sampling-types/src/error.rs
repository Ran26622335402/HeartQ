...
//!
//! TODO: Move from heartq-shell/src/sampling/error.rs
...
/// Flat error from the HeartQ proxy/gateway: `{"code": "...", "error": "..."}`.
#[derive(Debug, Deserialize)]
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!("HeartQ is temporarily unavailable. Please try again in a moment. (HTTP {code}).")
        }
        // Cloudflare edge codes (origin down / connect fail / timeout / …).
        code @ 520..=524 => {
            format!(
                "Connection to HeartQ timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
...
<html lang="en-US">
<head><title>heartq.com | 524: A timeout occurred</title></head>
