//! Headless markdown analysis sharing HeartQ Build's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `heartq-markdown` so analysis matches what HeartQ
//! Build actually renders 1:1.
//!
//! After parsing, HeartQ applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set HeartQ Build uses to render markdown.
///
...
/// Offset event stream from HeartQ's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with HeartQ Build's options; count elements and flag structural issues.
