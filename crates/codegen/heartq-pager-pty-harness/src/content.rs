...
//! [`MockInferenceServer`] from `heartq-test-support` and provides the
...
pub use heartq_test_support::mock_server::LogEntry;
pub use heartq_test_support::mock_server::MockModelEntry as MockModel;
pub use heartq_test_support::mock_server::StorageUpload;
pub use heartq_test_support::sse;
pub use heartq_test_support::{
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.heartq
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit HEARTQ_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // HEARTQ_HOME is set in the test runner's env).
            ("HEARTQ_HOME".into(), heartq_home),
            ("HEARTQ_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("HEARTQ_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("HEARTQ_PROMPT_SUGGESTIONS".into(), "false".into()),
        ]
    }
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-heartq-turn-idx", "1")
            .header("x-heartq-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
    ...
    /// on the SuperHeartQ upsell screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("HEARTQ_HOME").as_deref(),
            content.home().join(".heartq").to_str()
        );
        assert_eq!(get("HEARTQ_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("HEARTQ_XAI_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("HEARTQ_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("HEARTQ_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("HEARTQ_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("HEARTQ_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("HEARTQ_MAX_RETRIES"), None);
