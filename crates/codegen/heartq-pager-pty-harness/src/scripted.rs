//! Scripted TUI scenario runner for heartq-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$HEARTQ_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&heartq_home)
                .with_context(|| format!("create scenario HEARTQ_HOME {}", heartq_home.display()))?;
            fs::write(heartq_home.join("config.toml"), config_toml)
