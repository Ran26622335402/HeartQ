//! Unified PTY harness for heartq-pager.
//!
...
//!    `heartq-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `HEARTQ_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use heartq_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through heartq's stdout. Used to reproduce the doubled-line class of bugs
    /// where heartq's diff renderer never re-asserts a region it didn't write
