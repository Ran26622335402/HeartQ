//! `pty-bench` — PTY benchmark CLI for `heartq-pager`.
//!
...
//! ```bash
//! cargo bench -p heartq-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/heartq-${VERSION}-linux-x86_64 \
//!   cargo bench -p heartq-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for heartq-pager",
    long_about = None,
)]
struct Cli {
