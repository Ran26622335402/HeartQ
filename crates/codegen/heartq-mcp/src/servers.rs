...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"heartq_com_linear"`).
...
pub struct McpState {
    ...
    /// Persisted to `~/.heartq/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
impl McpState {
    ...
    /// `HeartQClientHandler` cloned during `try_handshake` reads
...
/// MCP initialization strategy. Defined in `heartq-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use heartq_telemetry::enums::McpInitStrategy;
...
if mime_type
        ...
        heartq_telemetry::session_ctx::log_event(heartq_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
/// Uses [`HeartQClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, HeartQClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`HeartQClientHandler`] when the server pushes a notification we
...
pub struct McpClient {
    ...
    /// [`HeartQClientHandler`]** constructed by
...
/// and the [`HeartQClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to heartq-mcp
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: heartq_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, HeartQClientHandler>, McpError> {
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("heartq-shell-{server_name}"),
    ...
    /// Build the [`HeartQClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> HeartQClientHandler {
        HeartQClientHandler {
    ...
    /// references (the [`HeartQClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            heartq_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = heartq_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.heartq/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = heartq_config::heartq_home().join("logs").join("mcp");
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`heartq_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "heartq_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("heartq-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = heartq_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = heartq_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        heartq_telemetry::session_ctx::log_event(
                            heartq_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: heartq_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    heartq_telemetry::instrumentation::timer("mcp_http_auth_discovery");
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = heartq_telemetry::instrumentation::timer("mcp_start_servers");
...
#[derive(Debug)]
pub struct HeartQClientHandler {
...
impl ClientHandler for HeartQClientHandler {
    ...
    // See the [`HeartQClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("heartq_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("HEARTQ_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("heartq_com_FIGMA", "https://example.com/mcp"));
        ...
        assert!(!is_figma_mcp("heartq_com_linear", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_heartq_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = HeartQClientHandler {
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'heartq_com_notion' handshake failed: Auth required, when send initialize request"
        ));
