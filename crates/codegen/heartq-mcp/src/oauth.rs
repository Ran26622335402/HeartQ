...
const MCP_OAUTH_CLIENT_NAME: &str = "HeartQ";
...
// mode disabled, multiple `heartq` invocations).
//
// Layer 1 (cross-process): filesystem lock at $HEARTQ_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    heartq_config::heartq_home().join(format!("mcp_auth_{safe}.lock"))
}
...
mod tests {
    ...
    async fn manager_ready_for_exchange(token_endpoint: String) -> (AuthorizationManager, String) {
        ...
        mgr.configure_client(
            OAuthClientConfig::new("heartq-test-client", "http://127.0.0.1:0/callback")
