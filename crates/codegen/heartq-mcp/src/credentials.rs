...
//!
//! Credentials are stored in `$HEARTQ_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `heartq-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$HEARTQ_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$HEARTQ_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$HEARTQ_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user heartq home (set $HEARTQ_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$HEARTQ_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(heartq_config::user_heartq_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$HEARTQ_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("heartq-mcp-credentials-test");
