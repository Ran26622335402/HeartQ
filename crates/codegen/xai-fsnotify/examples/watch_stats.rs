...
//! startup latency, under either strategy (`HEARTQ_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! HEARTQ_FSNOTIFY_PER_DIR=0 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! HEARTQ_FSNOTIFY_PER_DIR=1 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "heartq holds 55k inotify watches" report).
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("HEARTQ_FSNOTIFY_PER_DIR").unwrap_or_default();
