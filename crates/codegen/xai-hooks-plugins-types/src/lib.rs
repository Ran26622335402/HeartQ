...
//! `heartq-shell` and `heartq-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `heartq-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `heartq-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// HeartQ's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `heartq-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.heartq/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.heartq/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.heartq/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.heartq/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserHeartQ),
            conflict: None,
        };
