...
//! it" buckets.
//!
//! `heartq-sampler` is intentionally decoupled from `heartq-shell`
...
/// tools live in `heartq-tools`
/// (`crates/codegen/heartq-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `heartq_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `heartq_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `heartq-shell/src/auth/manager.rs` to match, otherwise the
