...
//! `heartq_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `heartq_sampling_types::SamplingConfig`
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-heartq-doom-loop-check` header on
