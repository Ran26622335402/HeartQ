...
use heartq_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use heartq_pager::client_identity::PAGER_CLIENT_VERSION;
use heartq_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use heartq_shell::agent::config::Config as AgentConfig;
use heartq_shell::leader::{
...
use heartq_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.heartq_ws_origin {
        config.heartq_com_config.heartq_ws_origin = v.clone();
    }
    if let Some(v) = &args.heartq_ws_url {
        config.heartq_com_config.heartq_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &heartq_pager::app::AgentArgs,
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   HeartQ agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `heartq -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use heartq_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(heartq_telemetry::sampling_log::layer())
        .with(heartq_telemetry::instrumentation::layer())
        .with(heartq_telemetry::hooks_log::layer())
        .with(heartq_telemetry::otel_layer::build_otel_layer(
            heartq_telemetry::otel_layer::OtelClientInfo {
                client_name: "heartq-pager",
                client_version: heartq_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            heartq_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    heartq_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    heartq_telemetry::external::init(
        heartq_shell::agent::config::resolve_external_otel_config(
            heartq_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: heartq_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `heartq setup`: rendering + exit codes only; fetch logic lives in `heartq_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use heartq_shell::managed_config::{self, SetupOutcome};
    if !managed_config::has_principal() {
        eprintln!("No deployment key or team sign-in found.");
        eprintln!();
        eprintln!("To install managed configuration, sign in with a team using `heartq login`,");
        eprintln!("or set a deployment key:");
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export HEARTQ_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:HEARTQ_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  heartq setup");
        eprintln!();
        eprintln!("Or add the key to ~/.heartq/config.toml:");
        eprintln!();
        eprintln!("  [endpoints]");
        eprintln!("  deployment_key = \"<your-key>\"");
        eprintln!();
        eprintln!(
            "If you don't have a deployment key, contact your organization's HeartQ administrator."
        );
        std::process::exit(1);
    }
    if json {
    ...
    match managed_config::run_setup().await {
        ...
        SetupOutcome::Skipped => {
            eprintln!(
                "Managed configuration was not applied this run (another process held the apply lock, or the credential changed during the fetch). Run `heartq setup` again."
            );
        }
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = heartq_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !heartq_shell::util::is_heartq_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a heartq process, removing stale lock");
        ...
        if let Err(e) = heartq_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(heartq_shell::env::HeartQBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, heartq_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = heartq_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = heartq_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "heartq-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&heartq_shell::leader::ControlPayload>,
...
/// Env override for the `heartq workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "HEARTQ_WORKSPACE_COMMAND";
/// Resolution of the `heartq workspace` gate. `Unknown` is kept separate from
...
/// The `HEARTQ_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&heartq_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for heartq on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<heartq_shell::util::config::RemoteSettings> {
    join_early_prefetch(heartq_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`heartq workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `heartq workspace`. Check your \
             network connection (run `heartq login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.heartq_com_config.heartq_ws_url;
    let socket = socket_path_for_ws_url(ws_url);
    LeaderClient::connect(
        socket,
        "heartq-workspace-cli",
        ClientMode::Stdio,
        ClientCapabilities::default(),
    )
    .await
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a heartq session, or run `heartq workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = heartq_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use heartq_shell::auth::ensure_authenticated;
    heartq_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = heartq_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`heartq workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.heartq/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.heartq_com_config,
        false,
        Some("No cached credentials found. Run `heartq login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.heartq_com_config);
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    heartq_telemetry::sentry::flush_on_shutdown();
    heartq_telemetry::otel_layer::shutdown_otel();
    heartq_telemetry::debug_log::flush();
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "heartq: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<heartq_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use heartq_pager::app::signal_handler::next_signal_code;
    ...
    let _otel_guard = heartq_telemetry::otel_layer::otel_guard();
    heartq_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => heartq_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = heartq_shell::agent::models::start_early_prefetch(None);
    heartq_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "HeartQ Build (pager) - v{}",
            heartq_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                heartq_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(heartq_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = heartq_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("heartq: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = heartq_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&heartq_shell::agent::config::RuntimeResolutionContext {
    ...
    let managed_install = is_managed_install(
        ...
        &heartq_shell::util::heartq_home::heartq_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = heartq_shell::leader::LeaderEnvUrls::from(&agent_config.heartq_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = heartq_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            heartq_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(heartq_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = heartq_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = heartq_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `HEARTQ_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `heartq dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `heartq dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `HEARTQ_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !heartq_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.heartq/config.toml and \
             unsetting HEARTQ_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("HEARTQ_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<heartq_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(heartq_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<heartq_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(heartq_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    heartq_shell::heap_profile::install(heartq_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "heartq {}\n",
        heartq_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), channel_label,)
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = heartq_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    if let Some(code) = heartq_pager::app::mermaid_worker::maybe_run_render_subprocess() {
    ...
    heartq_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    heartq_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        heartq_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        heartq_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    heartq_pager::memory_trace::start(
        heartq_shell::util::heartq_home::heartq_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = heartq_config::validate_requirements() {
        eprintln!("Couldn't start HeartQ: {e}");
        eprintln!();
        eprintln!(
            "Update HeartQ to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = heartq_telemetry::sentry::init(heartq_telemetry::sentry::Config {
        client: "heartq-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: heartq_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    heartq_pager::docs::extract_user_guide_docs(&heartq_shell::util::heartq_home::heartq_home());
    xai_crash_handler::install_terminal_restore_only();
    if heartq_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = heartq_shell::util::heartq_home::heartq_home().join("crash");
        if let Some(report) = xai_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("HeartQ crashed during your last session.");
    ...
    let crashed = heartq_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("HEARTQ_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("HEARTQ_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(heartq_shell::agent::chat_modes::HEARTQ_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(heartq_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("HEARTQ_DEBUG_LOG", path);
            std::env::remove_var("HEARTQ_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("HEARTQ_DEBUG_LOG", "1");
        set_if_unset("HEARTQ_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        heartq_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return heartq_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        heartq_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        heartq_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    heartq_shell::config::apply_sandbox(
        None,
    ...
    heartq_shell::http::set_client_name(if is_interactive {
        heartq_workspace::permission::ClientType::HeartQPager
    } else {
        heartq_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        heartq_update::channel_name().unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `heartq-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                heartq_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return heartq_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = heartq_telemetry::otel_layer::otel_guard();
                return heartq_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = heartq_telemetry::otel_layer::otel_guard();
                let config = heartq_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return heartq_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return heartq_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return heartq_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return heartq_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return heartq_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return heartq_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return heartq_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                heartq_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                heartq_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = heartq_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                heartq_shell::auth::run_cli_logout(&config)?;
                heartq_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = heartq_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(heartq_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == heartq_pager::headless::OutputFormat::Plain
        {
            args.output_format = heartq_pager::headless::OutputFormat::Json;
        }
        return heartq_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            heartq_pager::headless::HeadlessOptions {
    ...
    let result = heartq_pager::app::run(args, bg_update_rx).await;
    heartq_sandbox::flush();
    match result {
        Ok(true) => {
            let adopted = bg_update_wait.lock().await.take();
            if finish_update_on_exit(adopted, &update_config).await {
                eprintln!("Update installed. Run `heartq` to start.");
            } else {
                eprintln!("Update did not complete. Run `heartq update` to retry.");
            }
            Ok(())
        }
        Ok(false) => Ok(()),
...
///
/// Prefers awaiting the parked waiter for the background `heartq update` child
...
/// fall back to a fresh blocking `heartq update`, which itself resolves to
...
fn build_update_config() -> UpdateConfig {
    let environment = heartq_shell::env::HeartQBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                heartq_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("HEARTQ_NPM_REGISTRY"))
        .ok()
        .or_else(heartq_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = heartq_shell::config::load_effective_config_disk_only()
        && let Some(ch) = heartq_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("HEARTQ_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<heartq_home>/bin/heartq` resolves to, the
...
/// npm shim hardcodes `~/.heartq`, so a custom `HEARTQ_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, heartq_home: &std::path::Path) -> bool {
    if heartq_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = heartq_config::heartq_application_in(heartq_home);
...
/// Handle `heartq-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
/// After a successful `heartq update`, ask any running leader on this machine that
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in heartq_shell::leader::discover_leaders().await {
        if d.classification != heartq_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match heartq_shell::leader::LeaderClient::connect(
            socket_path,
            "heartq-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(heartq_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(heartq_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn version_output_writer_preserves_channel_aware_contract() {
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("heartq "));
    ...
    fn version_flags_and_doctor_are_distinct_early_intents() {
        let version = PagerArgs::try_parse_from(["heartq", "--version"]).unwrap();
        assert!(version.version);
        assert!(version.command.is_none());
        let short = PagerArgs::try_parse_from(["heartq", "-v"]).unwrap();
        assert!(short.version);
        assert!(short.command.is_none());
        let subcommand = PagerArgs::try_parse_from(["heartq", "version"]).unwrap();
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            heartq_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: heartq_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/heartq-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            heartq_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!heartq_shell::heap_profile::prof_available());
            return;
        }
        assert!(heartq_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(heartq_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(heartq_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        heartq_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_heartq_target() {
        let home =
            std::env::temp_dir().join(format!("heartq-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("heartq")),
            &home
        ));
        ...
        let target = home.join("downloads").join("heartq-1.2.3");
        std::fs::write(&target, b"binary").unwrap();
        std::os::unix::fs::symlink(&target, home.join("bin").join("heartq")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("heartq")),
            &home
        ));
        assert!(is_managed_install(Some(target.clone()), &home));
        let pinned = home.join("bin").join("heartq-9.9.9");
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("HEARTQ_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("HEARTQ_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `heartq dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(HEARTQ_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["heartq", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(HEARTQ_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("HEARTQ_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("HEARTQ_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("HEARTQ_WORKSPACE_COMMAND", "off") };
