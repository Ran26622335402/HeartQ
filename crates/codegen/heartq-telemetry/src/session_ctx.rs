...
//!
//! Extracted from `heartq-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `heartq-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `heartq-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "heartq-shell-",
            EmitterOrigin::Workspace => "heartq-workspace-",
        }
    }
}
...
/// event-name prefix (`heartq-shell-*` vs `heartq-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "heartq-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "heartq-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("heartq-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("heartq-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "heartq-workspace-turn");
    }
