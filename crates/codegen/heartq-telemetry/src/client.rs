...
//!
//! Extracted from `heartq-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`heartq-shell-` /
/// `heartq-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("SuperHeartQ Heavy")
/// and JWT-derived keys ("superheartq_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperHeartQ Heavy" | "superheartq_heavy" => "superheartq_heavy",
        "SuperHeartQ" | "superheartq" => "superheartq",
        "SuperHeartQ Lite" | "superheartq_lite" => "superheartq_lite",
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("HeartQ Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("heartq-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("heartq-shell-turn"), "turn");
        assert_eq!(
            event_value("heartq-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("heartq-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("heartq-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperHeartQ"), "superheartq");
        assert_eq!(normalize_tier("SuperHeartQ Heavy"), "superheartq_heavy");
        assert_eq!(normalize_tier("superheartq_heavy"), "superheartq_heavy");
        ...
        assert_eq!(normalize_tier("SuperHeartQ Lite"), "superheartq_lite");
    ...
    /// (e.g. a future `"heartq-shell-ext-"` origin), an earlier `ALL` entry could
