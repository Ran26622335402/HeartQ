...
//! file logger that writes to `~/.heartq/logs/memory.log`.
//!
//! ## When to use
//!
...
//! HEARTQ_MEMORY_LOG=0 heartq                # disable even when enabled
//! tail -f ~/.heartq/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "HEARTQ_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.heartq/logs/memory.log`. Filters to `xai_memory=trace`.
    /// Set `HEARTQ_MEMORY_LOG=0` to disable, `HEARTQ_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || heartq_home().join("logs").join("memory.log");
