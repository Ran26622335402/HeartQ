...
//!
//! These structs were extracted from `heartq-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `heartq wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
// ---------------------------------------------------------------------------
// Pager events (called from heartq-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when HeartQ Build is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct SuperHeartQUpsellShown {
    pub source: SuperHeartQUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperHeartQUpsellClicked {
    pub source: SuperHeartQUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperHeartQUpsell`]; lets the funnel attribute the click to the
...
pub struct ClipboardCopy {
    ...
    /// An explicit `heartq wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// is lifted — i.e. they subscribed (externally on heartq.com) and came back.
/// This is the actual conversion signal for SuperHeartQ Heavy subscriptions
/// attributed to HeartQ Build: the user saw the gate in HeartQ Build, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to HeartQ Build's upsell surface.
...
/// `heartq-shell-manual_auth` (the `manual_auth` binding gets the `heartq-shell-`
...
telemetry_event!(SuperHeartQUpsellShown, "superheartq_upsell_shown");
telemetry_event!(SuperHeartQUpsellClicked, "superheartq_upsell_clicked");
