...
/// Wire schema version, exported as resource attr `heartq_code.schema.version`.
/// Additive changes (new events/attrs) do not bump it; renames/removals do.
pub const SCHEMA_VERSION: &str = "v1";

/// Meter/logger instrumentation scope name.
pub const SCOPE_NAME: &str = "ai.xai.heartq_code";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "heartq_code.session_start",
            Self::SessionEnd => "heartq_code.session_end",
            Self::UserPrompt => "heartq_code.user_prompt",
            Self::TurnCompleted => "heartq_code.turn_completed",
            Self::ApiRequest => "heartq_code.api_request",
            Self::ApiError => "heartq_code.api_error",
            Self::ToolResult => "heartq_code.tool_result",
            Self::ToolDecision => "heartq_code.tool_decision",
            Self::McpServerConnection => "heartq_code.mcp_server_connection",
            Self::PermissionModeChanged => "heartq_code.permission_mode_changed",
            Self::SkillActivated => "heartq_code.skill_activated",
            Self::PluginLoaded => "heartq_code.plugin_loaded",
            Self::Compaction => "heartq_code.compaction",
            Self::Subagent => "heartq_code.subagent",
            Self::Auth => "heartq_code.auth",
            Self::InternalError => "heartq_code.internal_error",
            Self::ModelSwitched => "heartq_code.model_switched",
            Self::ContextualTip => "heartq_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `heartq_code.session.count`.
    SessionCount,
    /// `heartq_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `heartq_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `heartq_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `heartq_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `heartq_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "heartq_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "heartq_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "heartq_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "heartq_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "heartq_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "heartq_code.error.count";
...
/// Built-in tool names (closed enum surface in `heartq-tools`) that pass
...
/// `SessionHarness` → `heartq_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `heartq_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `heartq_code.session_end`.
...
/// `PromptSubmitted` → `heartq_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `heartq_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `heartq_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `heartq_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `heartq_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `heartq_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `heartq_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `heartq_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `heartq_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `heartq_code.permission_mode_changed`.
...
/// `ContextualTip` → `heartq_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `heartq_code.permission_mode_changed`.
...
/// `SkillDispatched` → `heartq_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `heartq_code.plugin_loaded`.
...
/// `PluginUsed` → `heartq_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `heartq_code.compaction`.
...
/// `CompactionTriggered` → `heartq_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `heartq_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `heartq_code.subagent` (`phase=completed`).
...
/// `InternalError` → `heartq_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `heartq_code.model_switched`.
