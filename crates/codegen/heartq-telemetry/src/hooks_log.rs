...
//! file logger that writes to `~/.heartq/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `heartq_hooks` or `heartq_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! HEARTQ_HOOKS_LOG=1 heartq              # enable, write to ~/.heartq/logs/hooks.log
//! HEARTQ_HOOKS_LOG=/tmp/h.log heartq     # write to custom path
//! HEARTQ_HOOKS_LOG=0 heartq              # explicitly disable
//! tail -f ~/.heartq/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "HEARTQ_HOOKS_LOG";
...
///
/// Writes to `~/.heartq/logs/hooks.log` (or custom path via `HEARTQ_HOOKS_LOG`).
/// Filters to hooks (`heartq_hooks`) and plugins (`heartq_agent::plugins`) targets.
/// Set `HEARTQ_HOOKS_LOG=0` to disable, `HEARTQ_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "heartq_hooks=debug,heartq_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || heartq_home().join("logs").join("hooks.log");
