//! Tracing layer for `target: "sampling_log"` → `~/.heartq/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `HEARTQ_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "HEARTQ_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = heartq_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
