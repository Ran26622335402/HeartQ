...
fn external_stream_end_to_end() {
    ...
    let mut cfg = heartq_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "HEARTQ_EXTERNAL_OTEL" => Some("1".into()),
    ...
    cfg.client = heartq_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    heartq_telemetry::external::init(Some(cfg));
    assert!(heartq_telemetry::external::is_active());
    ...
    assert!(!heartq_telemetry::is_enabled());
    heartq_telemetry::log_event(heartq_telemetry::events::SessionNew {
        session_id: "sess-int-1".into(),
        client_identifier: None,
        client_version: None,
        is_git_repo: true,
        permission_mode: heartq_telemetry::enums::PermissionMode::Ask,
    });
    heartq_telemetry::log_event(heartq_telemetry::events::SessionHarness {
        session_id: "sess-int-1".into(),
    ...
    heartq_telemetry::log_event(heartq_telemetry::events::PromptSubmitted {
    ...
    heartq_telemetry::log_event(heartq_telemetry::events::ModelResponseReceived {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("heartq-cli")),
        "service.name=heartq-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "heartq_code.session_start",
        "heartq_code.user_prompt",
        "heartq_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "heartq_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "heartq_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "heartq_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "heartq_code.token.usage"));
    assert_eq!(
    ...
    heartq_telemetry::external::shutdown();
    assert!(
