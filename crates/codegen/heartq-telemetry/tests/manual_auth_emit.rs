...
//! `heartq-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use heartq_telemetry::client;
use heartq_telemetry::config::{TelemetryConfig, TelemetryMode};
use heartq_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn manual_auth_posts_to_events_endpoint_as_heartq_shell_manual_auth() {
    ...
    heartq_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "heartq-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no heartq-shell-manual_auth POST received"
        );
